...
export default function InventaireProductionPage() {
  ...
  const rows = useMemo(() => data.map((row) => {
    ...
    const countedNumber = counted === undefined ? row.quantite : Number(counted);
    return {
      ...row,
      stockCompte: countedNumber,
      ecart: counted === undefined ? 0 : countedNumber - row.quantite,
    };
  }), [data, counts]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      setCounts({});
      queryClient.invalidateQueries({ queryKey: ['inventaire-production'] });
