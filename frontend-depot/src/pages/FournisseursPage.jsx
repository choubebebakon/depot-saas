        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="bg-slate-800 rounded-xl p-4 space-y-3">
            ...
            <div>
              ...
              <input type="number" min="0" value={montantPaye} onChange={e => setMontantPaye(e.target.value)}
                className="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500" />
            </div>
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex gap-3">
          <button onClick={() => setModalNouvel(true)}
            className="bg-slate-700 hover:bg-slate-600 text-white font-bold px-4 py-2.5 rounded-xl text-sm transition-all">+ Fournisseur</button>
        ...
        <div className="bg-slate-800/50 border border-slate-700 rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-16 text-slate-500">
              <p className="text-4xl mb-3">{String.fromCodePoint(0x1F3ED)}</p>
              <p className="font-semibold">Aucun fournisseur</p>
              <button onClick={() => setModalNouvel(true)} className="mt-4 text-indigo-400 text-sm font-bold">+ Ajouter un fournisseur</button>
            </div>
          ) : (
                  ...
                  <tr key={f.id} className="hover:bg-slate-700/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-orange-600/20 rounded-full flex items-center justify-center text-orange-400 font-black text-sm">{f.nom[0].toUpperCase()}</div>
      ...
      {onglet === 'receptions' && (
        <div className="bg-slate-800/50 border border-slate-700 rounded-2xl overflow-hidden">
          {receptions.length === 0 ? (
            <div className="text-center py-16 text-slate-500">
              <p className="text-4xl mb-3">{String.fromCodePoint(0x1F4E6)}</p>
              <p className="font-semibold">Aucune réception enregistrée</p>
              <button onClick={() => setModalReception(true)} className="mt-4 text-indigo-400 text-sm font-bold">Première réception</button>
            </div>
          ) : (
            <table className="w-full text-left">
              <thead>
                  ...
                  <tr key={r.id} className="hover:bg-slate-700/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {r.montantDette > 0
                        ...
                        : <span className="text-emerald-400 text-sm">Soldé</span>}
                    </td>
                    <td className="px-6 py-4 text-slate-400 text-sm">{new Date(r.createdAt).toLocaleDateString('fr-FR')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
