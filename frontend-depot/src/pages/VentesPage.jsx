        ...
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 w-full print:hidden overflow-y-auto">
          <div className="flex justify-between items-start mb-6 gap-4">
            <div>
              ...
              {vente.createur && <p className="text-xs text-slate-500 mt-1">Créée par {vente.createur.email}</p>}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-5 text-sm">
            <div className="bg-slate-800 rounded-xl p-3">
              ...
              <p className="text-white font-bold">{vente.depot?.nom || '—'}</p>
            </div>
            <div className="bg-slate-800 rounded-xl p-3">
              ...
              <tbody className="divide-y divide-slate-700/50">
                {vente.lignes.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td className="px-4 py-3 text-right text-slate-400">{l.prix.toLocaleString('fr-FR')}</td>
              ...
              <tfoot className="border-t border-slate-700 bg-slate-900/50">
                {totalRemise > 0 && (
                  <tr>
                    ...
                    <td className="px-4 py-2 text-right text-amber-400 font-bold">-{totalRemise.toLocaleString('fr-FR')} FCFA</td>
                  </tr>
                )}
                <tr>
                  <td colSpan="4" className="px-4 py-3 text-white font-black text-right">TOTAL</td>
                  <td className="px-4 py-3 text-right text-indigo-400 font-black text-lg">{vente.total.toLocaleString('fr-FR')} FCFA</td>
                </tr>
              </tfoot>
            </table>
          </div>
          ...
          {vente.statut !== 'ANNULE' && showAnnulation && canCancel && (
            <div className="mb-4 space-y-3">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowAnnulation(false)} className="flex-1 bg-slate-800 text-slate-300 font-bold py-2 rounded-xl text-sm">Annuler</button>
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={onClose} className="flex-1 min-w-[140px] bg-slate-800 text-slate-300 font-bold py-3 rounded-xl text-sm">Fermer</button>
            <button onClick={handleImprimer} className="flex-1 min-w-[160px] bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 rounded-xl text-sm transition-all">Imprimer PDF</button>
            {canValidate && vente.statut === 'ATTENTE' && (
              <button onClick={handleValiderSortie} disabled={loading}
                className="bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-400 font-bold py-3 px-4 rounded-xl text-sm transition-all">Valider sortie</button>
            )}
            {canCancel && vente.statut !== 'ANNULE' && !showAnnulation && (
              <button onClick={() => setShowAnnulation(true)}
                className="bg-red-600/20 hover:bg-red-600/30 border border-red-500/30 text-red-400 font-bold py-3 px-4 rounded-xl text-sm transition-all">Annuler</button>
            )}
          </div>
        </div>
      </div>
...
export default function VentesPage() {
  ...
  const { metier: metierAuth, tenantId, role } = useAuth();
  ...
  const params = useMemo(() => {
    ...
    if (filtres.startDate) p.startDate = filtres.startDate;
    if (filtres.endDate) p.endDate = filtres.endDate;
    if (filtres.statut) p.statut = filtres.statut;
    return p;
  }, [tenantId, depotId, filtres]);

  const { data: ventes = [],
    loading,
    refetch,
   } = useData('/ventes', { params, enabled: !!tenantId && !!depotId });

  const { data: ventesEnAttente = [],
   } = useData('/ventes/validations/en-attente', {
    params: { tenantId, depotId },
    enabled: !!tenantId && !!depotId && canValidate,
  });

  const handleValidate = async (venteId) => {
    try {
      await api.patch(`/ventes/${venteId}/valider-sortie`, { tenantId, depotId });
      success('Sortie validée');
      refetch();
    } catch (err) {
      notifError(err.response?.data?.message || 'Erreur de validation', 'Échec');
    }
  };
  ...
  }, [ventes, filtres.recherche]);

  const totalCA = useMemo(() =>
    ventesFiltrees.filter(v => v.statut === 'PAYE').reduce((acc, v) => acc + v.total, 0),
    [ventesFiltrees]
  );
  const nbAnnulees = useMemo(() =>
    ventesFiltrees.filter(v => v.statut === 'ANNULE').length,
    [ventesFiltrees]
  );
      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-5 mb-6">
        ...
        <div className="flex justify-end mt-3">
          <button onClick={() => setFiltres({ startDate: '', endDate: '', statut: '', recherche: '' })}
            className="text-slate-400 hover:text-white text-xs font-bold transition-colors">Réinitialiser filtres</button>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {ventesFiltrees.map((v) => (
                <tr key={v.id} className={`hover:bg-slate-700/30 transition-colors ${v.statut === 'ANNULE' ? 'opacity-50' : ''}`}>
                  <td className="px-6 py-4 text-indigo-400 font-black text-sm">{v.reference}</td>
                  <td className="px-6 py-4 text-slate-400 text-sm">{new Date(v.date).toLocaleDateString('fr-FR')}</td>
                  <td className="px-6 py-4 text-slate-300 text-sm">{v.depot?.nom || '—'}</td>
                  <td className="px-6 py-4 text-slate-400 text-sm">{v.client?.nom || 'Passant'}</td>
                  <td className="px-6 py-4 text-slate-500 text-xs">{v.createur?.email || v.createur?.nom || '—'}</td>
                  <td className="px-6 py-4">
                    <span className="text-xs text-slate-400 bg-slate-700 px-2 py-1 rounded-lg">{v.modePaiement || 'CASH'}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => setVenteDetail(v)}
                      className="bg-indigo-600/20 hover:bg-indigo-600/40 border border-indigo-500/30 text-indigo-400 font-bold px-3 py-1.5 rounded-lg text-xs transition-all">Détail</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
function PendingValidationPanel({ ventes, onValidate, canValidate }) {
  ...
  return (
    <div className="mb-8 rounded-2xl border border-orange-500/20 bg-orange-500/5 p-5">
      ...
      {ventes.length === 0 ? (
        <div className="rounded-xl border border-slate-700 bg-slate-900/50 px-4 py-6 text-sm text-slate-500">Aucune sortie à valider.</div>
      ) : (
          ...
          {ventes.map((vente) => (
            <div key={vente.id} className="rounded-xl border border-slate-700 bg-slate-900/70 p-4">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                ...
                <button
                  onClick={() => { setProcessingId(vente.id); onValidate(vente.id).finally(() => setProcessingId('')); }}
