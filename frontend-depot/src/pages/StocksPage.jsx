...
function SkeletonRow() {
  return (
    <tr>
      <td className="px-6 py-5"><div className="h-4 bg-slate-700 rounded w-32 animate-pulse" /></td>
      <td className="px-6 py-5"><div className="h-4 bg-slate-700 rounded w-16 animate-pulse" /></td>
      <td className="px-6 py-5"><div className="h-4 bg-slate-700 rounded w-12 animate-pulse" /></td>
      <td className="px-6 py-5"><div className="h-4 bg-slate-700 rounded w-20 animate-pulse" /></td>
      <td className="px-6 py-5"><div className="h-4 bg-slate-700 rounded w-16 animate-pulse ml-auto" /></td>
    </tr>
  );
}
...
function ErrorCard({ message, onRetry }) {
  return (
    <div className="bg-red-500/10 border border-red-500/30 p-6 rounded-3xl text-center">
      ...
      <p className="text-red-400 font-bold text-lg mb-2">Erreur de chargement</p>
...
const StocksPage = () => {
  ...
  const { rayons } = useDataContext();
  const { canEdit: canEditStock } = usePermission('STOCK');
  ...
  const stocksOpts = { params: { tenantId, depotId }, enabled: !!tenantId };
  const statsOpts = { params: { tenantId, depotId }, enabled: !!tenantId };
  const mvtsOpts = { params: { tenantId, depotId }, enabled: !!tenantId && onglet === 'mouvements' };
  ...
  } = useData('/stocks', stocksOpts);

  const {
    data: statsStocks = {},
    loading: loadingStats,
  } = useData('/stocks/stats', statsOpts);

  const { data: mouvements = [],
    loading, error: loadingMvts,
   } = useData('/stocks/mouvements', mvtsOpts);

  const mutationAjuster = {
    ...
    <div className="p-6 pb-24 space-y-8 bg-slate-900 min-h-screen text-slate-200">
      ...
      {onglet === 'stocks' && (
        <div className="space-y-6">
          ...
          {stocksError ? (
            <ErrorCard message={stocksError?.message || 'Impossible de charger les stocks'} onRetry={refetchStocks} />
          ) : (
              ...
              <div key={mv.id} className="bg-slate-800 border border-slate-700 p-5 rounded-3xl flex items-center justify-between hover:border-indigo-500/50 transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-slate-500 text-[10px] mt-1">
                      {new Date(mv.createdAt).toLocaleString('fr-FR')} &bull; {mv.type.replace(/_/g, ' ')}
                    </div>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1">
                  ...
                  <div className="text-[10px] text-slate-500 truncate max-w-[200px] italic">
                    &ldquo;{mv.motif || 'Aucun motif'}&rdquo;
                  </div>
                </div>
              </div>
            ))}
