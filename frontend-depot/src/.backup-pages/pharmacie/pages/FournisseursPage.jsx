...
export default function FournisseursPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const res = await api.get('/pharmacie/fournisseurs'); setFournisseurs(res.data?.data || res.data || []); }
    catch (_) {}
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.patch(`/pharmacie/fournisseurs/${confirmDelete.id}`, { actif: false });
      showNotif('Fournisseur supprimé ✓');
  ...
  const filtres = fournisseurs.filter(f => {
    ...
    return !q || f.nom?.toLowerCase().includes(q) || f.contact?.toLowerCase().includes(q) || f.telephone?.includes(q);
  });
  ...
  return (
    <div className="p-6">
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? (
                <tr><td colSpan={4} className="text-center py-16 text-slate-500">Aucun fournisseur trouvé</td></tr>
              ) : paginated.map(f => (
                <tr key={f.id} className="hover:bg-slate-700/20 transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-emerald-500/20 rounded-xl flex items-center justify-center text-emerald-400 font-black text-sm">{f.nom?.[0]?.toUpperCase()}</div>
                      <div>
      ...
      {formOpen && <FournisseurForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { showNotif(editItem ? 'Fournisseur modifié ✓' : 'Fournisseur créé ✓'); load(); }} edit={editItem} metier="pharmacie" />}
