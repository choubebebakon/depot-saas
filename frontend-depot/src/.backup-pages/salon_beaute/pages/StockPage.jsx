...
export default function StockPage() {
  ...
  const load = useCallback(async () => { setLoading(true);
    try { const r = await api.get('/salon/stock', { params: { page, limit: LIMIT, search } }); setItems(r.data.data); setTotal(r.data.total); }
  ...
  const handleSubmit = async (e) => { e.preventDefault(); setSaving(true);
    try { if (editItem) await api.patch(`/salon/stock/${editItem.id}`, form); else await api.post('/salon/stock', form); setFormOpen(false); load(); }
    catch { alert('Erreur'); } finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!confirmDelete) return; setDeleting(true);
    try { await api.delete(`/salon/stock/${confirmDelete.id}`); setConfirmDelete(null); load(); }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-3xl font-black text-white tracking-tight">🧴 Stock Produits</h1><p className="text-slate-400 text-sm">{total} article(s)</p></div>
        <button onClick={openCreate} className="bg-pink-500 hover:bg-pink-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all">+ Nouvel article</button>
      </div>
      ...
      {loading ? <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-pink-500 border-t-transparent rounded-full animate-spin" /></div> : (
        <div className="overflow-x-auto rounded-2xl border border-slate-700/50">
          <table className="w-full text-sm"><thead className="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-wider"><tr><th className="text-left p-4">Nom</th><th className="text-left p-4">Catégorie</th><th className="text-right p-4">Qté</th><th className="text-right p-4">Seuil</th><th className="text-right p-4">Prix un.</th><th className="text-center p-4">Actions</th></tr></thead><tbody>{items.map(item => (
            <tr key={item.id} className="border-t border-slate-800 hover:bg-slate-800/40"><td className="p-4 text-white font-semibold">{item.nom}</td><td className="p-4"><span className="text-[10px] font-black uppercase px-2 py-1 rounded-full bg-slate-700 text-slate-300">{item.categorie}</span></td><td className="p-4 text-right text-white font-bold">{item.quantite || 0}</td><td className="p-4 text-right text-slate-300">{item.seuilAlerte || '-'}</td><td className="p-4 text-right text-white font-bold">{Number(item.prixUnitaire || 0).toLocaleString('fr-FR')} F</td><td className="p-4 text-center"><div className="flex justify-center gap-2"><button onClick={() => openEdit(item)} className="text-pink-400 hover:text-pink-300 text-xs font-bold">✏️</button><button onClick={() => setConfirmDelete(item)} className="text-red-400 hover:text-red-300 text-xs font-bold">🗑️</button></div></td></tr>
          ))}</tbody></table>
        </div>
      )}
