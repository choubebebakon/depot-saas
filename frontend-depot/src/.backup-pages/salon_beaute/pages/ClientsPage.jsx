...
export default function ClientsPage() {
  ...
  const load = useCallback(async () => { setLoading(true);
    try { const r = await api.get('/salon/clients', { params: { page, limit: LIMIT, search } }); setClients(r.data.data); setTotal(r.data.total); }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/salon/clients/${confirmDelete.id}`); setConfirmDelete(null); load(); }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={openCreate} className="bg-pink-500 hover:bg-pink-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all">+ Nouveau client</button>
      </div>
      ...
      {loading ? <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-pink-500 border-t-transparent rounded-full animate-spin" /></div> : (
        <div className="overflow-x-auto rounded-2xl border border-slate-700/50">
          <table className="w-full text-sm"><thead className="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-wider"><tr><th className="text-left p-4">Nom</th><th className="text-left p-4">Téléphone</th><th className="text-left p-4">Email</th><th className="text-left p-4">Notes</th><th className="text-center p-4">Actions</th></tr></thead><tbody>{clients.map(c => (
            <tr key={c.id} className="border-t border-slate-800 hover:bg-slate-800/40"><td className="p-4 text-white font-semibold">{c.nom}</td><td className="p-4 text-slate-300">{c.telephone}</td><td className="p-4 text-slate-300">{c.email || '-'}</td><td className="p-4 text-slate-300">{c.notes || '-'}</td><td className="p-4 text-center"><div className="flex justify-center gap-2"><button onClick={() => openEdit(c)} className="text-pink-400 hover:text-pink-300 text-xs font-bold">✏️</button><button onClick={() => setConfirmDelete(c)} className="text-red-400 hover:text-red-300 text-xs font-bold">🗑️</button></div></td></tr>
          ))}</tbody></table>
        </div>
      )}
      ...
      <ClientForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={load} edit={editItem} metier="salon" />
