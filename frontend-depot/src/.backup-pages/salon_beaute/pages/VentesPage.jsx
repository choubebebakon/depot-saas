...
export default function VentesPage() {
  ...
  const [encaissementOpen, setEncaissementOpen] = useState(false); const [encaissementRDV, setEncaissementRDV] = useState(null);

  const load = useCallback(async () => { setLoading(true);
    try { const r = await api.get('/salon/ventes', { params: { page, limit: LIMIT, search } }); setVentes(r.data.data); setTotal(r.data.total); }
    catch { setVentes([]); } finally { setLoading(false); }
  }, [page, search]);

  useEffect(() => { load(); }, [load]);

  const openCreate = () => { setForm({ clientNom: '', prestation: '', montant: '', modePaiement: 'ESPECES', notes: '' }); setFormOpen(true); };

  const handleSubmit = async (e) => { e.preventDefault(); setSaving(true);
    try { await api.post('/salon/ventes', form); setFormOpen(false); load(); }
    catch { alert('Erreur'); } finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!confirmDelete) return; setDeleting(true);
    try { await api.delete(`/salon/ventes/${confirmDelete.id}`); setConfirmDelete(null); load(); }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title="💰 Nouvelle vente" loading={saving} submitLabel="Créer">
        ...
        <div><label className="text-slate-400 text-xs font-bold uppercase block mb-1.5">Mode de paiement</label><select value={form.modePaiement} onChange={set('modePaiement')} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white w-full text-sm"><option value="ESPECES">Espèces</option><option value="CARTE">Carte</option><option value="MOBILE">Mobile money</option></select></div>
      </FormModal>
      ...
      <EncaissementSalonForm isOpen={encaissementOpen} onClose={() => setEncaissementOpen(false)} onSuccess={() => load()} metier="salon" rendezVous={encaissementRDV} />
    </div>
  );
}
