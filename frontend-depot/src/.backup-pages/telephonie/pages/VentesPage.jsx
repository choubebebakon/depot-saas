...
export default function VentesPage() {
  ...
  const [form, setForm] = useState({ telephoneId: '', accessoireId: '', clientId: '', quantite: 1, montant: '', modePaiement: 'Espèces', dateVente: '' });
  const [telephones, setTelephones] = useState([]); const [accessoires, setAccessoires] = useState([]); const [clients, setClients] = useState([]); const [saving, setSaving] = useState(false);
  const [page, setPage] = useState(1); const perm = usePermission(PERMISSIONS, 'ventes'); const itemsPerPage = 20;
  const load = useCallback(async () => { setLoading(true); try { const res = await api.get('/telephonie/ventes'); setVentes(res.data?.data || res.data || []); } catch (_) {} finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  const showNotif = (msg, type = 'success') => { setNotif({ msg, type }); setTimeout(() => setNotif(null), 3000); };
  const openCreate = () => { setEditItem(null); setForm({ telephoneId: '', accessoireId: '', clientId: '', quantite: 1, montant: '', modePaiement: 'Espèces', dateVente: '' }); api.get('/telephonie/telephones').then(r => setTelephones(r.data?.data || r.data || [])).catch(() => {}); api.get('/telephonie/accessoires').then(r => setAccessoires(r.data?.data || r.data || [])).catch(() => {}); api.get('/telephonie/clients').then(r => setClients(r.data?.data || r.data || [])).catch(() => {}); setFormOpen(true); };
  const openEdit = (v) => { setEditItem(v); setForm({ telephoneId: v.telephoneId || '', accessoireId: v.accessoireId || '', clientId: v.clientId || '', quantite: v.quantite || 1, montant: v.montant || '', modePaiement: v.modePaiement || 'Espèces', dateVente: v.dateVente || '' }); api.get('/telephonie/telephones').then(r => setTelephones(r.data?.data || r.data || [])).catch(() => {}); api.get('/telephonie/accessoires').then(r => setAccessoires(r.data?.data || r.data || [])).catch(() => {}); api.get('/telephonie/clients').then(r => setClients(r.data?.data || r.data || [])).catch(() => {}); setFormOpen(true); };
  const handleSubmit = async (e) => { e.preventDefault(); setSaving(true); try { if (editItem) await api.patch(`/telephonie/ventes/${editItem.id}`, form); else await api.post('/telephonie/ventes', form); setFormOpen(false); showNotif(editItem ? 'Vente modifiée ✓' : 'Vente créée ✓'); load(); } catch (err) { showNotif(err.response?.data?.message || 'Erreur', 'error'); } finally { setSaving(false); } };
  const handleDelete = async () => { if (!confirmDelete) return; try { await api.delete(`/telephonie/ventes/${confirmDelete.id}`); showNotif('Vente supprimée ✓'); setConfirmDelete(null); load(); } catch (_) { showNotif('Erreur lors de la suppression', 'error'); } };
  const setF = (f) => (e) => setForm({ ...form, [f]: e.target.value });
  const filtres = ventes.filter(v => { const q = search.toLowerCase(); return !q || v.telephone?.modele?.toLowerCase().includes(q) || v.accessoire?.designation?.toLowerCase().includes(q); });
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {perm.canCreate && <button onClick={openCreate} className="bg-purple-500 hover:bg-purple-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all shadow-lg shadow-purple-500/20">+ Nouvelle Vente</button>}
      </div>
      <div className="mb-6"><input type="text" placeholder="🔍 Téléphone ou accessoire..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-purple-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-60" /></div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(v => (
                <tr key={v.id} className="hover:bg-slate-700/20 transition-colors">
                  <td className="px-5 py-4 text-white font-semibold text-sm">{v.telephone ? `${v.telephone.marque} ${v.telephone.modele}` : v.accessoire?.designation || '—'}</td>
                  <td className="px-5 py-4 text-slate-300 text-sm">{v.client?.nom || '—'}</td>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '✏️ Modifier vente' : '💰 Nouvelle vente'} loading={saving} submitLabel={editItem ? 'Modifier' : 'Créer'}>
        <div className="grid grid-cols-2 gap-4">
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Téléphone</label><select value={form.telephoneId} onChange={setF('telephoneId')} className={inputClass}><option value="">Aucun</option>{telephones.map(t => <option key={t.id} value={t.id}>{t.marque} {t.modele}</option>)}</select></div>
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Accessoire</label><select value={form.accessoireId} onChange={setF('accessoireId')} className={inputClass}><option value="">Aucun</option>{accessoires.map(a => <option key={a.id} value={a.id}>{a.designation}</option>)}</select></div>
          ...
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Paiement</label><select value={form.modePaiement} onChange={setF('modePaiement')} className={inputClass}><option>Espèces</option><option>Mobile Money</option><option>Carte</option></select></div>
      ...
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer la vente" message={`Êtes-vous sûr de vouloir supprimer cette vente ?`} />
    </div>
  );
}
