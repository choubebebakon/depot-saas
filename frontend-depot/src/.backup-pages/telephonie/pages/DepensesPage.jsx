...
const CATEGORIES = ['Stock', 'Loyer', 'Électricité', 'Eau', 'Salaire', 'Marketing', 'Réparation', 'Transport', 'Taxe', 'Autre'];
...
export default function DepensesPage() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const res = await api.get('/telephonie/depenses'); setDepenses(res.data?.data || res.data || []); } catch (_) {} finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const handleSubmit = async (e) => { e.preventDefault(); setSaving(true); try { if (editItem) await api.patch(`/telephonie/depenses/${editItem.id}`, form); else await api.post('/telephonie/depenses', form); setFormOpen(false); showNotif(editItem ? 'Dépense modifiée ✓' : 'Dépense créée ✓'); load(); } catch (err) { showNotif(err.response?.data?.message || 'Erreur', 'error'); } finally { setSaving(false); } };
  const handleDelete = async () => { if (!confirmDelete) return; try { await api.delete(`/telephonie/depenses/${confirmDelete.id}`); showNotif('Dépense supprimée ✓'); setConfirmDelete(null); load(); } catch (_) { showNotif('Erreur lors de la suppression', 'error'); } };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {perm.canCreate && <button onClick={openCreate} className="bg-purple-500 hover:bg-purple-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all shadow-lg shadow-purple-500/20">+ Nouvelle Dépense</button>}
      </div>
      <div className="flex gap-3 mb-6 flex-wrap">
        <input type="text" placeholder="🔍 Libellé ou catégorie..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-purple-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-60" />
      ...
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer la dépense" message={`Êtes-vous sûr de vouloir supprimer cette dépense ?`} />
    </div>
  );
}
