...
export default function FournisseursPage() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const res = await api.get('/quincaillerie/fournisseurs'); setFournisseurs(res.data?.data || res.data || []); } catch (_) {} finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  const showNotif = (msg, type = 'success') => { setNotif({ msg, type }); setTimeout(() => setNotif(null), 3000); };
  const handleDelete = async () => { if (!confirmDelete) return; setDeleting(true); try { await api.delete(`/quincaillerie/fournisseurs/${confirmDelete.id}`); showNotif('Fournisseur supprimé ✓'); setConfirmDelete(null); load(); } catch { showNotif('Erreur lors de la suppression', 'error'); } finally { setDeleting(false); } };
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6"><input type="text" placeholder="🔍 Nom ou spécialité..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-amber-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-60" /></div>
      ...
      <FournisseurForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { showNotif(editItem ? 'Fournisseur modifié ✓' : 'Fournisseur créé ✓'); load(); }} edit={editItem} metier="quincaillerie" />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer le fournisseur" message={`Êtes-vous sûr de vouloir supprimer "${confirmDelete?.nom}" ?`} />
    </div>
  );
}
