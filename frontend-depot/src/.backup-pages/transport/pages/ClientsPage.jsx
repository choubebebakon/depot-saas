...
export default function ClientsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.get('/transport/clients', { params: { page, limit, search } })
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/transport/clients/${confirmDelete.id}`);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">{total} client(s)</p>
        </div>
                ...
                <tr key={c.id} className="border-t border-slate-800 hover:bg-slate-800/40">
                  ...
                  <td className="p-4 text-center">
                    <div className="flex justify-center gap-2">
                      <button onClick={() => openEdit(c)} className="text-orange-400 text-xs">✏️</button>
                      <button onClick={() => setConfirmDelete(c)} className="text-red-400 text-xs">🗑️</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <TransportClientForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={load} edit={editItem} />
