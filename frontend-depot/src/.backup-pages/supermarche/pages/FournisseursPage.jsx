...
export default function FournisseursPage() {
  ...
  const load = useCallback(async () => { setLoading(true);
    try { const res = await api.get('/supermarche/fournisseurs'); setFournisseurs(res.data?.data || res.data || []); }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/supermarche/fournisseurs/${confirmDelete.id}`); setConfirmDelete(null); showNotif('Fournisseur supprimé'); load(); }
  ...
  return (
    <div className="p-6">
          ...
          <table className="w-full">
            <thead className="bg-slate-900/50">
              <tr className="text-slate-500 text-xs font-bold uppercase tracking-widest">
                ...
                <th className="text-left px-5 py-4">Délai livraison</th>
      ...
      <FournisseurForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { showNotif(editItem ? 'Fournisseur modifié ✓' : 'Fournisseur créé ✓'); load(); }} edit={editItem} metier="supermarche" />
