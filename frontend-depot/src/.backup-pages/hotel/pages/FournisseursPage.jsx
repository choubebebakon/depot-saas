...
export default function FournisseursPage() {
  ...
  const load = useCallback(async () => { setLoading(true);
    try { const res = await api.get('/hotel/fournisseurs'); setFournisseurs(res.data?.data || res.data || []); }
    catch (_) {}
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/hotel/fournisseurs/${confirmDelete.id}`); setConfirmDelete(null); load(); }
    catch { alert('Erreur lors de la suppression'); } finally { setDeleting(false); }
  };

  const filtres = fournisseurs.filter(f => {
    const q = search.toLowerCase();
    return !q || f.nom?.toLowerCase().includes(q) || f.contact?.toLowerCase().includes(q) || f.typeService?.toLowerCase().includes(q);
  });
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{fournisseurs.length} fournisseur{fournisseurs.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="mb-6">
        <input type="text" placeholder="🔍 Rechercher..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
          className="bg-slate-800 border border-slate-700 focus:border-violet-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-72" />
      </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-900/50">
              <tr className="text-slate-500 text-xs font-bold uppercase tracking-widest">
                ...
                <th className="text-left px-5 py-4">Type service</th>
                <th className="text-right px-5 py-4">Téléphone</th>
                <th className="text-center px-5 py-4">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-16 text-slate-500">Aucun fournisseur trouvé</td></tr>
              ) : paginated.map(f => (
                <tr key={f.id} className="hover:bg-slate-700/20 transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-violet-500/20 rounded-xl flex items-center justify-center text-violet-400 font-black text-sm">{f.nom?.[0]?.toUpperCase()}</div>
                      <div>
                  ...
                  <td className="px-5 py-4 text-slate-300 text-sm">{f.typeService || '—'}</td>
      ...
      <FournisseurForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={load} edit={editItem} metier="hotel" />
