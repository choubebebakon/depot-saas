...
export default function ClientsPage() {
  ...
  const load = useCallback(async () => { setLoading(true);
    try { const res = await api.get('/hotel/clients'); setClients(res.data?.data || res.data || []); }
    catch (_) {}
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/hotel/clients/${confirmDelete.id}`); setConfirmDelete(null); load(); }
  ...
  return (
    <div className="p-6">
          ...
          <table className="w-full">
            <thead className="bg-slate-900/50">
              <tr className="text-slate-500 text-xs font-bold uppercase tracking-widest">
                ...
                <th className="text-center px-5 py-4">Pièce</th>
                <th className="text-center px-5 py-4">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-16 text-slate-500">Aucun client trouvé</td></tr>
              ) : paginated.map(c => (
                <tr key={c.id} className="hover:bg-slate-700/20 transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-violet-500/20 rounded-xl flex items-center justify-center text-violet-400 font-black text-sm">{c.nom?.[0]?.toUpperCase()}</div>
                      <div>
                        <p className="text-white font-semibold text-sm">{c.nom}</p>
                        {c.nationalite && <p className="text-slate-500 text-xs">{c.nationalite}</p>}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-4 text-center text-slate-300 text-xs">{c.pieceIdentite ? `${c.pieceIdentite}: ${c.numeroPiece || ''}` : '—'}</td>
      ...
      <ClientForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={load} edit={editItem} metier="hotel" />
