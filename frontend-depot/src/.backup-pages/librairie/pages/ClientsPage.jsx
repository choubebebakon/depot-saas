import { useState, useEffect, useCallback } from 'react'; import api from '../../../api'; import { PERMISSIONS } from '../permissions'; import { usePermission } from '../../../shared/hooks/usePermission'; import ClientLibrairieForm from '../forms/ClientLibrairieForm'; import ConfirmModal from '../../../shared/components/forms/ConfirmModal';
...
export default function ClientsPage() {
  ...
  const perm = usePermission(PERMISSIONS, 'clients'); const itemsPerPage = 20;
  const showNotif = (msg, type = 'success') => { setNotif({ msg, type }); setTimeout(() => setNotif(null), 3000); };
  const load = useCallback(async () => { setLoading(true); try { const res = await api.get('/librairie/clients'); const items = res.data?.data || res.data || []; setData(items); setTotal(items.length); } catch (_) { setData([]); setTotal(0); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  const openCreate = () => { setEditItem(null); setFormOpen(true); };
  const openEdit = (item) => { setEditItem(item); setFormOpen(true); };
  const handleDelete = async () => { if (!confirmDelete) return; setDeleting(true); try { await api.delete(`/librairie/clients/${confirmDelete.id}`); setConfirmDelete(null); showNotif('Supprimé ✓'); load(); } catch { showNotif('Erreur', 'error'); } finally { setDeleting(false); } };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">👤 Clients</h1><p className="text-slate-400 text-sm mt-1">{data.length} client{data.length !== 1 ? 's' : ''}</p></div>
        {perm.canCreate && <button onClick={openCreate} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-indigo-600/20">+ Nouveau Client</button>}
      </div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(i => (
                <tr key={i.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-slate-300 text-sm">{i.adresse || '—'}</td>
      ...
      <ClientLibrairieForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { showNotif(editItem ? 'Client modifié ✓' : 'Client créé ✓'); load(); }} edit={editItem} />
