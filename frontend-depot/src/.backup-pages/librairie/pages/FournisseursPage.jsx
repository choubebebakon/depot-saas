import { useState, useEffect, useCallback } from 'react'; import api from '../../../api'; import { PERMISSIONS } from '../permissions'; import { usePermission } from '../../../shared/hooks/usePermission'; import FournisseurLibrairieForm from '../forms/FournisseurLibrairieForm'; import ConfirmModal from '../../../shared/components/forms/ConfirmModal';
...
export default function FournisseursPage() {
  ...
  const perm = usePermission(PERMISSIONS, 'fournisseurs'); const itemsPerPage = 20;
  const showNotif = (msg, type = 'success') => { setNotif({ msg, type }); setTimeout(() => setNotif(null), 3000); };
  const load = useCallback(async () => { setLoading(true); try { const res = await api.get('/librairie/fournisseurs'); const items = res.data?.data || res.data || []; setData(items); setTotal(items.length); } catch (_) { setData([]); setTotal(0); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  const openCreate = () => { setEditItem(null); setFormOpen(true); };
  const openEdit = (item) => { setEditItem(item); setFormOpen(true); };
  const handleDelete = async () => { if (!confirmDelete) return; setDeleting(true); try { await api.delete(`/librairie/fournisseurs/${confirmDelete.id}`); setConfirmDelete(null); showNotif('Supprimé ✓'); load(); } catch { showNotif('Erreur', 'error'); } finally { setDeleting(false); } };
  const filtres = data.filter(i => { const q = search.toLowerCase(); return !q || i.nom?.toLowerCase().includes(q) || i.email?.toLowerCase().includes(q) || i.telephone?.includes(q); });
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">🏭 Fournisseurs</h1><p className="text-slate-400 text-sm mt-1">{data.length} fournisseur{data.length !== 1 ? 's' : ''}</p></div>
        {perm.canCreate && <button onClick={openCreate} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-indigo-600/20">+ Nouveau Fournisseur</button>}
      </div>
      <div className="mb-6"><input type="text" placeholder="🔍 Nom, email..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-indigo-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-72" /></div>
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full"><thead className="bg-slate-900/50"><tr className="text-slate-500 text-xs font-bold uppercase tracking-widest"><th className="text-left px-5 py-4">Nom</th><th className="text-left px-5 py-4">Contact</th><th className="text-left px-5 py-4">Email</th><th className="text-left px-5 py-4">Téléphone</th><th className="text-center px-5 py-4">Actions</th></tr></thead>
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? <tr><td colSpan={5} className="text-center py-16 text-slate-500">Aucun fournisseur</td></tr>
              : paginated.map(i => (
                <tr key={i.id} className="hover:bg-slate-700/20 transition-colors">
                  <td className="px-5 py-4"><div className="flex items-center gap-3"><div className="w-9 h-9 bg-indigo-600/20 rounded-xl flex items-center justify-center text-indigo-400 font-black text-sm">{i.nom?.slice(0,2).toUpperCase()}</div><span className="text-white font-semibold text-sm">{i.nom}</span></div></td>
                  <td className="px-5 py-4 text-white">{i.contact || '—'}</td>
      ...
      <FournisseurLibrairieForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { showNotif(editItem ? 'Fournisseur modifié ✓' : 'Fournisseur créé ✓'); load(); }} edit={editItem} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} message="Supprimer ce fournisseur ?" loading={deleting} />
    </div>
  );
}
