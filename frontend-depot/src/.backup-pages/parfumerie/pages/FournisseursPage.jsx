...
import FournisseurParfumerieForm from '../forms/FournisseurParfumerieForm';
...
export default function FournisseursPage() {
  ...
  const limit = 20;
  const load = () => { setLoading(true); api.get('/parfumerie/fournisseurs', { params: { page, limit, search } }).then(r => { setData(r.data.data); setTotal(r.data.total); }).catch(() => setData([])).finally(() => setLoading(false)); };
  useEffect(() => { load(); }, [page, search]);
  const handleDelete = async () => { if (!confirm) return; try { await api.delete(`/parfumerie/fournisseurs/${confirm.id}`); setConfirm(null); load(); } catch { alert('Erreur'); } };
  const totalPages = Math.ceil(total / limit);

  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-white tracking-tight">🏭 Fournisseurs</h1><p className="text-slate-400 text-sm">{total} fournisseur(s)</p></div><button onClick={() => { setEdit(null); setShowForm(true); }} className="bg-fuchsia-500 hover:bg-fuchsia-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm">+ Nouveau</button></div>
      ...
      {loading ? <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-fuchsia-500 border-t-transparent rounded-full animate-spin" /></div> : (
        <div className="overflow-x-auto rounded-2xl border border-slate-700/50">
          <table className="w-full text-sm"><thead className="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-wider"><tr><th className="text-left p-4">Nom</th><th className="text-left p-4">Contact</th><th className="text-left p-4">Téléphone</th><th className="text-left p-4">Email</th><th className="text-center p-4">Actions</th></tr></thead><tbody>{data.map(f => (
            <tr key={f.id} className="border-t border-slate-800 hover:bg-slate-800/40"><td className="p-4 text-white font-semibold">{f.nom}</td><td className="p-4 text-slate-300">{f.contact || '-'}</td><td className="p-4 text-slate-300">{f.telephone}</td><td className="p-4 text-slate-300">{f.email || '-'}</td><td className="p-4 text-center"><div className="flex justify-center gap-2"><button onClick={() => { setEdit(f); setShowForm(true); }} className="text-fuchsia-400 text-xs">✏️</button><button onClick={() => setConfirm(f)} className="text-red-400 text-xs">🗑️</button></div></td></tr>
          ))}</tbody></table>
        </div>
      )}
      ...
      <FournisseurParfumerieForm isOpen={showForm} onClose={() => { setShowForm(false); setEdit(null); }} onSuccess={load} edit={edit} />
