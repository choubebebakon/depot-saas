...
const TYPES_ALIM = ['Fourrage', 'Granulés', 'Céréales', 'Minéraux', 'Vitamines', 'Eau'];
function ModalAlim({ onClose, onSuccess, edit }) {
  const [form, setForm] = useState({ type: edit?.type || 'Fourrage', quantite: edit?.quantite || '', unite: edit?.unite || 'kg', dateDistribution: edit?.dateDistribution || '', enclos: edit?.enclos || '', notes: edit?.notes || '' });
  const [loading, setLoading] = useState(false); const [erreur, setErreur] = useState('');
  const handleSubmit = async (e) => { e.preventDefault(); setLoading(true); try { if (edit) await api.patch(`/elevage/alimentation/${edit.id}`, form); else await api.post('/elevage/alimentation', form); onSuccess(); onClose(); } catch (err) { setErreur(err.response?.data?.message || 'Erreur'); } finally { setLoading(false); } };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-slate-900 border border-slate-700 rounded-2xl p-8 w-full max-w-lg shadow-2xl">
        <h3 className="text-white font-black text-xl mb-6">{edit ? '✏️ Modifier' : '🌾 Nouvelle'} Distribution</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Type</label><select value={form.type} onChange={e => setForm({...form, type: e.target.value})} className={inputClass}>{TYPES_ALIM.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
            ...
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Unité</label><select value={form.unite} onChange={e => setForm({...form, unite: e.target.value})} className={inputClass}><option>kg</option><option>sac</option><option>L</option></select></div>
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Date</label><input type="date" value={form.dateDistribution} onChange={e => setForm({...form, dateDistribution: e.target.value})} className={inputClass} /></div>
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Enclos</label><input value={form.enclos} onChange={e => setForm({...form, enclos: e.target.value})} className={inputClass} /></div>
...
export default function AlimentationPage() {
  const [distribs, setDistribs] = useState([]); const [loading, setLoading] = useState(true); const [search, setSearch] = useState(''); const [filtreType, setFiltreType] = useState('');
  ...
  const [page, setPage] = useState(1); const perm = usePermission(PERMISSIONS, 'alimentation'); const itemsPerPage = 20;
  const load = useCallback(async () => { setLoading(true); try { const res = await api.get('/elevage/alimentation'); setDistribs(res.data?.data || res.data || []); } catch (_) {} finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  const showNotif = (msg, type = 'success') => { setNotif({ msg, type }); setTimeout(() => setNotif(null), 3000); };
  const handleDelete = async () => { if (!confirmDelete) return; setDeleting(true); try { await api.delete(`/elevage/alimentation/${confirmDelete.id}`); setConfirmDelete(null); showNotif('Distribution supprimée ✓'); load(); } catch (_) { showNotif('Erreur suppression', 'error'); } finally { setDeleting(false); } };
  const filtres = distribs.filter(d => { const q = search.toLowerCase(); return !q || d.type?.toLowerCase().includes(q) || d.enclos?.toLowerCase().includes(q); });
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">🌾 Alimentation</h1><p className="text-slate-400 text-sm mt-1">{distribs.length} distribution{distribs.length !== 1 ? 's' : ''}</p></div>
      ...
      {formOpen && <ModalAlim onClose={() => setFormOpen(false)} onSuccess={() => { showNotif(editItem ? 'Distribution modifiée ✓' : 'Distribution créée ✓'); load(); }} edit={editItem} />}
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer la distribution" message={`Supprimer cette distribution ? Cette action est irréversible.`} />
    </div>
  );
}
