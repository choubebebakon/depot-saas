  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-white">Rservations</h1>
          <p className="text-slate-400 text-sm mt-1">{totalItems} rservation{totalItems !== 1 ? 's' : ''}</p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-900/50">
              <tr className="text-slate-500 text-xs font-bold uppercase tracking-widest">
                ...
                <th className="text-center px-5 py-4">Arrive</th>
                <th className="text-center px-5 py-4">Dpart</th>
            ...
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-16 text-slate-500">Aucune rservation trouvée</td></tr>
              ) : paginated.map(r => (
      ...
      <ReservationHotelForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} />
