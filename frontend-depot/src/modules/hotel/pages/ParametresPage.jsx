...
export default function ParametresPage() {
  ...
  const handleSave = async (e) => {
    ...
    setLoading(true);
    try {
      await api.put('/hotellerie/config', config);
      setSaved(true);
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">Configuration de l'hôtel</p>
      </div>

      <form onSubmit={handleSave} className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-8 space-y-6">
        <div>
          <h2 className="text-white font-bold text-sm mb-4">🏨 Informations générales</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2"><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Nom de l'hôtel</label><input value={config.nomHotel} onChange={e => setConfig({...config, nomHotel: e.target.value})} className={inputClass} /></div>
        ...
        <div className="border-t border-slate-700/50 pt-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">TVA (%)</label><input value={config.TVA} onChange={e => setConfig({...config, TVA: e.target.value})} className={inputClass} /></div>
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Heure écheck-in</label><input type="time" value={config.écheckinHeure} onChange={e => setConfig({...config, écheckinHeure: e.target.value})} className={inputClass} /></div>
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Heure écheck-out</label><input type="time" value={config.écheckoutHeure} onChange={e => setConfig({...config, écheckoutHeure: e.target.value})} className={inputClass} /></div>
          </div>
        </div>
        ...
        {perm.canEdit && (
          <div className="flex items-center gap-4 pt-2">
            <button type="submit" disabled={loading}
              className="bg-violet-500 hover:bg-violet-400 disabled:opacity-40 text-white font-bold px-8 py-3 rounded-xl text-sm transition-all shadow-lg shadow-violet-500/20">
              {loading ? '⏳...' : '💾 Enregistrer'}
            </button>
