    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{totalItems} membre{totalItems !== 1 ? 's' : ''}</p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-900/50">
              <tr className="text-slate-500 text-xs font-bold uppercase tracking-widest">
                ...
                <th className="text-center px-5 py-4">Tlphone</th>
              ...
              {paginated.length === 0 ? (
                ...
                <tr key={p.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${p.statut === 'Actif' ? 'bg-emerald-500/20 text-emerald-400' : p.statut === 'Cong' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-red-500/20 text-red-400'}`}>{p.statut || 'Actif'}</span>
                  </td>
                  <td className="px-5 py-4 text-center">
      ...
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting}
        title="Supprimer le membre" message={`Supprimer  ${confirmDelete?.nom}  ? Cette action est irrversible.`} />
    </div>
  );
}
