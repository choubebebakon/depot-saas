...
export default function CaissePageSecure() {
  ...
  const total = useMemo(
    () => cart.reduce((sum, line) => sum + line.prixUnitaire * line.quantite, 0),
    [cart],
  );
  ...
  const openMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowOpen(false);
      queryClient.invalidateQueries({ queryKey: ['depot-caisse-statut', depotId] });
      notif.success('Caisse ouverte.');
    },
    onError: (error) => notif.error(errorMessage(error, 'Impossible d’ouvrir la caisse.')),
  });

  const closeMutation = useMutation({
    mutationFn: () => depotApi.fermerCaisse({ depotId, fondFinal: caisse?.solde ?? 0, ecart: 0 }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['depot-caisse-statut', depotId] });
      notif.success('Caisse fermée.');
    },
  ...
  const movementMutation = useMutation({
    ...
    onSuccess: () => {
      setMovement({ typeMouvement: 'ENTREE', montant: '', motif: '' });
      setShowMovement(false);
      queryClient.invalidateQueries({ queryKey: ['depot-caisse-statut', depotId] });
      notif.success('Mouvement de caisse enregistré.');
    },
  ...
  function addArticle(article) {
    if (article.quantite <= 0) {
      notif.error(`Stock indisponible pour ${article.designation}.`);
      return;
    }
    setCart((current) => {
      ...
      if (found) {
        if (found.quantite >= article.quantite) {
          notif.error('Quantité maximale disponible atteinte.');
          return current;
        }
        return current.map((line) => line.articleId === article.id ? { ...line, quantite: line.quantite + 1 } : line);
      }
      return [...current, { articleId: article.id, designation: article.designation, quantite: 1, prixUnitaire: Number(article.prix || 0), stockDisponible: article.quantite }];
    });
  }
  ...
  const canSubmit = canWrite && Boolean(caisse?.statut === 'OUVERTE') && cart.length > 0 && total > 0 && paymentValid && !venteMutation.isPending;

  return (
    <div className="p-4 md:p-6 space-y-5 text-white">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-widest text-amber-400 font-bold">Dépôt de boissons</p>
          <h1 className="text-2xl font-black">Caisse / Point de vente</h1>
        </div>
        <div className="flex flex-wrap gap-2">
          {caisse?.statut === 'OUVERTE' ? (
            <>
              <span className="px-3 py-2 rounded-xl bg-emerald-500/15 text-emerald-300 text-sm font-bold">● Caisse ouverte · {money(caisse.solde)} FCFA</span>
              <button disabled={!canWrite || closeMutation.isPending} onClick={() => closeMutation.mutate()} className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-sm font-bold">Fermer</button>
              <button disabled={!canWrite} onClick={() => setShowMovement(true)} className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-sm font-bold">Mouvement</button>
            </>
          ) : (
            <button disabled={!canWrite || openMutation.isPending} onClick={() => setShowOpen(true)} className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-sm font-black">Ouvrir la caisse</button>
          )}
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[1.35fr_.9fr] gap-5">
        <section className="rounded-2xl border border-slate-800 bg-slate-900/70 overflow-hidden">
          <div className="p-4 border-b border-slate-800">
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher un article…" className="w-full rounded-xl bg-slate-800 border border-slate-700 px-4 py-3 outline-none focus:border-amber-500" />
          </div>
          <div className="p-4 grid sm:grid-cols-2 xl:grid-cols-3 gap-3 max-h-[60vh] overflow-y-auto">
            {articlesLoading ? <p className="text-slate-500 col-span-full">Chargement des articles…</p> : articles.map((article) => (
              <button key={article.id} type="button" onClick={() => addArticle(article)} className="text-left p-4 rounded-xl border border-slate-800 hover:border-amber-500/60 bg-slate-950/50 transition-colors">
                <p className="font-bold truncate">{article.designation}</p>
                <p className="text-amber-300 font-black mt-1">{money(article.prix)} FCFA</p>
                <p className={`text-xs mt-2 ${article.quantite > 0 ? 'text-slate-400' : 'text-red-400'}`}>Stock : {article.quantite}</p>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 space-y-4">
          ...
          <div className="space-y-2 max-h-56 overflow-y-auto">
            {!cart.length && <p className="text-sm text-slate-500 py-8 text-center border border-dashed border-slate-800 rounded-xl">Ajoutez des articles.</p>}
            {cart.map((line) => (
              <div key={line.articleId} className="p-3 rounded-xl bg-slate-800/60 border border-slate-700/60">
                <div className="flex justify-between gap-3"><span className="font-semibold truncate">{line.designation}</span><button type="button" onClick={() => setCart((c) => c.filter((x) => x.articleId !== line.articleId))} className="text-red-400">×</button></div>
                <div className="flex items-center justify-between mt-2"><input type="number" min="1" max={line.stockDisponible} value={line.quantite} onChange={(e) => setQuantity(line.articleId, e.target.value)} className="w-20 bg-slate-900 border border-slate-700 rounded-lg px-2 py-1" /><span className="font-bold">{money(line.prixUnitaire * line.quantite)} FCFA</span></div>
              </div>
            ))}
          </div>
