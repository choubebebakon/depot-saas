...
import { Factory, Handshake, Pencil, Plus, RefreshCw, Search, Wallet } from 'lucide-react';
...
export default function FournisseursPage() {
  ...
  const fournisseurs = useMemo(() => {
    ...
    if (!term) return raw;
    return raw.filter((f) => [f.nom, f.telephone, f.email, f.adresse, f.contact].filter(Boolean).some((value) => String(value).toLowerCase().includes(term)));
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {totalItems === 0 ? <div className="col-span-full rounded-xl border border-slate-700/50 bg-slate-800/30 p-12 text-center"><Handshake className="mx-auto mb-3 text-slate-600" size={34} /><p className="text-lg font-medium text-slate-300">{search ? 'Aucun fournisseur trouvé' : 'Aucun fournisseur'}</p><p className="mt-1 text-sm text-slate-500">{search ? 'Modifiez votre recherche.' : 'Ajoutez votre premier fournisseur.'}</p></div> : paginated.map((fournisseur) => {
          const dette = Number(fournisseur.dette ?? fournisseur.solde ?? 0);
          const soldeInitial = Number(fournisseur.soldeInitial ?? 0);
          return <div key={fournisseur.id} className="flex flex-col justify-between rounded-xl border border-slate-700/50 bg-slate-800/60 p-5 transition-all hover:border-blue-500/30"><div><div className="mb-3 flex items-start justify-between"><div><p className="font-bold text-white">{fournisseur.nom}</p><p className="mt-0.5 text-xs text-slate-500">{fournisseur.telephone || '-'}</p>{fournisseur.email && <p className="text-xs text-slate-500">{fournisseur.email}</p>}{fournisseur.adresse && <p className="mt-1 text-xs text-slate-400">{fournisseur.adresse}</p>}</div>{canWrite && <button type="button" onClick={() => openEdit(fournisseur)} className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-xs text-slate-500 hover:bg-slate-700/60 hover:text-white"><Pencil size={13} /> Modifier</button>}</div><div className="mb-3 rounded-lg border border-slate-600/40 bg-slate-900/50 p-2"><p className="text-[10px] uppercase tracking-wider text-slate-500">Solde initial</p><p className="text-sm font-bold text-slate-200">{soldeInitial.toLocaleString('fr-FR')} FCFA</p></div>{dette > 0 && <div className="mb-3 rounded-lg border border-red-500/30 bg-red-500/10 p-2 text-center"><p className="text-[10px] uppercase tracking-wider text-red-400">Dette</p><p className="text-sm font-bold text-red-400">{dette.toLocaleString('fr-FR')} FCFA</p></div>}</div><div className="mt-4 flex flex-wrap gap-1.5">{dette > 0 && <button type="button" onClick={() => { setSelectedFournisseur(fournisseur); setShowModal('regler'); }} className="inline-flex items-center gap-1.5 rounded-lg bg-amber-600/80 px-3 py-1.5 text-[10px] font-bold text-white transition hover:bg-amber-500"><Wallet size={13} /> Régler</button>}</div></div>;
        })}
      </div>
      ...
      <FournisseurForm isOpen={formOpen} onClose={() => { setFormOpen(false); setEditItem(null); }} edit={editItem} metier="depot-boissons" depotId={depotId} />
    </div>
  );
}
