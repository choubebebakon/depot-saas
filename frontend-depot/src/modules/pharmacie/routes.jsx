...
const DashboardPharmacie = lazy(() => import('./pages/DashboardPharmacie'));
const MedicamentsPage    = lazy(() => import('./pages/MedicamentsPage'));
const OrdonnancesPage    = lazy(() => import('./pages/OrdonnancesPage'));
const AlertesDlcPage     = lazy(() => import('./pages/AlertesDlcPage'));
...
function MetierGuard({ children }) {
  ...
  if (metier && metier !== 'PHARMACIE') return <Navigate to="/onboarding/metier" replace />;
  if (!metier && stored && stored !== 'PHARMACIE') return <Navigate to="/onboarding/metier" replace />;
  return children;
}

function PharmacieLayout() {
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 px-6 flex items-center justify-between z-40">
          ...
          <div className="flex-1 flex justify-end items-center gap-6">
            ...
            <div className="flex items-center gap-3 bg-slate-800/50 px-3 py-1.5 rounded-2xl border border-slate-700 shadow-inner">
              <div className="w-7 h-7 bg-emerald-500 rounded-full flex items-center justify-center text-white font-bold text-[10px]">
                {user?.email?.[0]?.toUpperCase()}
              </div>
              <div className="hidden md:flex flex-col">
                <span className="text-white text-xs font-semibold leading-none">{user?.email}</span>
                <span className="text-emerald-400 text-[9px] font-black uppercase tracking-widest mt-0.5">Pharmacie</span>
              </div>
            </div>
          </div>
        </header>
...
export default function PharmacieRoutes() {
  return (
    <MetierGuard>
      <Routes>
        <Route element={<PharmacieLayout />}>
          <Route path="dashboard"     element={<DashboardPharmacie />} />
          <Route path="medicaments"   element={<MedicamentsPage />} />
          <Route path="ordonnances"   element={<OrdonnancesPage />} />
          <Route path="alertes-dlc"   element={<AlertesDlcPage />} />
