  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={() => { setEdit(null); setForm({ nom: '', role: 'COIFFEUR', telephone: '', email: '', specialite: '', salaire: '' }); setShowModal(true); }} className="bg-pink-500 hover:bg-pink-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all">+ Nouvel employ</button>
      </div>
      ...
      <div><label className="text-slate-400 text-xs font-bold uppercase block mb-1.5">Rle</label><select value={form.role} onChange={e => setForm({...form, role: e.target.value })} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white w-full text-sm"><option value="COIFFEUR">Coiffeur</option><option value="ESTHETICIEN">Esthticien</option><option value="MANUCURE">Manucure</option><option value="RECEPTEUR">Rcepteur</option><option value="GERANT">Grant</option></select></div>
