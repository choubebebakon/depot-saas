    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">{[{k: 'JOUR', l: 'Jour'},{k: 'SEMAINE', l: 'Semaine'},{k: 'MOIS', l: 'Mois'},{k: 'ANNEE', l: 'Année'}].map(p => (
          <button key={p.k} onClick={() => setPeriode(p.k)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${periode === p.k ? 'bg-pink-500 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>{p.l}</button>
        ))}</div>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: '📈', label: 'Marge', value: `${(rapport?.marge || 0).toLocaleString('fr-FR')} F`, color: '#ec4899' },
