...
const initialState = { article: '', quantite: '', seuilAlerte: '' };

export default function StockGlacierForm({ isOpen, onClose, onSuccess, edit, metier = 'glacier_snack' }) {
  ...
  useEffect(() => {
    if (edit) setForm({ article: edit.article || '', quantite: edit.quantite || '', seuilAlerte: edit.seuilAlerte || edit.seuil || '' });
    else setForm({ ...initialState });
  }, [edit, isOpen]);
  const prefix = `/${metier}`;
  const validate = () => { const errs = {}; if (!form.article) errs.article = "L'article est requis"; if (!form.quantite || Number(form.quantite) < 0) errs.quantite = 'La quantité est requise'; return errs; };
  const handleSubmit = async (e) => {
    e.preventDefault(); const errs = validate(); setErrors(errs);
    if (Object.keys(errs).length > 0) return; setLoading(true);
    try { if (edit) await api.patch(`${prefix}/stock/${edit.id}`, form); else await api.post(`${prefix}/stock`, form); onSuccess(); onClose(); }
