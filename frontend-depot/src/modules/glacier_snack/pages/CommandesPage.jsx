...
export default function CommandesPage() {
  ...
  const { data: dataData = [], loading, refetch } = useData(`/${prefix}/commandes`, { enabled: true });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-white tracking-tight">Commandes</h1><p className="text-slate-400 text-sm">{totalItems} commande(s)</p></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 border border-slate-700 rounded-2xl p-8 w-full max-w-lg shadow-2xl overflow-y-auto max-h-[90vh] animate-slide-up">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-white font-black text-xl">{editItem ? '?? Modifier' : '? Nouvelle'} commande</h3>
            ...
            <form onSubmit={handleFormSubmit}>
              <div className="space-y-4">
                ...
                <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1.5 block">Statut</label><select value={form.statut} onChange={setFormField('statut')} className="w-full bg-slate-800 border border-slate-600 text-white rounded-xl px-4 py-3 text-sm outline-none transition-all"><option value="EN_ATTENTE">En attente</option><option value="EN_PREPARATION">En prparation</option><option value="PRETE">Prte</option></select></div>
              </div>
