...
;
  ...
  const cards = [
    { label: 'Ventes téléphones', value: stats?.ventes || 0, icon: '📱', color: '#7c3aed' },
    { label: 'Accessoires', value: stats?.accessoires || 0, icon: '🎧', color: '#3b82f6' },
    { label: 'Réparations', value: stats?.reparations || 0, icon: '🔧', color: '#f59e0b' },
    { label: 'Recharges', value: stats?.recharges || 0, icon: '🔋', color: '#10b981' },
    { label: 'Total clients', value: stats?.clients || 0, icon: '👥', color: '#ec4899' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          ...
          <div className="flex items-end gap-2 h-48">
            {[8, 12, 10, 15, 18, 22, 20, 24, 18, 22, 26, 28].map((val, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[9px] text-slate-400 font-semibold">{val}M</span>
                <div className="w-full rounded-lg bg-purple-500/70 transition-all" style={{ height: `${val * 3}px` }} />
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {[
              { label: 'Vente téléphones', pct: 50, color: '#7c3aed' },
              { label: 'Accessoires', pct: 20, color: '#3b82f6' },
              { label: 'Réparations', pct: 18, color: '#f59e0b' },
              { label: 'Recharges', pct: 12, color: '#10b981' },
            ].map((item, i) => (
              <div key={i}>
