...
export default function FournisseursPage() {
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6"><input type="text" placeholder="🔍 Nom ou spcialit..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-purple-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-60" /></div>
      ...
      {formOpen && <FournisseurForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Fournisseur modifié ?' : 'Fournisseur cr ?'); refetch(); }} edit={editItem} metier={prefix} />}
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer le fournisseur" message={`tes-vous sr de vouloir suppriméer "${confirmDelete?.nom}" ?`} />
    </div>
  );
}
