...
export default function VentesPage() {
  ...
  const [form, setForm] = useState({ telephoneId: '', accessoireId: '', clientId: '', quantite: 1, montant: '', modePaiement: 'Espces', dateVente: '' });
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6"><input type="text" placeholder="🔍 Tlphone ou accessoire..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-purple-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-60" /></div>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '?? Modifier vente' : '?? Nouvelle vente'} loading={saving} submitLabel={editItem ? 'Modifier' : 'Crer'}>
        <div className="grid grid-cols-2 gap-4">
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Tlphone</label><select value={form.telephoneId} onChange={setF('telephoneId')} className={inputClass}><option value="">Aucun</option>{telephones.map(t => <option key={t.id} value={t.id}>{t.marque} {t.modele}</option>)}</select></div>
          ...
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Paiement</label><select value={form.modePaiement} onChange={setF('modePaiement')} className={inputClass}><option>Espces</option><option>Mobile Money</option><option>Carte</option></select></div>
      ...
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer la vente" message={`tes-vous sr de vouloir suppriméer cette vente ?`} />
    </div>
  );
}
