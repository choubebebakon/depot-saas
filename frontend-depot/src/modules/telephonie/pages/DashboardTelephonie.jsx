...
return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: '📱', label: 'Téléphones stock', value: stats?.telephonesStock || 0, color: '#7c3aed' },
          { icon: '🔧', label: 'Réparations en cours', value: stats?.reparationsEnCours || 0, color: '#ef4444' },
          { icon: '💰', label: 'CA du jour', value: `${(stats?.caJour || 0).toLocaleString('fr-FR')} F`, color: '#10b981' },
          { icon: '🔋', label: 'Recharges jour', value: stats?.rechargesJour || 0, color: '#f59e0b' },
        ].map((k, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">👨‍🔧 Techniciens</h2>
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 rounded-2xl bg-blue-500/20 flex items-center justify-center"><span className="text-4xl">👨‍🔧</span></div>
            <div><p className="text-white font-black text-5xl">{stats?.techniciens || 0}</p><p className="text-slate-400 text-sm">techniciens actifs</p></div>
          </div>
        </div>
      </div>
    </div>
