...
export default function PersonnelPage() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        await api.patch(`/${prefix}/personnel/${editItem.id}`, formData);
      } else {
        await api.post(`/${prefix}/personnel`, formData);
      }
      success(editItem ? 'Employ modifié' : 'Employ cr');
      setFormOpen(false);
      setEditItem(null);
      refetch();
    } catch (err) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        {perm.canCreate && <button onClick={openCreate} className="bg-teal-600 hover:bg-teal-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-teal-600/20">+ Nouvel Employ</button>}
      </div>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleFormSubmit} title={editItem ? '?? Modifier employ' : '?? Nouvel employ'} loading={formLoading}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Poste</label><select value={formData.poste} onChange={set('poste')} className={inputClass}><option value="AGENT_IMMO">Agent immobilier</option><option value="GERANT">Grant</option><option value="COMPTABLE">Comptable</option><option value="COMMERCIAL">Commercial</option><option value="CONCIERGE">Concierge</option></select></div>
          ...
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Tlphone</label><input required value={formData.telephone} onChange={set('telephone')} className={inputClass} /></div>
        </div>
      </FormModal>
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer l'employ" message={`Supprimer  ${confirmDelete?.nom}  ? Cette action est irrversible.`} />
    </div>
  );
}
