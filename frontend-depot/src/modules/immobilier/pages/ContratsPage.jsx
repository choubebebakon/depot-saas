...
export default function ContratsPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/contrats/${confirmDelete.id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Contrats</h1><p className="text-slate-400 text-sm mt-1">{totalItems} contrat{totalItems !== 1 ? 's' : ''}</p></div>
      ...
      <ContratLocationForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} metier={prefix} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer le contrat" message={`Supprimer le contrat pour  ${confirmDelete?.bien?.nom}  ? Cette action est irrversible.`} />
    </div>
  );
}
