...
export default function BiensPage() {
  ...
  const perm = usePermission(PERMISSIONS, 'biens');

  const { data: biensData = [], loading, refetch } = useData(`/${prefix}/biens`, { enabled: true });
  const biens = Array.isArray(biensData?.data) ? biensData.data : (Array.isArray(biensData) ? biensData : []);

  // Pagination centralisÃ©e â FIX: totalPages non dÃ©fini
  const filtres = (biens || []).filter(item =>
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/biens/${confirmDelete.id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Biens Immobiliers</h1><p className="text-slate-400 text-sm mt-1">{totalItems} bien{totalItems !== 1 ? 's' : ''}</p></div>
                ...
                <tr key={b.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center">
                    <div className="flex justify-center gap-1">
                      {perm.canEdit && <button onClick={() => { setEditItem(b); setFormOpen(true); }} className="text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-700 text-sm transition-colors">✏️ Modifier</button>}
                      {perm.canDelete && <button onClick={() => setConfirmDelete(b)} className="text-slate-400 hover:text-red-400 p-1.5 rounded-lg hover:bg-slate-700 text-sm transition-colors">🗑️ Supprimer</button>}
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <BienImmobilierForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} metier={prefix} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer le bien" message={`Supprimer  ${confirmDelete?.nom}  ? Cette action est irrversible.`} />
    </div>
  );
}
