...
export default function InterventionsPage() {
  ...
  const perm = usePermission(PERMISSIONS, 'interventions');

  const { data: itemsData = [], loading, refetch } = useData(`/${prefix}/interventions`, { enabled: true });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/interventions/${confirmDelete.id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Interventions</h1><p className="text-slate-400 text-sm mt-1">{totalItems} intervention{totalItems !== 1 ? 's' : ''}</p></div>
      ...
      <InterventionBienForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} metier={prefix} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer l'intervention" message={`Supprimer l'intervention  ${confirmDelete?.type}  ? Cette action est irrversible.`} />
    </div>
  );
}
