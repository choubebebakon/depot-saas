...
import FlotteForm from '../forms/FlotteForm';
...
export default function FlottePage() {
  ...
  const { data: dataData = [], loading, refetch } = useData(`/${prefix}/vehicules`, { enabled: true });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-white tracking-tight">Flotte</h1><p className="text-slate-400 text-sm">{totalItems} vhicule(s)</p></div>
        <button onClick={openCreate} className="bg-orange-600 hover:bg-orange-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm">+ Nouveau vhicule</button></div>
      ...
      <FlotteForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} message={confirmDelete ? `Supprimer le vhicule "${confirmDelete.immatriculation}" ?` : ''} loading={deleting} />
    </div>
  );
}
