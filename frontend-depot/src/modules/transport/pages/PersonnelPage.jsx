    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">{totalItems} employ(s)</p>
        </div>
        <button onClick={openCreate} className="bg-orange-600 hover:bg-orange-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm">+ Nouvel employ</button>
      </div>
                ...
                <tr key={p.id} className="border-t border-slate-800 hover:bg-slate-800/40">
                  ...
                  <td className="p-4 text-center">
                    <div className="flex justify-center gap-2">
                      <button onClick={() => openEdit(p)} className="text-orange-400 text-xs">✏️ Modifier</button>
                      <button onClick={() => setConfirmDelete(p)} className="text-red-400 text-xs">🗑️ Supprimer</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <PersonnelForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} />
