...
import TrajetForm from '../forms/TrajetForm';
...
export default function TrajetsPage() {
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/transport/trajets/${confirmDelete.id}`); setConfirmDelete(null); load(); }
    catch { alert('Erreur lors de la suppression'); } finally { setDeleting(false); }
  };


  // Pagination centralisÃ©e â€” FIX: totalPages non dÃ©fini
  const filtres = (trajets || []).filter(item =>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-white tracking-tight">🚛 Trajets</h1><p className="text-slate-400 text-sm">{total} trajet(s)</p></div>
        <button onClick={openCreate} className="bg-orange-600 hover:bg-orange-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm">+ Nouveau trajet</button></div>
