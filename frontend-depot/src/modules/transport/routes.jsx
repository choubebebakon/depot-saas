...
const DashboardTransport = lazy(() => import('./pages/DashboardTransport'));
const ColisPage          = lazy(() => import('./pages/ColisPage'));
const TrajetsPage        = lazy(() => import('./pages/TrajetsPage'));
...
function Loader() { return <div className="flex items-center justify-center py-32"><div className="w-10 h-10 border-4 border-orange-500 border-t-transparent rounded-full animate-spin" /></div>; }

function MetierGuard({ children }) {
  const { metier } = useAuth(); const stored = localStorage.getItem('gestock_metier');
  if (metier && metier !== 'TRANSPORT') return <Navigate to="/onboarding/metier" replace />;
  if (!metier && stored && stored !== 'TRANSPORT') return <Navigate to="/onboarding/metier" replace />;
  return children;
}
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 px-6 flex items-center justify-between z-40">
          ...
          <div className="flex-1 flex justify-end items-center gap-6">
            ...
            <div className="flex items-center gap-3 bg-slate-800/50 px-3 py-1.5 rounded-2xl border border-slate-700 shadow-inner">
              <div className="w-7 h-7 bg-orange-600 rounded-full flex items-center justify-center text-white font-bold text-[10px]">{user?.email?.[0]?.toUpperCase()}</div>
              <div className="hidden md:flex flex-col"><span className="text-white text-xs font-semibold leading-none">{user?.email}</span><span className="text-orange-400 text-[9px] font-black uppercase tracking-widest mt-0.5">Transport</span></div>
            </div>
          </div>
        </header>
    ...
    <MetierGuard>
      <Routes>
        <Route element={<TransportLayout />}>
          <Route path="dashboard"    element={<DashboardTransport />} />
