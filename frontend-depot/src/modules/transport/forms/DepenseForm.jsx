...
const initialState = { libelle: '', categorie: 'CARBURANT', montant: '', date: '' };

export default function DepenseForm({ isOpen, onClose, onSuccess, edit, metier = 'transport' }) {
  ...
  useEffect(() => {
    if (edit) setForm({ libelle: edit.libelle || '', categorie: edit.categorie || 'CARBURANT', montant: edit.montant || '', date: edit.date || '' });
    else setForm({ ...initialState });
  }, [edit, isOpen]);
  const prefix = `/${metier}`;
  const validate = () => { const errs = {}; if (!form.libelle) errs.libelle = 'Le libellé est requis'; if (!form.montant || Number(form.montant) <= 0) errs.montant = 'Le montant est requis'; return errs; };
