  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-3xl font-black text-white tracking-tight">Dpenses</h1><p className="text-slate-400 text-sm">{totalItems} dpense(s)</p></div>
        <button onClick={openCreate} className="bg-purple-500 hover:bg-purple-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all">+ Nouvelle dpense</button>
      </div>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '?? Modifier dpense' : '?? Nouvelle dpense'} loading={saving} submitLabel={editItem ? 'Modifier' : 'Crer'}>
        <div><label className="text-slate-400 text-xs font-bold uppercase block mb-1.5">Catgorie</label><select value={form.categorie} onChange={set('categorie')} className={inputClass}>{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
        {['libelle','montant','dateDepense','notes'].map(f => (
          <div key={f}><label className="text-slate-400 text-xs font-bold uppercase block mb-1.5">{f === 'libelle' ? 'Libell' : f === 'montant' ? 'Montant (F)' : f === 'dateDepense' ? 'Date' : 'Notes'}</label><input type={f === 'montant' ? 'number' : f === 'dateDepense' ? 'date' : 'text'} value={form[f]} onChange={set(f)} className={inputClass} required={f === 'libelle' || f === 'montant'} /></div>
        ))}
      </FormModal>

      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting}
        title="Supprimer la dpense" message={`Supprimer la dpense  ${confirmDelete?.libelle}  (${Number(confirmDelete?.montant || 0).toLocaleString('fr-FR')} F) ?`} />
    </div>
  );
}
