...
export default function TicketsPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/tickets/${confirmDelete.id}`);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-3xl font-black text-white tracking-tight">??? Tickets Dpts</h1><p className="text-slate-400 text-sm">{totalItems} ticket(s)</p></div>
      ...
      <TicketPressingForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} />
      <RetraitPressingForm isOpen={retraitOpen} onClose={() => setRetraitOpen(false)} onSuccess={() => refetch()} metier={prefix} ticket={retraitTicket} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting}
        title="Supprimer le ticket" message={`Supprimer le ticket de ${confirmDelete?.clientNom || ''} ? Cette action est irrversible.`} />
    </div>
  );
}
