...
export default function ServicesPage() {
  ...
  const SERVICE_ICONS = { 'LAVAGE': '👕', 'REPASSAGE': '🔥', 'NETTOYAGE': '✨', 'TEINTURE': '🎨', 'RETOUCHE': '🪡' };
  const SERVICE_LABELS = { 'LAVAGE': 'Lavage', 'REPASSAGE': 'Repassage', 'NETTOYAGE': 'Nettoyage', 'TEINTURE': 'Teinture', 'RETOUCHE': 'Retouche' };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-3xl font-black text-white tracking-tight">Services</h1><p className="text-slate-400 text-sm">{totalItems} service(s)</p></div>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '?? Modifier service' : '?? Nouveau service'} loading={saving} submitLabel={editItem ? 'Modifier' : 'Crer'}>
        ...
        {['nom','description','prix','delai'].map(f => (
          <div key={f}><label className="text-slate-400 text-xs font-bold uppercase block mb-1.5">{f === 'nom' ? 'Nom' : f === 'description' ? 'Description' : f === 'prix' ? 'Prix (F)' : 'Dlai'}</label><input type={f === 'prix' ? 'number' : 'text'} value={form[f]} onChange={set(f)} className={inputClass} required={f === 'nom' || f === 'prix'} /></div>
        ))}
      </FormModal>

      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting}
        title="Supprimer le service" message={`Supprimer  ${confirmDelete?.nom}  ? Cette action est irrversible.`} />
    </div>
  );
}
