    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">{[{k: 'JOUR', l: 'Jour'},{k: 'SEMAINE', l: 'Semaine'},{k: 'MOIS', l: 'Mois'},{k: 'ANNEE', l: 'Année'}].map(p => (
          <button key={p.k} onClick={() => setPeriode(p.k)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${periode === p.k ? 'bg-purple-500 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>{p.l}</button>
        ))}</div>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: '📈', label: 'Marge', value: `${(rapport?.marge || 0).toLocaleString('fr-FR')} F`, color: '#7c3aed' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">🏷️ Tickets</h2>
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 rounded-2xl bg-purple-500/20 flex items-center justify-center"><span className="text-4xl">🏷️</span></div>
            <div><p className="text-white font-black text-5xl">{rapport?.totalTickets || 0}</p><p className="text-slate-400 text-sm">tickets traités</p></div>
          </div>
        </div>
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">🧼 Services</h2>
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 rounded-2xl bg-blue-500/20 flex items-center justify-center"><span className="text-4xl">🧼</span></div>
            <div><p className="text-white font-black text-5xl">{rapport?.services || 0}</p><p className="text-slate-400 text-sm">services réalisés</p></div>
          </div>
        </div>
      </div>
    </div>
