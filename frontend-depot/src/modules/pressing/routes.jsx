...
const DashboardPressing = lazy(() => import('./pages/DashboardPressing'));
...
function MetierGuard({ children }) {
  ...
  if (metier && metier !== 'PRESSING') return <Navigate to="/onboarding/metier" replace />;
  if (!metier && stored && stored !== 'PRESSING') return <Navigate to="/onboarding/metier" replace />;
  return children;
}

function PressingLayout() {
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 px-6 flex items-center justify-between z-40">
          ...
          <div className="flex-1 flex justify-end items-center gap-6">
            ...
            <div className="flex items-center gap-3 bg-slate-800/50 px-3 py-1.5 rounded-2xl border border-slate-700 shadow-inner">
              ...
              <div className="hidden md:flex flex-col"><span className="text-white text-xs font-semibold leading-none">{user?.email}</span><span className="text-purple-400 text-[9px] font-black uppercase tracking-widest mt-0.5">Pressing</span></div>
            </div>
          </div>
        </header>
...
export default function PressingRoutes() {
  return (
    <MetierGuard>
      <Routes>
        <Route element={<PressingLayout />}>
          <Route path="dashboard"  element={<DashboardPressing />} />
