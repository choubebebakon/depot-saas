  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-3xl font-black text-white tracking-tight">Clients</h1><p className="text-slate-400 text-sm">{totalItems} client(s)</p></div>
      ...
      {loading ? <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-fuchsia-500 border-t-transparent rounded-full animate-spin" /></div> : (
        <div className="overflow-x-auto rounded-2xl border border-slate-700/50">
          <table className="w-full text-sm"><thead className="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-wider"><tr><th className="text-left p-4">Nom</th><th className="text-left p-4">Tlphone</th><th className="text-left p-4">Email</th><th className="text-right p-4">Points</th><th className="text-center p-4">Actions</th></tr></thead><tbody>{paginated.map(c => (
            <tr key={c.id} className="border-t border-slate-800 hover:bg-slate-800/40"><td className="p-4 text-white font-semibold">{c.nom}</td><td className="p-4 text-slate-300">{c.telephone}</td><td className="p-4 text-slate-300">{c.email || '-'}</td><td className="p-4 text-right text-fuchsia-400 font-bold">{c.points || 0}</td><td className="p-4 text-center"><div className="flex justify-center gap-2"><button onClick={() => { setEdit(c); setShowForm(true); }} className="text-fuchsia-400 text-xs">✏️ Modifier</button><button onClick={() => setConfirm(c)} className="text-red-400 text-xs">🗑️ Supprimer</button></div></td></tr>
          ))}</tbody></table>
        </div>
      )}
      ...
      <ClientParfumerieForm isOpen={showForm} onClose={() => { setShowForm(false); setEdit(null); }} onSuccess={refetch} edit={edit} />
