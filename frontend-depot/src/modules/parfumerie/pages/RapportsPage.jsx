...
;
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-white tracking-tight">📈 Rapports</h1><p className="text-slate-400 text-sm">Période : {periode}</p></div>
        <div className="flex gap-2">{[{k: 'JOUR', l: 'Jour'},{k: 'SEMAINE', l: 'Semaine'},{k: 'MOIS', l: 'Mois'},{k: 'ANNEE', l: 'Année'}].map(p => (
          <button key={p.k} onClick={() => setPeriode(p.k)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${periode === p.k ? 'bg-fuchsia-500 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>{p.l}</button>
        ))}</div>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: '📈', label: 'Marge', value: `${(rapport?.marge || 0).toLocaleString('fr-FR')} F`, color: '#d946ef' },
          { icon: '📊', label: 'Évolution', value: rapport?.evolution || '0%', color: '#3b82f6' },
        ].map((k, i) => (
          <div key={i} className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5"><div className="w-11 h-11 rounded-xl flex items-center justify-center text-2xl mb-3" style={{backgroundColor: k.color+'22'}}>{k.icon}</div><p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">{k.label}</p><p className="text-white font-black text-2xl leading-none">{k.value}</p></div>
        ))}
      </div>
    </div>
  );
