...
const initialState = { clientNom: '', points: '', operation: 'AJOUT', motif: '' };
export default function FideliteParfumerieForm({ isOpen, onClose, onSuccess, edit, metier = 'parfumerie' }) {
  ...
  const prefix = `/${metier}`;
  const validate = () => { const errs = {}; if (!form.clientNom?.trim()) errs.clientNom = 'Client requis'; return errs; };
  const handleSubmit = async (e) => { e.preventDefault(); const errs = validate(); if (Object.keys(errs).length) { setErrors(errs); return; } setLoading(true); setErrors({}); try { if (edit) await api.patch(`${prefix}/fidelite/${edit.id}`, form); else await api.post(`${prefix}/fidelite`, form); onSuccess(); onClose(); } catch (err) { setErrors({ general: err.response?.data?.message || "Erreur lors de l'enregistréement" }); } finally { setLoading(false); } };
  return (
