...
const initialState = { nom: '', role: 'VENDEUR', telephone: '', salaire: '' };
export default function PersonnelParfumerieForm({ isOpen, onClose, onSuccess, edit, metier = 'parfumerie' }) {
  ...
  const handleSubmit = async (e) => { e.preventDefault(); const errs = validate(); if (Object.keys(errs).length) { setErrors(errs); return; } setLoading(true); setErrors({}); try { if (edit) await api.patch(`${prefix}/personnel/${edit.id}`, form); else await api.post(`${prefix}/personnel`, form); onSuccess(); onClose(); } catch (err) { setErrors({ general: err.response?.data?.message || "Erreur lors de l'enregistréement" }); } finally { setLoading(false); } };
  return (
