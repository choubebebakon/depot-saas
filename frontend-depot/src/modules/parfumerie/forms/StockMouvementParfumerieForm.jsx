...
const initialState = { produitNom: '', type: 'ENTREE', quantite: '', motif: '' };
export default function StockMouvementParfumerieForm({ isOpen, onClose, onSuccess, edit, metier = 'parfumerie' }) {
  ...
  const prefix = `/${metier}`;
  const validate = () => { const errs = {}; if (!form.produitNom?.trim()) errs.produitNom = 'Produit requis'; if (!form.quantite || Number(form.quantite) <= 0) errs.quantite = 'Quantité requise'; return errs; };
  const handleSubmit = async (e) => { e.preventDefault(); const errs = validate(); if (Object.keys(errs).length) { setErrors(errs); return; } setLoading(true); setErrors({}); try { if (edit) await api.patch(`${prefix}/stock/mouvement/${edit.id}`, form); else await api.post(`${prefix}/stock/mouvement`, form); onSuccess(); onClose(); } catch (err) { setErrors({ general: err.response?.data?.message || "Erreur lors de l'enregistréement" }); } finally { setLoading(false); } };
  return (
