...
const initialState = { nom: '', prix: '', categorie: 'PARFUM', stock: '' };
export default function ProduitParfumerieForm({ isOpen, onClose, onSuccess, edit, metier = 'parfumerie' }) {
  ...
  const prefix = `/${metier}`;
  const validate = () => { const errs = {}; if (!form.nom?.trim()) errs.nom = 'Nom requis'; if (!form.prix || Number(form.prix) <= 0) errs.prix = 'Prix requis'; return errs; };
  const handleSubmit = async (e) => { e.preventDefault(); const errs = validate(); if (Object.keys(errs).length) { setErrors(errs); return; } setLoading(true); setErrors({}); try { if (edit) await api.patch(`${prefix}/produits/${edit.id}`, form); else await api.post(`${prefix}/produits`, form); onSuccess(); onClose(); } catch (err) { setErrors({ general: err.response?.data?.message || "Erreur lors de l'enregistréement" }); } finally { setLoading(false); } };
  return (
