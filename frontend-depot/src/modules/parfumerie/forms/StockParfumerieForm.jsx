...
const initialState = { produit: '', quantite: '', prix: '', seuilAlerte: '' };
export default function StockParfumerieForm({ isOpen, onClose, onSuccess, edit, metier = 'parfumerie' }) {
  ...
  const prefix = `/${metier}`;
  const validate = () => { const errs = {}; if (!form.produit?.trim()) errs.produit = 'Produit requis'; return errs; };
  const handleSubmit = async (e) => { e.preventDefault(); const errs = validate(); if (Object.keys(errs).length) { setErrors(errs); return; } setLoading(true); setErrors({}); try { if (edit) await api.patch(`${prefix}/stock/${edit.id}`, form); else await api.post(`${prefix}/stock`, form); onSuccess(); onClose(); } catch (err) { setErrors({ general: err.response?.data?.message || "Erreur lors de l'enregistréement" }); } finally { setLoading(false); } };
  return (
