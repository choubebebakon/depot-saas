...
export default function InventairePage() {
  ...
  // edits : { [produitId]: valeurStockReel }
  ...
  const produits = Array.isArray(produitsData?.data)
    ? produitsData.data
  ...
  const rayons = Array.isArray(rayonsData?.data)
    ? rayonsData.data
    : (Array.isArray(rayonsData) ? rayonsData : []);

  // ── Handler : modification locale du stock réel ──────────────────────────
  const handleStockEdit = useCallback((produitId, value) => {
    setEdits(prev => ({ ...prev, [produitId]: value }));
  }, []);

  // ── Handler : sauvegarde du stock ajusté via PATCH ────────────────────────
  const saveStock = useCallback(async (produit) => {
    const stockReel = parseInt(edits[produit.id], 10);
    if (isNaN(stockReel) || stockReel < 0) {
      notifError('Valeur de stock invalide');
      return;
    }
    setSaving(prev => ({ ...prev, [produit.id]: true }));
    try {
      await api.patch(`/${prefix}/produits/${produit.id}/stock`, {
        stock: stockReel,
        motif: 'Ajustement inventaire',
      });
      // Mise à jour optimiste du cache
      queryClient.invalidateQueries({ queryKey: ['supermarche-articles'] });
      // Nettoyer l'edit pour ce produit
      setEdits(prev => {
        const next = { ...prev };
        delete next[produit.id];
        return next;
      });
      success(`Stock de ${produit.nom} ajusté à ${stockReel}`);
    } catch (err) {
      notifError(err.response?.data?.message || `Erreur lors de l'ajustement du stock`);
    } finally {
      setSaving(prev => ({ ...prev, [produit.id]: false }));
    }
  }, [edits, prefix, queryClient, success, notifError]);

  // ── Métriques ────────────────────────────────────────────────────────────
  const ruptureCount = produits.filter(i => (i.quantite || i.stock || 0) === 0).length;
  const faibleCount = produits.filter(i => {
    const q = i.quantite || i.stock || 0;
    return q > 0 && q <= (i.seuil || i.seuilAlerte || 5);
  }).length;
  const totalValeur = produits.reduce(
    (acc, i) => acc + (i.valeurStock || i.valeur || ((i.quantite || i.stock || 0) * (i.prixAchat || i.prix || 0))),
    0
  );
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            <BarChart3 className="w-6 h-6" /> Inventaire
          </h1>
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {filtres.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center py-16 text-slate-500">Aucun produit trouvé</td>
                </tr>
              ) : filtres.map(p => {
                const rayon = rayons.find(r => r.id === p.rayonId);
                const stockActuel = p.stock || p.quantite || 0;
                ...
                const isFaible = stockActuel > 0 && stockActuel <= (p.seuilAlerte || p.seuil || 5);

                return (
                  <tr
                    key={p.id}
                    className={`transition-colors ${isRupture ? 'bg-red-500/5' : isFaible ? 'bg-amber-500/5' : 'hover:bg-slate-700/20'}`}
                  >
