...
export default function ArticleSupermarcheForm({ isOpen, onClose, onSuccess, edit }) {
  ...
  const mutation=useMutation({ mutationFn:async(data)=>{ const {rayonId,prixGros,datePeremption,...fields}=data; const parsed=datePeremption?new Date(datePeremption):null; if(parsed&&Number.isNaN(parsed.getTime())) throw new Error('Date de péremption invalide'); const payload={...fields, prixAchat:data.prixAchat===''?undefined:Number(data.prixAchat), prixGros:prixGros===''?undefined:Number(prixGros), seuilCritique:Number(data.seuilCritique), codeBarres:data.codeBarres||undefined, photoUrl:data.photoUrl||undefined, datePeremption:parsed?parsed.toISOString():null}; if(edit){const r=await supermarcheApi.updateArticle(edit.id,payload); if(rayonId) await supermarcheApi.assignArticleToRayon(rayonId,edit.id); return r.data;} const r=await supermarcheApi.createArticle(payload); const a=r.data?.data??r.data; if(rayonId&&a?.id) await supermarcheApi.assignArticleToRayon(rayonId,a.id); return a; }, onSuccess:()=>{queryClient.invalidateQueries({queryKey:['supermarche-articles']});queryClient.invalidateQueries({queryKey:['supermarche-stock']});queryClient.invalidateQueries({queryKey:['supermarche-rayons']});queryClient.invalidateQueries({queryKey:['supermarche-dashboard']});notif.success(edit?'Article mis à jour':'Article créé avec succès');onSuccess?.();onClose();}, onError:(err)=>notif.error(err.response?.data?.message||err.message||'Une erreur est survenue') });
  const values=watch();
