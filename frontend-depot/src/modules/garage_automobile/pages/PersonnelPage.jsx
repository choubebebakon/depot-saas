  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {perm.canCreate && <button onClick={() => { setEditItem(null); setShowModal(true); }} className="bg-orange-500 hover:bg-orange-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all shadow-lg shadow-orange-500/20">+ Nouvel Employ</button>}
      </div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(p => (
                <tr key={p.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center">{perm.canEdit && <button onClick={() => { setEditItem(p); setShowModal(true); }} className="text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-700 text-sm transition-colors">✏️ Modifier</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showModal && <ModalPersonnel onClose={() => setShowModal(false)} onSuccess={() => { success(editItem ? 'Employ modifié ?' : 'Employ cr ?'); refetch(); }} edit={editItem} />}
    </div>
  );
}

// Fallback automatique pour éviter le crash is not defined
function ModalPersonnel({ isOpen, onClose, onSuccess, edit, ...props }) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-lg p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-white">Sélection / Formulaire (ModalPersonnel)</h2>
