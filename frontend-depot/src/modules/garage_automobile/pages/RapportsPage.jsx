...
;
  ...
  const cards = [
    { label: "Main-d'œuvre", value: stats?.mainOeuvre || 0, icon: '🔧', color: '#f97316' },
    { label: 'Pièces détachées', value: stats?.pieces || 0, icon: '⚙️', color: '#3b82f6' },
    { label: 'Diagnostic', value: stats?.diagnostic || 0, icon: '🩺', color: '#f59e0b' },
    { label: 'Autres services', value: stats?.autresServices || 0, icon: '🛠️', color: '#8b5cf6' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          ...
          <div className="flex items-end gap-2 h-48">
            {[12, 18, 15, 22, 25, 30, 28, 32, 26, 34, 38, 42].map((val, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[9px] text-slate-400 font-semibold">{val}M</span>
                <div className="w-full rounded-lg bg-orange-500/70 transition-all" style={{ height: `${val * 3}px` }} />
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">🔧 Répartition des revenus</h2>
          <div className="space-y-4">
            {[
              { label: 'Main-d\'œuvre', pct: 45, color: '#f97316' },
              { label: 'Pièces détachées', pct: 35, color: '#3b82f6' },
              { label: 'Diagnostic', pct: 10, color: '#f59e0b' },
              { label: 'Autres services', pct: 10, color: '#8b5cf6' },
            ].map((item, i) => (
              <div key={i}>
