...
export default function DepensesPage() {
  ...
  const MODES_PAIEMENT = ['Espces', 'Mobile Money', 'Carte', 'Virement'];
  const openCreate = () => { setEditItem(null); setForm({ libelle: '', categorie: 'Autre', montant: '', dateDepense: '', modePaiement: 'Espces', notes: '' }); setFormOpen(true); };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {perm.canCreate && <button onClick={openCreate} className="bg-orange-500 hover:bg-orange-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all shadow-lg shadow-orange-500/20">+ Nouvelle Dpense</button>}
      </div>
      <div className="flex gap-3 mb-6 flex-wrap">
        <input type="text" placeholder="🔍 Libell ou catgorie..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-orange-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-60" />
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '?? Modifier dpense' : '?? Nouvelle dpense'} loading={formLoading} submitLabel={editItem ? 'Modifier' : 'Crer'}>
        ...
        <FormField label="Libell" name="libelle" value={form.libelle} onChange={setFormField('libelle')} required placeholder="Libell de la dpense" />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField label="Catgorie" name="categorie" type="select" value={form.categorie} onChange={setFormField('categorie')} options={CATEGORIES} />
