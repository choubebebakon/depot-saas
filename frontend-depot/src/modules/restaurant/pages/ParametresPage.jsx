...
export default function ParametresPage() {
  ...
  const handleSave = async (e) => {
    ...
    try { await api.put('/restaurant/config', config); setSaved(true); setTimeout(() => setSaved(false), 3000); }
    catch (_) {}
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="mb-8"><h1 className="text-2xl font-black text-white">⚙️ Paramètres</h1><p className="text-slate-400 text-sm mt-1">Configuration du restaurant</p></div>
      <form onSubmit={handleSave} className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-8 space-y-6">
        <div>
          <h2 className="text-white font-bold text-sm mb-4">🍽️ Informations</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2"><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Nom du restaurant</label><input value={config.nomRestaurant} onChange={e => setConfig({...config, nomRestaurant: e.target.value})} className={inputClass} /></div>
        ...
        {perm.canEdit && (
          <div className="flex items-center gap-4 pt-2">
            <button type="submit" disabled={loading} className="bg-red-500 hover:bg-red-400 disabled:opacity-40 text-white font-bold px-8 py-3 rounded-xl text-sm transition-all shadow-lg shadow-red-500/20">{loading ? '⏳...' : '💾 Enregistrer'}</button>
