...
export default function TablesPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/tables/${confirmDelete.id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">? Tables</h1><p className="text-slate-400 text-sm mt-1">{totalItems} table{totalItems !== 1 ? 's' : ''}</p></div>
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(t => (
                <tr key={t.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${t.statut === 'Libre' ? 'bg-emerald-500/20 text-emerald-400' : t.statut === 'Occupe' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>{t.statut}</span></td>
                  <td className="px-5 py-4 text-center"><div className="flex justify-center gap-2"><button onClick={() => { setEditItem(t); setFormOpen(true); }} className="text-pink-400 hover:text-pink-300 text-xs font-bold">✏️ Modifier</button><button onClick={() => setConfirmDelete(t)} className="text-red-400 hover:text-red-300 text-xs font-bold">🗑️ Supprimer</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-4 border-t border-slate-700/50 bg-slate-900/30">
              <span className="text-slate-400 text-xs">{totalItems} tables  Page {page}/{totalPages}</span>
      ...
      <TableForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} metier={prefix} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting}
        title="Supprimer la table" message={`Supprimer la table  ${confirmDelete?.numero}  ? Cette action est irrversible.`} />
    </div>
  );
}
