...
export default function ReservationsPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/reservations/${confirmDelete.id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Rservations</h1><p className="text-slate-400 text-sm mt-1">{totalItems} rservation{totalItems !== 1 ? 's' : ''}</p></div>
        {perm.canCreate && <button onClick={() => { setEditItem(null); setFormOpen(true); }} className="bg-red-500 hover:bg-red-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all shadow-lg shadow-red-500/20">+ Nouvelle Rservation</button>}
      </div>
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? <tr><td colSpan={6} className="text-center py-16 text-slate-500">Aucune rservation</td></tr>
              : paginated.map(r => (
                <tr key={r.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${r.statut === 'Confirme' ? 'bg-emerald-500/20 text-emerald-400' : r.statut === 'En attente' ? 'bg-yellow-500/20 text-yellow-400' : r.statut === 'Termine' ? 'bg-blue-500/20 text-blue-400' : 'bg-red-500/20 text-red-400'}`}>{r.statut}</span></td>
                  <td className="px-5 py-4 text-center"><div className="flex justify-center gap-2">{perm.canEdit && <button onClick={() => { setEditItem(r); setFormOpen(true); }} className="text-pink-400 hover:text-pink-300 text-xs font-bold">✏️ Modifier</button>}<button onClick={() => setConfirmDelete(r)} className="text-red-400 hover:text-red-300 text-xs font-bold">🗑️ Supprimer</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-4 border-t border-slate-700/50 bg-slate-900/30">
              <span className="text-slate-400 text-xs">{filtres.length} rservation{filtres.length > 1 ? 's' : ''}  Page {page}/{totalPages}</span>
      ...
      <ReservationRestaurantForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} metier={prefix} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting}
        title="Supprimer la rservation" message={`Supprimer la rservation de  ${confirmDelete?.clientNom}  ? Cette action est irrversible.`} />
    </div>
  );
}
