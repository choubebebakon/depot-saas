...
;
  ...
  const cards = [
    { label: 'Plats principaux', value: stats?.plats || 0, icon: '🍖', color: '#dc2626' },
    { label: 'Entrées', value: stats?.entrees || 0, icon: '🥗', color: '#f59e0b' },
    { label: 'Desserts', value: stats?.desserts || 0, icon: '🍰', color: '#8b5cf6' },
    { label: 'Boissons', value: stats?.boissons || 0, icon: '🥤', color: '#3b82f6' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          ...
          <div className="flex items-end gap-2 h-48">
            {[10, 14, 18, 12, 20, 24, 22, 26, 18, 22, 28, 30].map((val, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[9px] text-slate-400 font-semibold">{val}M</span>
                <div className="w-full rounded-lg bg-red-500/70 transition-all" style={{ height: `${val * 5}px` }} />
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">🍽️ Répartition des ventes</h2>
          <div className="space-y-4">
            {[
              { label: 'Plats principaux', pct: 55, color: '#dc2626' },
              { label: 'Entrées', pct: 18, color: '#f59e0b' },
              { label: 'Desserts', pct: 12, color: '#8b5cf6' },
              { label: 'Boissons', pct: 15, color: '#3b82f6' },
            ].map((item, i) => (
              <div key={i}>
