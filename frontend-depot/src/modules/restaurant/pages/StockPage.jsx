  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Stock Cuisine</h1><p className="text-slate-400 text-sm mt-1">{totalItems} article{totalItems !== 1 ? 's' : ''}</p></div>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        ...
        <select value={filtreCategorie} onChange={e => { setFiltreCategorie(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-2.5 text-sm outline-none"><option value="">Toutes catgories</option>{CATEGORIES_STOCK.map(c => <option key={c} value={c}>{c}</option>)}</select>
      </div>
      {loading ? (
      ...
      <ArticleBaseForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={refetch} edit={editItem} metier={prefix} title={editItem ? '?? Modifier l\'article' : '?? Nouvel article'} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting}
        title="Supprimer l'article" message={`Supprimer  ${confirmDelete?.nom || confirmDelete?.designation}  ? Cette action est irrversible.`} />
    </div>
  );
}
