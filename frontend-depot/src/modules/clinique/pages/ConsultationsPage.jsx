...
export default function ConsultationsPage() {
  ...
  const { data: consultationsData = [], loading, refetch } = useData(`/${prefix}/consultations`, { enabled: true });
  const consultations = Array.isArray(consultationsData?.data) ? consultationsData.data : (Array.isArray(consultationsData) ? consultationsData : []);

  // Pagination centralisÃ©e â FIX: totalPages non dÃ©fini
  const filtres = (consultations || []).filter(item =>
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/consultations/${confirmDelete.id}`);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Consultations</h1><p className="text-slate-400 text-sm mt-1">{totalItems} consultation{totalItems !== 1 ? 's' : ''}</p></div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(c => (
                <tr key={c.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center">{perm.canEdit && <button onClick={() => { setEditItem(c); setFormOpen(true); }} className="text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-700 text-sm transition-colors">✏️ Modifier</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {formOpen && <ConsultationForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Consultation modifiée ?' : 'Consultation cre ?'); refetch(); }} edit={editItem} />}
      {confirmDelete && (
        <ConfirmModal
          ...
          message={`tes-vous sr de vouloir suppriméer la consultation de "${confirmDelete.patientNom}" ?`}
        />
      )}
    </div>
  );
