...
;
  ...
  return (
    <div className="p-6 space-y-6">
      <div><h1 className="text-2xl font-black text-white">📈 Rapports</h1><p className="text-slate-400 text-sm mt-1">Indicateurs cliniques</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">📊 Consultations mensuelles</h2>
          <div className="flex items-end gap-2 h-48">
            {[18, 22, 25, 20, 28, 30, 32, 28, 24, 26, 30, 28].map((val, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[9px] text-slate-400 font-semibold">{val}</span>
                <div className="w-full rounded-lg bg-sky-500/70 transition-all" style={{ height: `${val * 5}px` }} />
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">📅 RDV par jour (semaine)</h2>
          <div className="flex items-end gap-2 h-48">
            {[8, 12, 10, 14, 16, 6, 2].map((val, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[9px] text-slate-400 font-semibold">{val}</span>
                <div className="w-full rounded-lg bg-emerald-500/70 transition-all" style={{ height: `${val * 10}px` }} />
                <span className="text-[9px] text-slate-500">{['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'][i]}</span>
              </div>
            ))}
          </div>
        </div>
