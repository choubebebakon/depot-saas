  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Patients</h1><p className="text-slate-400 text-sm mt-1">{totalItems} patient{totalItems !== 1 ? 's' : ''}</p></div>
      ...
      <div className="mb-6"><input type="text" placeholder="🔍 Nom, tlphone..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-sky-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-72" /></div>
      ...
      {formOpen && <PatientForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Patient modifié ?' : 'Patient cr ?'); refetch(); }} edit={editItem} />}
      {confirmDelete && (
        <ConfirmModal
      ...
      <DossierMedicalForm isOpen={dossierOpen} onClose={() => setDossierOpen(false)} onSuccess={() => { success('Dossier mdical mis  jour ?'); refetch(); }} metier={prefix} patient={dossierPatient} />
    </div>
  );
}
