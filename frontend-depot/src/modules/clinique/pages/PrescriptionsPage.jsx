...
export default function PrescriptionsPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/prescriptions/${confirmDelete.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        await api.patch(`/${prefix}/prescriptions/${editItem.id}`, form);
      } else {
        await api.post(`/${prefix}/prescriptions`, form);
      }
      setFormOpen(false);
      setEditItem(null);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Prescriptions</h1><p className="text-slate-400 text-sm mt-1">{totalItems} prescription{totalItems !== 1 ? 's' : ''}</p></div>
      ...
      <div className="mb-6"><input type="text" placeholder="🔍 Patient, mdecin..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-sky-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-72" /></div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(p => (
                <tr key={p.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center">{perm.canEdit && <button onClick={() => { setEditItem(p); setFormOpen(true); }} className="text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-700 text-sm transition-colors">✏️ Modifier</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '?? Modifier prescription' : '?? Nouvelle prescription'} loading={formLoading} submitLabel={editItem ? 'Modifier' : 'Crer'}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FormField label="Mdecin" name="medecinNom" value={formData.medecinNom} onChange={set('medecinNom')} placeholder="Nom du mdecin" />
          <FormField label="Date" name="date" type="date" value={formData.date} onChange={set('date')} />
          <div className="col-span-2"><FormField label="Mdicaments" name="medicaments" type="textarea" value={formData.medicaments} onChange={set('medicaments')} required rows={3} placeholder="Ex: Amoxicilline 500mg, Paractamol..." /></div>
          <div className="col-span-2"><FormField label="Posologie" name="posologie" type="textarea" value={formData.posologie} onChange={set('posologie')} rows={2} placeholder="Ex: 1 comprim matin et soir" /></div>
          <FormField label="Dure" name="duree" type="number" value={formData.duree} onChange={set('duree')} min={1} unit="jours" />
        </div>
      </FormModal>
      {confirmDelete && (
        <ConfirmModal
          ...
          message={`tes-vous sr de vouloir suppriméer cette prescription ?`}
        />
      )}
    </div>
  );
