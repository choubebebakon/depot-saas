...
export default function MedecinsPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/medecins/${confirmDelete.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        await api.patch(`/${prefix}/medecins/${editItem.id}`, form);
      } else {
        await api.post(`/${prefix}/medecins`, form);
      }
      setFormOpen(false);
      setEditItem(null);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">? Mdecins</h1><p className="text-slate-400 text-sm mt-1">{totalItems} mdecin{totalItems !== 1 ? 's' : ''}</p></div>
        {perm.canCreate && <button onClick={() => { setEditItem(null); setFormOpen(true); }} className="bg-sky-500 hover:bg-sky-400 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-all shadow-lg shadow-sky-500/20">+ Nouveau Mdecin</button>}
      </div>
      <div className="flex gap-3 mb-6 flex-wrap">
        ...
        <select value={filtreSpecialite} onChange={e => { setFiltreSpecialite(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-2.5 text-sm outline-none"><option value="">Toutes spcialits</option>{SPECIALITES.map(s => <option key={s} value={s}>{s}</option>)}</select>
      </div>
      ...
      : (
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full">
            <thead className="bg-slate-900/50"><tr className="text-slate-500 text-xs font-bold uppercase tracking-widest"><th className="text-left px-5 py-4">Mdecin</th><th className="text-left px-5 py-4">Spcialit</th><th className="text-right px-5 py-4">Honoraires</th><th className="text-center px-5 py-4">Tlphone</th><th className="text-center px-5 py-4">Actions</th></tr></thead>
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? <tr><td colSpan={5} className="text-center py-16 text-slate-500">Aucun mdecin trouvé</td></tr>
              : paginated.map(m => (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-4 border-t border-slate-700/50 bg-slate-900/30">
              <span className="text-slate-400 text-xs">{filtres.length} mdecin{filtres.length > 1 ? 's' : ''}  Page {page}/{totalPages}</span>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '?? Modifier mdecin' : '????? Nouveau mdecin'} loading={formLoading} submitLabel={editItem ? 'Modifier' : 'Crer'}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="col-span-2"><FormField label="Nom" name="nom" value={formData.nom} onChange={set('nom')} required placeholder="Nom du mdecin" /></div>
          <FormField label="Tlphone" name="telephone" type="tel" value={formData.telephone} onChange={set('telephone')} placeholder="6XXXXXXXX" />
          <FormField label="Email" name="email" type="email" value={formData.email} onChange={set('email')} placeholder="medecin@exemple.com" />
          <FormField label="Spcialit" name="specialite" type="select" value={formData.specialite} onChange={set('specialite')} options={SPECIALITES} />
          <FormField label="N Ordre" name="numeroOrdre" value={formData.numeroOrdre} onChange={set('numeroOrdre')} placeholder="Numro d'ordre" />
