...
const DashboardClinique = lazy(() => import('./pages/DashboardClinique'));
const MedecinsPage        = lazy(() => import('./pages/MedecinsPage'));
const PatientsPage        = lazy(() => import('./pages/PatientsPage'));
const ConsultationsPage   = lazy(() => import('./pages/ConsultationsPage'));
const PrescriptionsPage   = lazy(() => import('./pages/PrescriptionsPage'));
const RendezVousPage      = lazy(() => import('./pages/RendezVousPage'));
const MedicamentsPage     = lazy(() => import('./pages/MedicamentsPage'));
const CaissePage          = lazy(() => import('./pages/CaissePage'));
const RapportsPage        = lazy(() => import('./pages/RapportsPage'));
const ParametresPage      = lazy(() => import('./pages/ParametresPage'));

function Loader() {
  return <div className="flex items-center justify-center py-32"><div className="w-10 h-10 border-4 border-sky-400 border-t-transparent rounded-full animate-spin" /></div>;
}

function MetierGuard({ children }) {
  const { metier } = useAuth();
  const stored = localStorage.getItem('gestock_metier');
  if (metier && metier !== 'CLINIQUE') return <Navigate to="/onboarding/metier" replace />;
  if (!metier && stored && stored !== 'CLINIQUE') return <Navigate to="/onboarding/metier" replace />;
  return children;
}

function CliniqueLayout() {
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 px-6 flex items-center justify-between z-40">
          ...
          <div className="flex-1 flex justify-end items-center gap-6">
            ...
            <div className="flex items-center gap-3 bg-slate-800/50 px-3 py-1.5 rounded-2xl border border-slate-700 shadow-inner">
              <div className="w-7 h-7 bg-sky-500 rounded-full flex items-center justify-center text-white font-bold text-[10px]">{user?.email?.[0]?.toUpperCase()}</div>
              <div className="hidden md:flex flex-col">
                <span className="text-white text-xs font-semibold leading-none">{user?.email}</span>
                <span className="text-sky-400 text-[9px] font-black uppercase tracking-widest mt-0.5">Clinique</span>
              </div>
            </div>
          </div>
        </header>
        <main className="flex-1 overflow-y-auto relative custom-scrollbar">
          <div className="max-w-[1600px] mx-auto min-h-full"><Suspense fallback={<Loader />}><Outlet /></Suspense></div>
        </main>
      </div>
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />
          <div className="absolute left-0 top-0 h-full"><DynamicSidebar user={user} tenant={user?.tenant} onLogout={handleLogout} /></div>
        </div>
      )}
...
export default function CliniqueRoutes() {
  return (
    <MetierGuard>
      <Routes>
        <Route element={<CliniqueLayout />}>
          <Route path="dashboard"      element={<DashboardClinique />} />
