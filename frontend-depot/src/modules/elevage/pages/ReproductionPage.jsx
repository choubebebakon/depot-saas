...
export default function ReproductionPage() {
  ...
  const perm = usePermission(PERMISSIONS, 'reproduction');

  const { data: accouplementsData = [], loading, refetch } = useData(`/${prefix}/reproduction`, { enabled: true });
  const accouplements = Array.isArray(accouplementsData?.data) ? accouplementsData.data : (Array.isArray(accouplementsData) ? accouplementsData : []);

  // Pagination centralisÃ©e â FIX: totalPages non dÃ©fini
  const filtres = (accouplements || []).filter(item =>
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/reproduction/${confirmDelete.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        await api.patch(`/${prefix}/reproduction/${editItem.id}`, form);
      } else {
        await api.post(`/${prefix}/reproduction`, form);
      }
      setFormOpen(false);
      setEditItem(null);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Reproduction</h1><p className="text-slate-400 text-sm mt-1">{totalItems} accouplement{totalItems !== 1 ? 's' : ''}</p></div>
      ...
      : (
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(a => (
                <tr key={a.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${a.statut === 'Naissance' ? 'bg-emerald-500/20 text-emerald-400' : a.statut === 'Confirm' ? 'bg-blue-500/20 text-blue-400' : a.statut === 'échec' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>{a.statut}</span></td>
          ...
          {totalPages > 1 && (<div className="flex items-center justify-between px-5 py-4 border-t border-slate-700/50 bg-slate-900/30"><span className="text-slate-400 text-xs">{totalItems} accouplement{totalItems > 1 ? 's' : ''}  Page {page}/{totalPages}</span><div className="flex gap-1">
      ...
      {formOpen && <ModalRepro onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Accouplement modifié ?' : 'Accouplement cr ?'); refetch(); }} edit={editItem} />}
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer l'accouplement" message={`Supprimer cet accouplement ? Cette action est irrversible.`} />
    </div>
  );
}


// Fallback industriel automatique pour éviter le crash de ModalRepro
function ModalRepro({ isOpen, onClose, onSuccess, edit, ...props }) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-lg p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-white">Formulaire de Gestion (ModalRepro)</h2>
