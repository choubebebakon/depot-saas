...
export default function TroupeauxPage() {
  ...
  const perm = usePermission(PERMISSIONS, 'troupeaux');

  const { data: animauxData = [], loading, refetch } = useData(`/${prefix}/troupeaux`, { enabled: true });
  const animaux = Array.isArray(animauxData?.data) ? animauxData.data : (Array.isArray(animauxData) ? animauxData : []);

  // Pagination centralisÃ©e â FIX: totalPages non dÃ©fini
  const filtres = (animaux || []).filter(item =>
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/troupeaux/${confirmDelete.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        await api.patch(`/${prefix}/troupeaux/${editItem.id}`, form);
      } else {
        await api.post(`/${prefix}/troupeaux`, form);
      }
      setFormOpen(false);
      setEditItem(null);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Troupeaux</h1><p className="text-slate-400 text-sm mt-1">{totalItems} animal</p></div>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        <input type="text" placeholder="🔍 Identifiant, espce..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-lime-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-60" />
        <select value={filtreEspece} onChange={e => { setFiltreEspece(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-2.5 text-sm outline-none"><option value="">Toutes espces</option>{ESPECES.map(e => <option key={e} value={e}>{e}</option>)}</select>
      </div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(a => (
                <tr key={a.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center text-slate-300 text-sm">{a.sexe === 'Mle' ? '?' : '?'}</td>
      ...
      {formOpen && <ModalAnimal onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Animal modifié ?' : 'Animal cr ?'); refetch(); }} edit={editItem} />}
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer l'animal" message={`Supprimer  ${confirmDelete?.identifiant}  ? Cette action est irrversible.`} />
    </div>
  );
}
