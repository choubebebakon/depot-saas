  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Ventes</h1><p className="text-slate-400 text-sm mt-1">{totalItems} vente{totalItems !== 1 ? 's' : ''}  Total {ventes.reduce((s, v) => s + (v.quantite || 0) * (v.prixUnitaire || 0), 0).toLocaleString('fr-FR')} F</p></div>
      ...
      {formOpen && <ModalVente onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Vente modifiée ?' : 'Vente cre ?'); refetch(); }} edit={editItem} />}
...
function ModalVente({ isOpen, onClose, onSuccess, edit, ...props }) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-lg p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-white">Sélection / Formulaire (ModalVente)</h2>
