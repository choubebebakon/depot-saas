...
export default function DepensesPage() {
  ...
  const [form, setForm] = useState({ libelle: '', categorie: 'FARINE', montant: '', dateDepense: new Date().toISOString().split('T')[0] });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-white tracking-tight">Dpenses</h1><p className="text-slate-400 text-sm">{depenses.length} dpense(s)</p></div>
        <button onClick={() => { setEditItem(null); setForm({ libelle: '', categorie: 'FARINE', montant: '', dateDepense: new Date().toISOString().split('T')[0] }); setFormOpen(true); }} className="bg-amber-600 hover:bg-amber-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm">+ Nouvelle</button></div>
      ...
      <FormModal isOpen={formOpen} onClose={() => { setFormOpen(false); setEditItem(null); }} onSubmit={handleSubmit} title={editItem ? '?? Modifier dpense' : '?? Nouvelle dpense'} submitLabel={editItem ? 'Enregistrer' : 'Crer'}>
        ...
        {['libelle','montant','dateDepense'].map(f => (<div key={f}><label className="text-slate-400 text-xs font-bold uppercase block mb-1.5">{f}</label><input type={f === 'montant' ? 'number' : f === 'dateDepense' ? 'date' : 'text'} value={form[f]} onChange={e => setForm({...form, [f]: e.target.value })} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white w-full text-sm" required={f === 'libelle' || f === 'montant'} /></div>))}
      </FormModal>
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer la dpense" message="Cette action est irrversible." />
    </div>
  );
}
