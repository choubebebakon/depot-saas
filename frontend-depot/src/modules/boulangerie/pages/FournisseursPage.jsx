...
export default function FournisseursPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-white tracking-tight">Fournisseurs</h1><p className="text-slate-400 text-sm">{fournisseurs.length} fournisseur(s)</p></div>
        <button onClick={() => { setEditItem(null); setFormOpen(true); }} className="bg-amber-600 hover:bg-amber-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm">+ Nouveau</button></div>
      ...
      <FournisseurForm isOpen={formOpen} onClose={() => { setFormOpen(false); setEditItem(null); }} onSuccess={refetch} edit={editItem} metier={prefix} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer le fournisseur" message="Cette action est irrversible." />
    </div>
  );
}
