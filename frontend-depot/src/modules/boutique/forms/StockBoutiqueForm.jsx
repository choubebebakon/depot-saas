...
export default function StockBoutiqueForm({ isOpen, onClose, onSuccess, edit }) {
  ...
  const { control, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm({
    resolver: zodResolver(articleSchema), defaultValues,
  });
  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      ...
      return edit ? boutiqueApi.updateArticle(edit.id, payload) : boutiqueApi.createArticle(payload);
    },
    onSuccess: () => {
  ...
  return (
    <FormModal isOpen={isOpen} onClose={onClose} onSubmit={handleSubmit(mutation.mutate)}
      title={edit ? 'Modifier' : 'Nouvel Article'} loading={mutation.isPending} size="lg"
      submitLabel={edit ? 'Modifier' : 'Créer'}>
      <Controller name="designation" control={control} render={({ field }) => (
        <FormField label="Désignation" name="designation" value={field.value}
          onChange={(e) => field.onChange(e.target.value)} required error={errors.designation?.message}
          placeholder="Nom de l'article" />
      )} />
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
        {[
          ['prixVente', 'Prix vente', false], ['prixAchat', 'Prix achat', false], ['prixGros', 'Prix de gros', true],
        ].map(([name, label, optional]) => (
          <Controller key={name} name={name} control={control} render={({ field }) => (
            <FormField label={label} name={name} type="number" value={field.value}
              onChange={(e) => field.onChange(e.target.value)} min="0" unit="FCFA"
              hint={optional ? 'Optionnel' : undefined} error={errors[name]?.message} />
          )} />
        ))}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
        <Controller name="unite" control={control} render={({ field }) => (
          <FormField label="Unité" name="unite" type="select" value={field.value}
            onChange={(e) => field.onChange(e.target.value)} options={[
      ...
      <div className="grid grid-cols-1 gap-4 mt-4">
        <Controller name="codeBarres" control={control} render={({ field }) => (
          <FormField label="Code-barres" name="codeBarres" value={field.value}
            onChange={(e) => field.onChange(e.target.value)} placeholder="Saisir le code-barres"
            hint="Saisie manuelle ou scan direct" error={errors.codeBarres?.message} />
        )} />
        <div className="p-4 bg-slate-900/40 rounded-xl border border-slate-700/40 isolation-auto"
          onClick={(e) => e.stopPropagation()}
          onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); e.stopPropagation(); } }}>
          <p className="text-slate-400 text-xs font-semibold mb-2 uppercase tracking-wider">Option alternative : Scanner / Douchette externe</p>
          <div className="contents"><BarcodeScanner onScan={(code) => setValue('codeBarres', code, { shouldValidate: true })}
            placeholder="Cliquez ici puis flashez l'article" mode="both" /></div>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
        <Controller name="seuilCritique" control={control} render={({ field }) => (
          <FormField label="Seuil critique" name="seuilCritique" type="number" value={field.value}
            onChange={(e) => field.onChange(e.target.value)} min="0" error={errors.seuilCritique?.message} />
        )} />
        <Controller name="datePeremption" control={control} render={({ field }) => (
          <FormField label="Date et heure de péremption" name="datePeremption" type="datetime-local"
            value={field.value || ''} onChange={(e) => field.onChange(e.target.value)}
            hint="Optionnel" error={errors.datePeremption?.message} />
        )} />
      </div>
      <div className="mt-4"><PhotoUpload label="Photo de l'article" name="photoUrl" value={watch('photoUrl')}
        onChange={(e) => setValue('photoUrl', e.target.value)} /></div>
    </FormModal>
  );
}
