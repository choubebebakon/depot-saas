...
export default function ClientsPage() {
  ...
  const { data: itemsData = [], loading, refetch } = useData(`/${prefix}/clients`, { enabled: true });
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        {perm.canCreate && <button onClick={() => { setEditItem(null); setFormOpen(true); }} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-cyan-600/20">+ Nouveau Client</button>}
      </div>
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full"><thead className="bg-slate-900/50"><tr className="text-slate-500 text-xs font-bold uppercase tracking-widest"><th className="text-left px-5 py-4">Nom</th><th className="text-left px-5 py-4">Email</th><th className="text-left px-5 py-4">Tlphone</th><th className="text-center px-5 py-4">Actions</th></tr></thead>
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? <tr><td colSpan={4} className="text-center py-16 text-slate-500">Aucun client</td></tr>
              : paginated.map(i => (
      ...
      {formOpen && <ClientBoutiqueForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Client modifié ?' : 'Client cr ?'); refetch(); }} edit={editItem} />}
      {confirmDelete && <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer le client" message={`tes-vous sr de vouloir suppriméer "${confirmDelete.nom}" ?`} />}
    </div>
  );
}
