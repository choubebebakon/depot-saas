      ...
      <form onSubmit={handleSubmit(onOpen)} className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="space-y-4">
          <div>
            ...
            />
            {errors.montant && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> {errors.montant.message}</p>}
          </div>
          <div>
            <input
        ...
        <div className="space-y-4">
          <div className="flex gap-3">
            <button type="button" onClick={() => setValue('typeMouvement', 'ENTREE')}
              className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${typeMouvement === 'ENTREE' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400'}`}><ArrowDownToLine className="w-4 h-4 mx-auto" /> Entrée</button>
            <button type="button" onClick={() => setValue('typeMouvement', 'SORTIE')}
              className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${typeMouvement === 'SORTIE' ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-400'}`}><ArrowUpFromLine className="w-4 h-4 mx-auto" /> Sortie</button>
          </div>
          <div>
            <input
              type="number"
          ...
          <div>
            <input
              ...
              className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-cyan-500"
            />
            {errors.motif && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> {errors.motif.message}</p>}
          </div>
        </div>
        <div className="flex gap-3 mt-6">
          <button type="button" onClick={onClose}
...
export default function CaissePage() {
  ...
  const { data: caisse, isLoading, error: queryError } = useQuery({
    queryKey: ['boutique-caisse-statut', currentDepotId],
    queryFn: async () => {
  ...
  const ouvrirMutation = useMutation({
    mutationFn: (data) => api.post('/boutique/caisse/ouvrir', { 
        montantInitial: parseInt(data.montant), 
        motif: data.motif, 
        depotId: currentDepotId 
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['boutique-caisse-statut'] });
  ...
  const movimientoMutation = useMutation({
    mutationFn: (data) => api.post('/boutique/caisse/mouvement', {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['boutique-caisse-statut'] });
      notif.success('Mouvement enregistré');
      setShowModal(null);
    },
  ...
  async function handleRapport() {
    ...
    try {
      const res = await api.get('/boutique/caisse/rapport-journalier', { params: { depotId: currentDepotId } });
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-700 pb-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{depotActif.nom} — {estOuverte ? '🟢 Ouverte' : '🔴 Fermée'}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {!estOuverte ? (
            <button onClick={() => setShowModal('ouvrir')} className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-sm">🔓 Ouvrir caisse</button>
          ) : (
            <>
              <button onClick={() => setShowModal('mouvement')} className="px-4 py-2.5 bg-slate-600 hover:bg-slate-500 text-white font-bold rounded-xl text-sm">➕ Mouvement</button>
              <button onClick={() => setConfirmFermer(true)} className="px-4 py-2.5 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl text-sm">🔒 Fermer caisse</button>
            </>
          )}
          <button onClick={handleRapport} disabled={fetchingRapport} className="px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded-xl text-sm">📊 Rapport</button>
        </div>
      </div>
      ...
      {estOuverte ? (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3">
            <VenteBoutiqueForm depotId={currentDepotId} onSuccess={() => queryClient.invalidateQueries({ queryKey: ['boutique-caisse-statut'] })} />
          </div>
          <div className="space-y-4">
            <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-5">
              <p className="text-xs text-slate-400 uppercase">Solde</p>
              <p className="text-2xl font-black text-white mt-1">{(caisse?.solde || 0).toLocaleString('fr-FR')} FCFA</p>
            </div>
            <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-5">
              <h2 className="text-sm font-bold text-white mb-2 uppercase">Mouvements</h2>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {mouvements.length === 0 && <p className="text-xs text-slate-500 py-2">Aucun mouvement</p>}
                {mouvements.map((m, i) => (
                  <div key={i} className="flex items-center justify-between text-xs bg-slate-700/20 p-2 rounded">
                    <span>{m.typeMouvement === 'ENTREE' ? '📥' : '📤'} {m.motif}</span>
                    <span className={m.typeMouvement === 'ENTREE' ? 'text-emerald-400' : 'text-red-400'}>
                      {m.typeMouvement === 'ENTREE' ? '+' : '-'}{(m.montant || 0).toLocaleString('fr-FR')}
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="text-center py-20 bg-slate-800/30 rounded-2xl border border-slate-700/50">
          ...
          <p className="text-slate-400 mt-2">Vous devez ouvrir la caisse pour effectuer des ventes.</p>
          <button onClick={() => setShowModal('ouvrir')} className="mt-6 px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-lg shadow-emerald-600/20">
            Ouvrir la caisse maintenant
          </button>
        </div>
      )}

      <OuvrirCaisseModal isOpen={showModal === 'ouvrir'} onClose={() => setShowModal(null)} onOpen={(data) => ouvrirMutation.mutate(data)} />
      <MouvementCaisseModal isOpen={showModal === 'mouvement'} onClose={() => setShowModal(null)} onSubmit={(data) => movimientoMutation.mutate(data)} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-slate-800 p-4 rounded-xl"><p className="text-xs text-slate-400">Solde</p><p className="text-lg font-bold text-white">{(rapportData.solde || 0).toLocaleString('fr-FR')} F</p></div>
              <div className="bg-slate-800 p-4 rounded-xl"><p className="text-xs text-slate-400">Statut</p><p className={`text-lg font-bold ${rapportData.statut === 'OUVERTE' ? 'text-emerald-400' : 'text-red-400'}`}>{rapportData.statut || 'N/A'}</p></div>
              <div className="bg-slate-800 p-4 rounded-xl"><p className="text-xs text-slate-400">Entrées</p><p className="text-lg font-bold text-emerald-400">+{(rapportData.entreesJour || 0).toLocaleString('fr-FR')} F</p></div>
              <div className="bg-slate-800 p-4 rounded-xl"><p className="text-xs text-slate-400">Sorties</p><p className="text-lg font-bold text-red-400">-{(rapportData.sortiesJour || 0).toLocaleString('fr-FR')} F</p></div>
            </div>
            <button onClick={() => setShowModal(null)} className="w-full px-4 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl">Fermer</button>
          </div>
        </div>
      )}
      ...
      <ConfirmModal isOpen={!!confirmFermer} onConfirm={() => fermerMutation.mutate()} onCancel={() => setConfirmFermer(null)} loading={fermerMutation.isPending}
        title="Fermer la caisse" message="Voulez-vous vraiment fermer la caisse ?" />
    </div>
  );
}
