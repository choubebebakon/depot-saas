...
export default function CaissePage() {
  ...
  // Ticket 80mm (imprimante thermique) imprimé automatiquement après chaque vente.
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-700 pb-4"><div><h1 className="text-2xl font-black text-white">🏧 Caisse POS</h1><p className="text-slate-400 text-sm mt-1">{depotActif.nom} — {estOuverte ? '🟢 Ouverte' : '🔴 Fermée'}</p></div><div className="flex flex-wrap gap-2">{!estOuverte ? (hasAction('caisse.ouvrir') && <button onClick={() => setShowModal('ouvrir')} className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-sm">🔓 Ouvrir caisse</button>) : <>{hasAction('caisse.mouvement') && <button onClick={() => setShowModal('mouvement')} className="px-4 py-2.5 bg-slate-600 hover:bg-slate-500 text-white font-bold rounded-xl text-sm">➕ Mouvement</button>}{hasAction('caisse.fermer') && <button onClick={() => setConfirmFermer(true)} className="px-4 py-2.5 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl text-sm">🔒 Fermer caisse</button>}</>}<button onClick={handleRapport} disabled={fetchingRapport} className="px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded-xl text-sm">📊 Rapport</button></div></div>
      {factureNode}
      {estOuverte ? <div className="grid grid-cols-1 lg:grid-cols-4 gap-6"><div className="lg:col-span-3"><VenteBoutiqueForm depotId={currentDepotId} onSuccess={(vente) => { queryClient.invalidateQueries({ queryKey: caisseQueryKey }); if (vente?.id) printTicket(vente.id); }} /></div><div className="space-y-4"><div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-5"><p className="text-xs text-slate-400 uppercase">Solde</p><p className="text-2xl font-black text-white mt-1">{(caisse?.solde || 0).toLocaleString('fr-FR')} FCFA</p></div><div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-5"><h2 className="text-sm font-bold text-white mb-2 uppercase">Mouvements</h2><div className="space-y-2 max-h-64 overflow-y-auto pr-1">{mouvements.length === 0 && <p className="text-xs text-slate-500 py-2">Aucun mouvement</p>}{mouvements.map((m, i) => <div key={i} className="flex items-center justify-between text-xs bg-slate-700/20 p-2 rounded"><span>{m.typeMouvement === 'ENTREE' ? '📥' : '📤'} {m.motif}</span><span className={m.typeMouvement === 'ENTREE' ? 'text-emerald-400' : 'text-red-400'}>{m.typeMouvement === 'ENTREE' ? '+' : '-'}{(m.montant || 0).toLocaleString('fr-FR')}</span></div>)}</div></div></div></div> : <div className="text-center py-20 bg-slate-800/30 rounded-2xl border border-slate-700/50"><p className="text-6xl mb-4">🔒</p><p className="text-xl font-bold text-white">La caisse est fermée</p><p className="text-slate-400 mt-2">Vous devez ouvrir la caisse pour effectuer des ventes.</p>{hasAction('caisse.ouvrir') && <button onClick={() => setShowModal('ouvrir')} className="mt-6 px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-lg shadow-emerald-600/20">Ouvrir la caisse maintenant</button>}</div>}
