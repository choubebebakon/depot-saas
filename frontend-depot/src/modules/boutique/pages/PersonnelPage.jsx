...
export default function PersonnelPage() {
  ...
  const { data: personnelData, isLoading } = useQuery({
    queryKey: ['boutique-personnel', search],
    queryFn: async () => {
      const res = await boutiqueApi.getPersonnel({ search });
      return res.data;
    },
  });

  const items = personnelData?.data || [];
  const totalItems = personnelData?.total || 0;

  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      return boutiqueApi.deletePersonnel(id);
    },
    onSuccess: () => {
      notif.success('Employé supprimé');
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{totalItems} employé{totalItems !== 1 ? 's' : ''}</p>
        </div>
        {perm.canCreate && (
          <button
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {items.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-16 text-slate-500">Aucun employé</td></tr>
              ) : items.map(i => (
