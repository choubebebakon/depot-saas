...
export default function DepensesPage() {
  ...
  const { data: itemsData = [], loading, refetch } = useData(`/${prefix}/depenses`, { enabled: true });
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4"><div className="text-right"><p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Total</p><p className="font-black text-xl text-red-400">{totalDepenses.toLocaleString('fr-FR')} F</p></div>{perm.canCreate && <button onClick={() => { setEditItem(null); setFormOpen(true); }} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-cyan-600/20">+ Nouvelle Dpense</button>}</div>
      </div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(i => (
                <tr key={i.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4"><span className="text-[10px] font-black uppercase text-cyan-400 bg-cyan-500/10 px-2 py-1 rounded-full">{i.categorie}</span></td>
      ...
      {formOpen && <DepenseBoutiqueForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Dpense modifiée ?' : 'Dpense cre ?'); refetch(); }} edit={editItem} />}
      {confirmDelete && <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer la dpense" message={`tes-vous sr de vouloir suppriméer "${confirmDelete.libelle}" ?`} />}
    </div>
  );
}
