...
/**
 * Sous-module FACTURE (Boutique) — même principe que le module Dépôt de boissons.
 * Chaque facture correspond à une vente de la boutique :
 *   N° Facture = vente.reference · Client · Date · Montant · Statut · Action IMPRIMER (A4).
 * Source : GET /boutique/ventes (permission `ventes:read`) — la route est gated
 * par `factures` (matrice boutique : CAISSIER canRead).
 */
...
export default function FacturesPage() {
  ...
  // Connexion au module Ventes : chaque vente de la boutique = une facture.
  const q = useQuery({
    queryKey: ['boutique-factures', 'ventes', depotId],
    queryFn: async () => {
      const res = await boutiqueApi.getVentes({ page: 1, limit: LIMIT, depotId });
  ...
  const filtered = useMemo(() => ventes.filter((v) => {
    ...
    return [v.reference, v.client?.nom || v.client, v.modePaiement, v.produit].filter(Boolean).some((x) => String(x).toLowerCase().includes(needle));
  ...
  const totals = useMemo(() => {
    ...
    return {
      count: ventes.length,
      total: valides.reduce((acc, v) => acc + Number(v.total ?? v.montant ?? 0), 0),
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="font-black text-xl text-cyan-400">{money(totals.total)}</p>
          </div>
      ...
      {q.isLoading ? (
        <div className="p-16 text-center"><div className="w-10 h-10 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin mx-auto" /></div>
      ) : q.isError ? (
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={v.id} className="hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="px-5 py-4 text-slate-300">{v.client?.nom || v.client || 'Passant'}</td>
                    <td className="px-5 py-4 text-slate-300 text-sm">{v.date || v.createdAt ? new Date(v.date || v.createdAt).toLocaleDateString('fr-FR') : '—'}</td>
                    <td className="px-5 py-4 text-right text-white font-mono font-bold">{money(v.total ?? v.montant)}</td>
                    ...
                    <td className="px-5 py-4 text-center">
                      {annulee ? (
                        ...
                        <button onClick={() => print(v.id)} disabled={printingId === v.id}
                          title="Imprimer le ticket de caisse 80mm (imprimante thermique)"
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-cyan-600/20 text-cyan-300 hover:bg-cyan-600/40 hover:text-cyan-200 disabled:opacity-50 transition-all">
                          <Printer size={13} /> {printingId === v.id ? 'Préparation…' : 'Imprimer ticket'}
                        </button>
                      )}
                    </td>
                  </tr>
