...
import FactureBoutiqueForm from '../forms/FactureBoutiqueForm';
...
export default function FacturesPage() {
  ...
  const { data: itemsData = [], loading, refetch } = useData(`/${prefix}/factures`, { enabled: true });
  const items = Array.isArray(itemsData?.data) ? itemsData.data : (Array.isArray(itemsData) ? itemsData : []);

  const totalFacture = items.reduce((acc, i) => acc + (i.total || i.montant || 0), 0);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">Factures</h1><p className="text-slate-400 text-sm mt-1">{totalItems} facture{totalItems !== 1 ? 's' : ''}</p></div>
        <div className="flex items-center gap-4"><div className="text-right"><p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Total</p><p className="font-black text-xl text-cyan-400">{totalFacture.toLocaleString('fr-FR')} F</p></div>{perm.canCreate && <button onClick={() => { setEditItem(null); setFormOpen(true); }} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-cyan-600/20">+ Nouvelle Facture</button>}</div>
      </div>
      <div className="mb-6"><input type="text" placeholder="🔍 N facture, client..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-cyan-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-72" /></div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              ...
              : paginated.map(i => (
                <tr key={i.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center">{i.statut === 'PAYEE' ? <span className="text-[10px] font-black uppercase bg-green-500/20 text-green-400 px-2 py-1 rounded-full">Paye</span> : i.statut === 'EN_ATTENTE' ? <span className="text-[10px] font-black uppercase bg-yellow-500/20 text-yellow-400 px-2 py-1 rounded-full">En attente</span> : <span className="text-[10px] font-black uppercase bg-red-500/20 text-red-400 px-2 py-1 rounded-full">Impaye</span>}</td>
      ...
      {formOpen && <FactureBoutiqueForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Facture modifiée ?' : 'Facture cre ?'); refetch(); }} edit={editItem} />}
      {confirmDelete && <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer la facture" message={`tes-vous sr de vouloir suppriméer la facture "${confirmDelete.numero || confirmDelete.id}" ?`} />}
    </div>
  );
}
