...
export default function ParametresPage() {
  ...
  const handleSubmit = async (e) => { e.preventDefault(); setLoading(true); try { await api.put('/boutique/parametres', form); showNotif('Paramètres enregistrés ✓'); } catch { showNotif('Erreur', 'error'); } finally { setLoading(false); } };
  return (
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Nom de la boutique</label><input value={form.nom} onChange={e => setForm({...form, nom: e.target.value})} className={inputClass} placeholder="Ma Boutique" /></div>
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Email</label><input type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} className={inputClass} placeholder="contact@boutique.com" /></div>
        ...
        <button type="submit" disabled={loading} className="w-full bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 text-white font-bold py-4 rounded-xl text-lg">{loading ? '⏳ Enregistrement...' : '💾 Enregistrer les paramètres'}</button>
      </form>
    </div>
  );
}
