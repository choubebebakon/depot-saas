...
export default function PromotionsPage() {
  ...
  const { data: itemsData = [], loading, refetch } = useData(`/${prefix}/promotions`, { enabled: true });
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">? Promotions</h1><p className="text-slate-400 text-sm mt-1">{totalItems} promotion{totalItems !== 1 ? 's' : ''}</p></div>
        {perm.canCreate && <button onClick={() => { setEditItem(null); setFormOpen(true); }} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-cyan-600/20">+ Nouvelle Promotion</button>}
      </div>
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full"><thead className="bg-slate-900/50"><tr className="text-slate-500 text-xs font-bold uppercase tracking-widest"><th className="text-left px-5 py-4">Libell</th><th className="text-left px-5 py-4">Type</th><th className="text-right px-5 py-4">Valeur</th><th className="text-left px-5 py-4">Du</th><th className="text-left px-5 py-4">Au</th><th className="text-center px-5 py-4">Statut</th><th className="text-center px-5 py-4">Actions</th></tr></thead>
            <tbody className="divide-y divide-slate-700/50">
              {paginated.length === 0 ? <tr><td colSpan={7} className="text-center py-16 text-slate-500">Aucune promotion</td></tr>
              : paginated.map(i => (
                <tr key={i.id} className="hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-5 py-4"><span className="text-[10px] font-black uppercase bg-cyan-500/10 text-cyan-400 px-2 py-1 rounded-full">{i.type}</span></td>
                  <td className="px-5 py-4 text-right text-white font-mono">{i.valeur}{i.type === 'REMISE' ? '%' : ' F'}</td>
                  <td className="px-5 py-4 text-slate-300 text-sm">{i.dateDebut ? new Date(i.dateDebut).toLocaleDateString('fr-FR') : ''}</td>
                  <td className="px-5 py-4 text-slate-300 text-sm">{i.dateFin ? new Date(i.dateFin).toLocaleDateString('fr-FR') : ''}</td>
                  <td className="px-5 py-4 text-center">{i.actif ? <span className="text-[10px] font-black uppercase bg-green-500/20 text-green-400 px-2 py-1 rounded-full">Actif</span> : <span className="text-[10px] font-black uppercase bg-slate-500/20 text-slate-400 px-2 py-1 rounded-full">Inactif</span>}</td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-4 border-t border-slate-700/50 bg-slate-900/30">
              <span className="text-slate-400 text-xs">{filtres.length} promotion{filtres.length > 1 ? 's' : ''}  Page {page}/{totalPages}</span>
      ...
      {formOpen && <PromotionBoutiqueForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Promotion modifiée ?' : 'Promotion cre ?'); refetch(); }} edit={editItem} />}
      {confirmDelete && <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer la promotion" message={`tes-vous sr de vouloir suppriméer "${confirmDelete.libelle}" ?`} />}
    </div>
  );
}
