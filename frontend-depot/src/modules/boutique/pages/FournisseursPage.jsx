...
import FournisseurBoutiqueForm from '../forms/FournisseurBoutiqueForm';
...
export default function FournisseursPage() {
  ...
  const { data: itemsData = [], loading, refetch } = useData(`/${prefix}/fournisseurs`, { enabled: true });
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        {perm.canCreate && <button onClick={() => { setEditItem(null); setFormOpen(true); }} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-cyan-600/20">+ Nouveau Fournisseur</button>}
      </div>
      <div className="mb-6"><input type="text" placeholder="🔍 Nom, email..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-cyan-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-72" /></div>
      ...
      {formOpen && <FournisseurBoutiqueForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Fournisseur modifié ?' : 'Fournisseur cr ?'); refetch(); }} edit={editItem} />}
      {confirmDelete && <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} title="Supprimer le fournisseur" message={`tes-vous sr de vouloir suppriméer "${confirmDelete.nom}" ?`} />}
    </div>
  );
}
