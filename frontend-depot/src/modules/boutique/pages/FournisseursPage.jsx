...
export default function FournisseursPage() {
  ...
  const { data: fournisseursData, isLoading } = useQuery({
    queryKey: ['boutique-fournisseurs', search],
    queryFn: async () => {
      const res = await boutiqueApi.getFournisseurs({ search });
      return res.data;
    },
  });

  const items = fournisseursData?.data || [];
  const totalItems = fournisseursData?.total || 0;

  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      return boutiqueApi.deleteFournisseur(id);
    },
    onSuccess: () => {
      notif.success('Fournisseur supprimé');
      setConfirmDelete(null);
      queryClient.invalidateQueries({ queryKey: ['boutique-fournisseurs'] });
    },
    onError: () => {
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {items.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-16 text-slate-500">Aucun fournisseur</td></tr>
              ) : items.map(i => (
