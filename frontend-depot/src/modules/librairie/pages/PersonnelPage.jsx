...
import PersonnelLibrairieForm from '../forms/PersonnelLibrairieForm';
...
export default function PersonnelPage() {
  ...
  const { data: dataData = [], loading, refetch } = useData(`/${prefix}/personnel`, { enabled: true });
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {perm.canCreate && <button onClick={openCreate} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-indigo-600/20">+ Nouvel Employ</button>}
      </div>
      ...
      <PersonnelLibrairieForm isOpen={formOpen} onClose={() => setFormOpen(false)} onSuccess={() => { success(editItem ? 'Employ modifié ?' : 'Employ cr ?'); refetch(); }} edit={editItem} />
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} message="Supprimer cet employ ?" loading={deleting} />
    </div>
  );
}
