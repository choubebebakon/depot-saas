...
export default function ChantiersPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/chantiers/${confirmDelete}`);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-white tracking-tight">??? Chantiers</h1><p className="text-slate-400 text-sm">{totalItems} chantier(s)</p></div>
      ...
      <ChantierForm isOpen={showModal} onClose={() => { setShowModal(false); setEdit(null); }} onSuccess={() => { refetch(); }} edit={edit} metier="ciment-btp" />
    </div>
  );
}
