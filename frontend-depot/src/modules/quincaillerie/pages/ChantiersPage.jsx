...
export default function ChantiersPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/${prefix}/chantiers/${confirmDelete.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        await api.patch(`/${prefix}/chantiers/${editItem.id}`, form);
      } else {
        await api.post(`/${prefix}/chantiers`, form);
      }
      setFormOpen(false);
      setEditItem(null);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-black text-white">? Chantiers</h1><p className="text-slate-400 text-sm mt-1">{totalItems} chantier{totalItems !== 1 ? 's' : ''}</p></div>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '?? Modifier Chantier' : '??? Nouveau Chantier'} loading={saving} submitLabel={editItem ? 'Modifier' : 'Crer'}>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Date dbut</label><input type="date" value={form.dateDebut} onChange={setF('dateDebut')} className={inputClass} /></div>
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Date fin prvue</label><input type="date" value={form.dateFin} onChange={setF('dateFin')} className={inputClass} /></div>
      ...
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer le chantier" message={`Supprimer  ${confirmDelete?.nom}  ? Cette action est irrversible.`} />
    </div>
  );
}
