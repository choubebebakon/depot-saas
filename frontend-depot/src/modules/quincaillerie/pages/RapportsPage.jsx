...
;
  ...
  const cards = [
    { label: 'Quincaillerie', value: stats?.quincaillerie || 0, icon: '🔩', color: '#b45309' },
    { label: 'Matériaux', value: stats?.materiaux || 0, icon: '🧱', color: '#3b82f6' },
    { label: 'Plomberie', value: stats?.plomberie || 0, icon: '🔧', color: '#06b6d4' },
    { label: 'Électricité', value: stats?.electricite || 0, icon: '⚡', color: '#f59e0b' },
    { label: 'Total clients', value: stats?.clients || 0, icon: '👥', color: '#10b981' },
    { label: "Chiffre d'affaires", value: `${(stats?.ca || 0).toLocaleString('fr-FR')} F`, icon: '💰', color: '#ec4899' },
  ];
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">📊 Tendances du CA mensuel</h2>
          <div className="flex items-end gap-2 h-48">
            {[10, 14, 12, 18, 22, 25, 28, 24, 20, 26, 30, 32].map((val, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[9px] text-slate-400 font-semibold">{val}M</span>
                <div className="w-full rounded-lg bg-amber-600/70 transition-all" style={{ height: `${val * 3}px` }} />
        ...
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
          <h2 className="text-white font-black text-sm mb-4">🛠 Répartition des ventes</h2>
          <div className="space-y-4">
            {[
              { label: 'Quincaillerie', pct: 40, color: '#b45309' },
              { label: 'Matériaux', pct: 30, color: '#3b82f6' },
              { label: 'Plomberie', pct: 15, color: '#06b6d4' },
              { label: 'Électricité', pct: 10, color: '#f59e0b' },
              { label: 'Autres', pct: 5, color: '#6b7280' },
            ].map((item, i) => (
              <div key={i}>
