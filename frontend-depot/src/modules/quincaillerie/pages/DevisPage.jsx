...
export default function DevisPage() {
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6"><input type="text" placeholder="🔍 Libell..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-slate-800 border border-slate-700 focus:border-amber-500 text-white rounded-xl px-4 py-2.5 text-sm outline-none w-60" /></div>
      ...
      <FormModal isOpen={formOpen} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} title={editItem ? '?? Modifier Devis' : '?? Nouveau Devis'} loading={saving} submitLabel={editItem ? 'Modifier' : 'Crer'}>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Validit (jours)</label><input type="number" value={form.validite} onChange={setF('validite')} className={inputClass} /></div>
      ...
      <ConfirmModal isOpen={!!confirmDelete} onConfirm={handleDelete} onCancel={() => setConfirmDelete(null)} loading={deleting} title="Supprimer le devis" message={`Supprimer  ${confirmDelete?.libelle}  ? Cette action est irrversible.`} />
    </div>
  );
}
