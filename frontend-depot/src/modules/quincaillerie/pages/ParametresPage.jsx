...
export default function ParametresPage() {
  ...
  const handleSave = async (e) => {
    ...
    try { await api.put('/quincaillerie/config', config); setSaved(true); setTimeout(() => setSaved(false), 3000); }
    catch (_) {}
    finally { setLoading(false); }
  };
  return (
    <div className="p-6 max-w-2xl">
      <div className="mb-8"><h1 className="text-2xl font-black text-white">⚙️ Paramètres</h1><p className="text-slate-400 text-sm mt-1">Configuration de la quincaillerie</p></div>
      <form onSubmit={handleSave} className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-8 space-y-6">
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2"><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Nom du magasin</label><input value={config.nomMagasin} onChange={e => setConfig({...config, nomMagasin: e.target.value})} className={inputClass} /></div>
        ...
        <div className="border-t border-slate-700/50 pt-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 block">Superficie (m²)</label><input type="number" value={config.superficie} onChange={e => setConfig({...config, superficie: e.target.value})} className={inputClass} /></div>
          </div>
        </div>
        {perm.canEdit && (
          <div className="flex items-center gap-4 pt-2">
            <button type="submit" disabled={loading} className="bg-amber-600 hover:bg-amber-500 disabled:opacity-40 text-white font-bold px-8 py-3 rounded-xl text-sm transition-all shadow-lg shadow-amber-600/20">{loading ? '⏳...' : '💾 Enregistrer'}</button>
