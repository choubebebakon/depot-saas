...
- [x] **7. Sécuriser `fermerCaisse` dans les 3 métiers.** Fichiers : `boutique.service.ts`, `supermarche.service.ts`, `depot-boissons.service.ts`, fonction `fermerCaisse` (identique dans les 3). Même correction : vérifier `count === 0` après le `updateMany` sur `sessionCaisse` et lever `BadRequestException('Aucune session de caisse ouverte à fermer.')`.
...
- [x] **10. Empêcher la création de compte inter-tenant.** Fichier : `backend-depot/src/auth/auth.service.ts` ou le module approprié de gestion des utilisateurs, méthode `register()` / création d'utilisateur. Retirer ou écraser le champ `tenantId` dans le DTO entrant avec le `req.user.tenantId` du créateur pour forcer la création dans le même tenant. Si c'est l'inscription principale (nouveau tenant), le backend génère déjà le `tenantId` ; il faut s'assurer qu'un utilisateur ne peut pas en injecter un manuellement. C'est la correction la plus prioritaire de tout le fichier.
