...
export function isQuotaExceeded(
  plan: PlanType,
  ressource: keyof Pick<PlanQuota, 'depots' | 'utilisateurs' | 'articles' | 'clients' | 'factures' | 'fournisseurs'>,
  count: number,
): boolean {
  const limit = PLAN_QUOTAS[plan][ressource] as number;
  return count >= limit;
}

// ─── Helper : message d'erreur lisible ───────────────────────────
export function getQuotaErrorMessage(plan: PlanType, ressource: string): string {
...
export function getQuotaLimit(plan: PlanType, ressource: keyof PlanQuota): number | boolean | string {
