...
export class TourneeScopeInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: import('@nestjs/common').CallHandler): Observable<unknown> {
    ...
    if (!user || (!isTourneeRoute(request) && !isLegacyTricycleRoute(request))) {
      return next.handle();
    }
    ...
    return new Observable((observer) => {
      ...
      void this.resolveDepot(user, request)
        .then(async (depotId) => {
          ...
          this.forceDepotScope(request, depotId);

          if (isTourneeRoute(request)) {
            await this.assertTourneeTarget(request, user.tenantId, depotId);
          }
          ...
          subscription = this.depotScope.run(
            {
              ...
              metier: (request as any).auditMetier ?? 'DEPOT_BOISSONS',
            },
  ...
  private async resolveDepot(user: AuthenticatedUser, request: Request): Promise<string | null> {
    ...
    const depot = await this.prisma.depot.findFirst({
      where: { id: requested, tenantId: user.tenantId, isArchived: false },
      select: { id: true },
    });
  ...
  private forceDepotScope(request: ScopedRequest, depotId: string): void {
    const body = request.body as Record<string, unknown> | undefined;
    if (body) body.depotId = depotId;

    const query = request.query as Record<string, unknown> | undefined;
    if (query) query.depotId = depotId;
  }
