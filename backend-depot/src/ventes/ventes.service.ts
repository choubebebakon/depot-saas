...
export class VentesService {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      for (const ligne of lignes) {
        ...
        if (!Number.isFinite(remise) || remise < 0) {
          throw new BadRequestException(
            'La remise doit être un montant positif ou nul.',
          );
        }
        ...
        if (ligne.conditionnementId) {
          ...
          prixBase =
            ligne.prix == null ? Number(article.prixVente) : Number(ligne.prix);
          deductions = composition.map((item: any) => {
            ...
            return {
              ...
              quantite: itemQty * quantite,
            };
          });
          for (const deduction of deductions) {
            const component = await tx.article.findFirst({
              where: { id: deduction.articleId, tenantId },
            });
            if (!component)
              throw new BadRequestException(
                'Article de composition introuvable.',
              );
          }
        } else if (ligne.prix != null) {
          throw new BadRequestException(
            'Le prix manuel est réservé aux conditionnements ou casiers mixtes.',
          );
        }
      ...
      const cash = Number(
        montantCash ?? (mode === ModePaiement.CASH ? montantTotal : 0),
      );
      const om = Number(
        montantOM ?? (mode === ModePaiement.ORANGE_MONEY ? montantTotal : 0),
      );
      const momo = Number(
        montantMoMo ?? (mode === ModePaiement.MTN_MOMO ? montantTotal : 0),
      );
      const credit = Number(
        montantCredit ?? (mode === ModePaiement.CREDIT ? montantTotal : 0),
      );
      ...
      if (mode === ModePaiement.CASH && cash <= 0) {
        throw new BadRequestException(
          'Le montant CASH est obligatoire pour une vente comptant.',
        );
      }
      ...
      if (clientId) {
        client = await tx.client.findFirst({
          where: {
            id: clientId,
            tenantId,
            OR: [{ depotId: selectedDepotId }, { depotId: null }],
          },
        });
        if (!client)
      ...
      if (tourneeId) {
        const tournee = await tx.tournee.findFirst({
          where: { id: tourneeId, tenantId, depotId: selectedDepotId },
        });
        if (!tournee)
      ...
      if (!reference)
        reference = `FAC-${new Date().getFullYear()}-${randomUUID()}`;
      ...
      for (const deduction of stockDeductions) {
        const updated = await tx.stock.updateMany({
          where: {
            ...
            depotId: selectedDepotId,
            quantite: { gte: deduction.quantite },
          },
          data: { quantite: { decrement: deduction.quantite } },
        });
        if (updated.count !== 1)
          throw new BadRequestException(
            `Stock insuffisant pour l'article ${deduction.articleId}. La vente n'a pas été enregistrée.`,
          );
      ...
      if (credit > 0 && clientId) {
        const existingDebt = await tx.detteClient.findFirst({
          where: {
            tenantId,
            depotId: selectedDepotId,
            clientId,
      ...
      if (retoursConsigne && Array.isArray(retoursConsigne) && clientId) {
        for (const retour of retoursConsigne) {
          ...
          if (
            !portefeuille ||
            !Number.isInteger(retourQty) ||
            retourQty <= 0 ||
            portefeuille.quantite < retourQty
          ) {
            throw new BadRequestException(
      ...
      const remiseTotale = lignes.reduce(
        (acc: number, l: any) => acc + Number(l.remise || 0),
        0,
      );
      if (remiseTotale > 0) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      for (const ligne of vente.lignes) {
        ...
        if (ligne.casierMixte && composition && Array.isArray(composition))
          ...
          const cond = await tx.conditionnement.findFirst({
            where: {
              id: ligne.conditionnementId,
              tenantId,
          ...
          if (!cond)
            throw new BadRequestException('Conditionnement introuvable.');
          stockDecs = [
            {
              articleId: ligne.articleId,
              quantite: ligne.quantite * cond.quantiteUnitaire,
            },
          ];
        } else
          stockDecs = [
          ...
          if (vente.tourneeId) {
            ...
            const updated = await tx.stock.updateMany({
              where: {
                ...
                quantite: { gte: dec.quantite },
              },
              data: { quantite: { decrement: dec.quantite } },
            });
            if (updated.count !== 1)
              throw new BadRequestException(
                `Stock insuffisant pour l'article ${dec.articleId}.`,
              );
  ...
  async annulerVente(
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (vente.statut === StatutVente.PAYE) {
        throw new BadRequestException(
          'Une vente déjà payée ne peut pas être annulée par cette opération. Utilisez le workflow de remboursement pour inverser les paiements.',
        );
      }
      const venteUpdated = await tx.vente.update({
        where: { id },
  ...
  async update(
    ...
    if (dto.statut !== undefined || dto.modePaiement !== undefined) {
      throw new BadRequestException(
        'Le statut et le mode de paiement d’une vente ne peuvent pas être modifiés via PUT. Utilisez les workflows dédiés.',
      );
    }
    if (
      dto.motifAnnulation !== undefined &&
      vente.statut !== StatutVente.ANNULE
    ) {
      throw new BadRequestException(
        'Le motif d’annulation ne peut être modifié que pour une vente déjà annulée.',
      );
    }
