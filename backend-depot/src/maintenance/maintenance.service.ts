...
export class MaintenanceService {
  ...
  async getStatsTricycle(tricycleId: string, tenantId: string) {
    ...
    const coutCarburantTotal = carburants.reduce(
      (acc, c) => acc + c.montantTotal,
      0,
    );
    ...
    const planifiees = maintenances.filter(
      (m) => m.statut === 'PLANIFIE',
    ).length;
    ...
    return {
      ...
      enRetard,
      consommationMoyenne:
        toursTotal > 0 ? (litresTotal / toursTotal).toFixed(2) : 0,
    };
  }
