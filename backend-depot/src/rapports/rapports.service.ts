...
export class RapportsService {
  ...
  private async resolveScope(
    ...
    if (!actor?.tenantId || !actor.role)
      throw new ForbiddenException('Contexte utilisateur invalide.');
    ...
    if (!requestedDepotId)
      return { tenantId: actor.tenantId, depotId: undefined };
  ...
  /** Restriction de périmètre COMMERCIAL (§7/§10 self-scope). */
  private static isSelfScopedCommercial(actor: ReportActor): boolean {
    return actor.role === RoleUser.COMMERCIAL;
  }

  async getTopProduitsParMarge(
    actor: ReportActor | undefined,
    requestedDepotId?: string,
    month?: string,
  ) {
    const { tenantId, depotId } = await this.resolveScope(
      actor,
      requestedDepotId,
    );
    ...
    for (const vente of ventes) {
      for (const ligne of vente.lignes) {
        ...
        current.tauxMarge =
          current.chiffreAffaires > 0
            ? Number(
                ((current.margeBrute / current.chiffreAffaires) * 100).toFixed(
                  2,
                ),
              )
            : 0;
  ...
  async getPerformanceCommerciaux(
    ...
    const commerciaux = await this.prisma.user.findMany({
      where: {
        ...
        // §7/§10 — un COMMERCIAL ne voit que sa propre performance.
        ...(actor.role === RoleUser.COMMERCIAL && actor.userId
          ? { id: actor.userId }
          : {}),
        ...(depotId ? { depotId } : {}),
      },
    ...
    const byCommercial = new Map(
      commerciaux
        // Défense en profondeur : un COMMERCIAL ne récupère que sa ligne.
        .filter(
          (user) =>
            actor.role !== RoleUser.COMMERCIAL || user.id === actor.userId,
        )
        .map((user) => [
        user.id,
        {
    ...
    for (const vente of ventes) {
      const commercialId =
        ...
        (vente.createur?.role === RoleUser.COMMERCIAL
          ? vente.createur.id
          : null);
      ...
      current.margeBrute += vente.lignes.reduce(
        (acc, ligne) =>
    ...
    for (const casse of casses) {
      ...
      if (commercialId && byCommercial.has(commercialId))
  ...
  async getPointMortMensuel(
    ...
    const coutVariable = ventes.reduce(
      (acc, vente) =>
        acc +
        vente.lignes.reduce(
          (sum, ligne) => sum + ligne.quantite * (ligne.article.prixAchat || 0),
          0,
        ),
      0,
    );
    ...
    return {
      ...
      progression:
        pointMortCA > 0
          ? Number(((chiffreAffaires / pointMortCA) * 100).toFixed(2))
          : 0,
      restePourPointMort: Number(
        Math.max(0, pointMortCA - chiffreAffaires).toFixed(2),
      ),
      surplusApresPointMort: Number(
        Math.max(0, chiffreAffaires - pointMortCA).toFixed(2),
      ),
