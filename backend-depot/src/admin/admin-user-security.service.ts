...
export class AdminUserSecurityService {
  ...
  private assertDifferentActor(actorUserId: string, targetUserId: string) {
    if (!actorUserId || actorUserId === targetUserId) {
      throw new BadRequestException(
        'Un SuperAdmin ne peut pas modifier son propre compte depuis cet espace.',
      );
    }
  }
  ...
  private normalizeReason(reason: string | undefined): string {
    ...
    if (value.length < 5)
      throw new BadRequestException(
        'Un motif de sécurité d’au moins 5 caractères est obligatoire.',
      );
    if (value.length > 500)
      throw new BadRequestException('Le motif de sécurité est trop long.');
    return value;
  }
  ...
  async toggleUserActive(
    ...
    try {
      ...
      return {
        success: true,
        user: result.updated,
        message: `Utilisateur ${result.updated.isActive ? 'activé' : 'désactivé'}`,
      };
    } catch (error) {
      if (
        ...
        throw new ConflictException(
          'Modification concurrente détectée. Veuillez réessayer.',
        );
      }
      throw error;
    }
    ...
    try {
      const result = await this.prisma.$transaction(
        async (tx) => {
          ...
          if (target.isSuperAdmin && role !== Role.PATRON)
            throw new BadRequestException(
              'Un SuperAdmin doit rester avec le rôle PATRON.',
            );
          if (
            target.role === Role.PATRON &&
            target.isSuperAdmin &&
            role !== Role.PATRON
          )
            throw new BadRequestException(
              'Retirez d’abord le statut SuperAdmin avant de changer ce rôle.',
            );
          const updated = await tx.user.update({
            where: { id: target.id },
            data: { role },
      ...
      return {
        success: true,
        user: result.updated,
        message: `Rôle mis à jour: ${result.updated.role}`,
      };
    } catch (error) {
      if (
        error instanceof Prisma.PrismaClientKnownRequestError &&
  ...
  async toggleSuperAdmin(
    ...
    try {
      const result = await this.prisma.$transaction(
        async (tx) => {
          ...
          if (!target.isSuperAdmin && !target.isActive)
            throw new BadRequestException(
              'Un compte inactif doit être activé avant de recevoir le statut SuperAdmin.',
            );
          if (target.isSuperAdmin && target.isActive) {
            const activeSuperAdmins = await tx.user.count({
              where: { isSuperAdmin: true, isActive: true },
            ...
            if (activeSuperAdmins <= 1)
              throw new ConflictException(
                'Impossible de retirer le statut du dernier SuperAdmin actif.',
              );
          }
          const updated = await tx.user.update({
            where: { id: target.id },
          ...
          const audit = await this.auditInTransaction(
            ...
            updated.isSuperAdmin
              ? AUDIT_ACTIONS.SUPERADMIN_GRANTED
              : AUDIT_ACTIONS.SUPERADMIN_REVOKED,
      ...
      return {
        success: true,
        user: result.updated,
        message: `Statut SuperAdmin ${result.updated.isSuperAdmin ? 'accordé' : 'retiré'}`,
      };
    } catch (error) {
      if (
        error instanceof Prisma.PrismaClientKnownRequestError &&
  ...
  async deleteUser(actorUserId: string, targetUserId: string, reason: string) {
    ...
    try {
      const result = await this.prisma.$transaction(
        async (tx) => {
          ...
          if (user.isSuperAdmin && user.isActive) {
            ...
            if (activeSuperAdmins <= 1)
              throw new ConflictException(
                'Impossible de supprimer le dernier SuperAdmin actif.',
              );
          }
          ...
          try {
            ...
            if (
              ...
              throw new ConflictException(
                'Cet utilisateur possède encore des données liées. Désactivez son compte plutôt que de le supprimer.',
              );
            }
            throw error;
          }
      ...
      if (
        ...
        throw new ConflictException(
          'Suppression concurrente détectée. Veuillez réessayer.',
        );
      throw error;
    }
  }
