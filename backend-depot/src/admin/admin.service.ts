...
export class AdminService {
  ...
  async toggleSuperAdmin(userId: string) {
    ...
    if (user.isSuperAdmin) {
      ...
      if (otherSuperAdmins === 0)
        throw new Error(
          'Impossible de retirer le statut super admin du dernier super admin',
        );
    }
    const updated = await this.prisma.user.update({
      where: { id: userId },
  ...
  async getUsageMetrics(period: string = '30d') {
    ...
    const [ventesCount, articlesCreated, usersActive, depotsActive] =
      await Promise.all([
        this.prisma.vente.count({ where: { date: { gte: startDate } } }),
        this.prisma.article.count({ where: { createdAt: { gte: startDate } } }),
        this.prisma.user.count({
          where: { isActive: true, updatedAt: { gte: startDate } },
        }),
        this.prisma.depot.count({
          where: { isArchived: false, updatedAt: { gte: startDate } },
        }),
      ]);
    return {
      period,
  ...
  async getRevenueAnalytics(period: string = '30d') {
    ...
    const payments = await this.prisma.payment.findMany({
      where: { status: PaymentStatus.SUCCESS, createdAt: { gte: startDate } },
  ...
  async getChurnAnalytics() {
    ...
    const [expiredLast30Days, expiredLast90Days, activeNow, active30DaysAgo] =
      await Promise.all([
        this.prisma.tenant.count({
          where: {
    ...
    const churnRate30d =
      active30DaysAgo > 0 ? (expiredLast30Days / active30DaysAgo) * 100 : 0;
    const churnRate90d =
      activeNow > 0 ? (expiredLast90Days / activeNow) * 100 : 0;
    return {
      churnRate30d,
      churnRate90d,
      expiredLast30Days,
  ...
  async getFeatureUsage() {
    ...
    const [
      ...
      this.prisma.vente.count({ where: { date: { gte: thirtyDaysAgo } } }),
      this.prisma.mouvementStock.count({
        where: { createdAt: { gte: thirtyDaysAgo } },
      }),
      this.prisma.sessionCaisse.count({
        where: { dateOuverture: { gte: thirtyDaysAgo } },
      }),
      this.prisma.commandeFournisseur.count({
        where: { dateCommande: { gte: thirtyDaysAgo } },
      }),
      ...
      features: [
        {
          name: 'Ventes',
          count: ventesCount,
          percentage: totalActions > 0 ? (ventesCount / totalActions) * 100 : 0,
        },
        {
          name: 'Mouvements Stock',
          count: stockMovements,
          percentage:
            totalActions > 0 ? (stockMovements / totalActions) * 100 : 0,
        },
        {
          name: 'Sessions Caisse',
          count: caisseSessions,
          percentage:
            totalActions > 0 ? (caisseSessions / totalActions) * 100 : 0,
        },
        {
          name: 'Commandes Fournisseur',
          count: commandesFournisseur,
          percentage:
            totalActions > 0 ? (commandesFournisseur / totalActions) * 100 : 0,
        },
        {
          name: 'Tournées',
          count: tournees,
          percentage: totalActions > 0 ? (tournees / totalActions) * 100 : 0,
        },
      ],
      totalActions,
    };
