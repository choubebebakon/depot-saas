...
export class CatalogueService {
  ...
  async updateArticle(id: string, tenantId: string, dto: UpdateArticleDto) {
    ...
    return this.prisma.article.update({
      ...
      data: {
        ...
        ...(dto.seuilCritique !== undefined && {
          seuilCritique: dto.seuilCritique,
        }),
        ...(dto.estConsigne !== undefined && { estConsigne: dto.estConsigne }),
        ...(dto.uniteParCasier !== undefined && {
          uniteParCasier: dto.uniteParCasier,
        }),
        ...(dto.uniteParPack !== undefined && {
          uniteParPack: dto.uniteParPack,
        }),
        ...(dto.uniteParPalette !== undefined && {
          uniteParPalette: dto.uniteParPalette,
        }),
  ...
  formaterAffichage(quantite: number, article: any): string {
    const { palettes, casiers, bouteilles } = this.convertirUnites(
      quantite,
      article,
    );
    const parts: string[] = [];
    if (palettes > 0)
      parts.push(`${palettes} palette${palettes > 1 ? 's' : ''}`);
    if (casiers > 0) parts.push(`${casiers} casier${casiers > 1 ? 's' : ''}`);
    if (bouteilles > 0)
      parts.push(`${bouteilles} bouteille${bouteilles > 1 ? 's' : ''}`);
