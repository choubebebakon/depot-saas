...
export class AuditService {
  ...
  private assertAuthoritativeScope(input: AuditInput): {
    tenantId: string;
    depotId: string | null;
  } {
    ...
    if (!scope.tenantId)
      throw new ForbiddenException(
        'Contexte tenant requis pour écrire un audit.',
      );
    if (input.tenantId !== scope.tenantId)
      throw new ForbiddenException(
        'Le tenant de l’audit ne correspond pas au contexte authentifié.',
      );

    const scopedDepotId = scope.depotId;
    if (scopedDepotId && input.depotId && input.depotId !== scopedDepotId) {
      throw new ForbiddenException(
        'Le dépôt de l’audit ne correspond pas au contexte authentifié.',
      );
    }
    if (!scopedDepotId && input.depotId) {
      throw new ForbiddenException(
        'Un dépôt ne peut pas être injecté hors du périmètre dépôt authentifié.',
      );
    }
    return {
      tenantId: scope.tenantId,
      depotId: input.depotId ?? scopedDepotId,
    };
  }
  ...
  private auditHashPayload(entry: any): Record<string, unknown> {
    return {
      ...
      valeurAvant: entry.valeurAvant ?? null,
      valeurApres: entry.valeurApres ?? null,
      metadataText: entry.metadataText ?? null,
      severite: entry.severite,
      resultat: entry.resultat ?? null,
      metier: entry.metier ?? null,
  ...
  private async persistEvent(
    ...
    input: AuditInput,
    authoritativeDepotId?: string | null,
  ) {
    const tenantId = input.tenantId.trim();
    if (!tenantId)
      throw new ForbiddenException('Tenant invalide pour l’audit.');
    ...
    const created = await tx.journalAudit.create({
      data: {
        ...
        valeurAvant: safeBefore ?? Prisma.JsonNull,
        valeurApres: safeAfter ?? Prisma.JsonNull,
  ...
  async logEvent(input: AuditInput) {
    ...
    const entry = await this.prisma.$transaction((tx) =>
      this.persistEvent(tx, input, authoritative.depotId),
    );
  ...
  private buildWhere(
    ...
    if (
      ...
      if (filters.montantMin !== undefined)
        montantFilter.gte = filters.montantMin;
      if (filters.montantMax !== undefined)
        montantFilter.lte = filters.montantMax;
    }
  ...
  async verifyIntegrity(tenantId: string) {
    ...
    `;
    const totalJournal = await this.prisma.journalAudit.count({
      where: { tenantId },
    });
    ...
    for (const row of rows) {
      if (row.previousHash !== previousHash)
        failures.push({
          journalAuditId: row.journalAuditId,
          reason: 'Chaînage précédent incohérent.',
        });
      const entry = {
        id: row.journalId,
        tenantId: row.journalTenantId,
        depotId: row.journalDepotId,
        ...
        valeurAvant: row.valeurAvant,
        valeurApres: row.valeurApres,
        ...
        montant: row.montant,
        createdAt: row.journalCreatedAt.toISOString(),
      };
      if (hashAuditEntry(entry, row.previousHash) !== row.hash)
        failures.push({
          journalAuditId: row.journalAuditId,
          reason: 'Empreinte SHA-256 invalide.',
        });
      previousHash = row.hash;
    }
    return {
      intact: failures.length === 0,
      protectedEntries: rows.length,
      unprotectedEntries: Math.max(totalJournal - rows.length, 0),
  ...
  async detectUnusualActivity(tenantId: string, hours = 24) {
    const safeHours = Math.min(
      Math.max(Number.isFinite(hours) ? Math.floor(hours) : 24, 1),
      168,
    );
    ...
    const byActorIp = new Map<string, Set<string>>();
    for (const row of rows) {
      const actor = row.actorUserId || row.actorEmail || 'unknown';
      byActor.set(actor, (byActor.get(actor) ?? 0) + 1);
      if (row.ipAddress) {
        if (!byActorIp.has(actor)) byActorIp.set(actor, new Set());
        byActorIp.get(actor)!.add(row.ipAddress);
      }
    }
    ...
    for (const [actor, count] of byActor) {
      if (count >= 100)
        alerts.push({
          ...
          actor,
          score: 90,
          detail: `${count} événements en ${safeHours}h.`,
        });
      ...
      if (ips >= 4)
        alerts.push({
          ...
          actor,
          score: 75,
          detail: `${ips} adresses IP distinctes sur la période.`,
        });
    }
    const failed = rows.filter(
      (row) => row.resultat === AuditResultat.ECHEC,
    ).length;
    const critical = rows.filter(
      (row) => row.severite === AuditSeverite.CRITIQUE,
    ).length;
    if (failed >= 10)
      alerts.push({
        type: 'FAILURES',
        score: 80,
        detail: `${failed} opérations en échec.`,
      });
    if (critical >= 5)
      alerts.push({
        type: 'CRITICAL_SPIKE',
        score: 85,
        detail: `${critical} événements critiques.`,
      });
    return {
  ...
  async getDashboard(tenantId: string, hours = 24) {
    ...
    const since = new Date(Date.now() - safeHours * 60 * 60 * 1000);
    const [total, successes, failures, critical, attention, info, actors] =
      await Promise.all([
    ...
    return {
      ...
      successes,
      failures,
      successRate: total ? Number(((successes / total) * 100).toFixed(2)) : 100,
      severity: { critical, attention, info },
      activeActors: actors.length,
      anomalyCount: anomalies.unusual.length,
      generatedAt: new Date().toISOString(),
    };
  }
  ...
  async exportJournalPDF(tenantId: string, filters?: AuditJournalFilters) {
    ...
    const drawHeader = () => {
      ...
      y -= 18;
      ['Date', 'Action', 'Sévérité', 'Utilisateur', 'Description'].forEach(
        (h, i) =>
          page.drawText(h, {
            x: [margin, margin + 90, margin + 190, margin + 260, margin + 400][
              i
            ],
            y,
            size: 9,
    ...
    for (const r of rows) {
      ...
      ].forEach((v, i) => page.drawText(v, { x: colX[i], y, size: 8, font }));
      y -= lineHeight;
    }
