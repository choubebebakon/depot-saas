...
export class DlcService {
  ...
  private normalizeDlc(dlc?: Date) {
    if (!dlc) return null;
    if (Number.isNaN(dlc.getTime())) {
      throw new BadRequestException('La date de péremption est invalide.');
    }
    return dlc;
  }
  ...
  async creerLot(data: {
    ...
    const depot = await this.prisma.depot.findFirst({
      where: { id: data.depotId, tenantId: data.tenantId },
      select: { id: true },
    });
    ...
    return this.prisma.lotStock.create({
      data: {
        ...
        dlc,
        numeroLot: data.numeroLot?.trim() || null,
      },
  ...
  async findLots(tenantId: string, depotId?: string) {
    ...
    const lots = await this.prisma.lotStock.findMany({
      where: { tenantId, depotId: selectedDepotId, quantite: { gt: 0 } },
      include: { article: { include: { famille: true, marque: true } }, depot: true },
      orderBy: [{ dlc: 'asc' }, { createdAt: 'asc' }],
    });
    ...
    return lots.map((lot) => ({
      ...
      joursRestants: lot.dlc ? Math.floor((lot.dlc.getTime() - now) / (1000 * 60 * 60 * 24)) : null,
    }));
  }
  ...
  async deduireLotFIFO(articleId: string, depotId: string, tenantId: string, quantiteADeduire: number) {
    ...
    if (restantADeduire > 0) {
      throw new BadRequestException(`Stock par lots insuffisant : ${restantADeduire} unité(s) non couvertes.`);
    }
  }
  ...
  async updateLot(id: string, tenantId: string, data: { dlc?: Date; numeroLot?: string }) {
    ...
    return this.prisma.lotStock.update({
      where: { id: lot.id },
      data: { dlc, numeroLot: data.numeroLot?.trim() || null },
      include: { article: true, depot: true },
    });
  }
}
