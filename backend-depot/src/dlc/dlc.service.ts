...
export class DlcService {
  ...
  private requireDepotId(depotId?: string) {
    if (!depotId) {
      throw new BadRequestException(
        'depotId est obligatoire pour isoler les lots du depot actif.',
      );
    }
  ...
  getStatutDLC(dlc: Date | null): StatutDLC {
    ...
    const diffJours = Math.floor(
      (dlc.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
    );
  ...
  async findLots(tenantId: string, depotId?: string) {
    ...
    return lots.map((lot) => ({
      ...
      joursRestants: lot.dlc
        ? Math.floor(
            (lot.dlc.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24),
          )
        : null,
    }));
  }
  ...
  async updateLot(
    id: string,
    tenantId: string,
    data: { dlc?: Date; numeroLot?: string },
  ) {
