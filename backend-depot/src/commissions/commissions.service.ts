...
export class CommissionsService {
  ...
  async calculerCommissions(dto: CalculerCommissionsDto) {
        ...
        try {
          ...
          ventes.forEach((vente) => {
            try {
                ...
                if (ligne.casierMixte && ligne.composition) {
                  ...
                  if (Array.isArray(comp)) {
                    comp.forEach((sub) => {
                      ...
                      let price = Number(sub.prix || 0);
                      if (isNaN(price) || price <= 0) {
                        const safeQtyTotale = comp.reduce(
                          (acc, c) => acc + (Number(c.quantite || 0) || 0),
                          0,
                        );
                        price =
                          (Number(ligne.total || 0) || 0) /
                          (Number(ligne.quantite || 1) || 1) /
                          (safeQtyTotale || 1);
                      }
                      const subLigneTotal = price * Number(sub.quantite || 0);
                      const totalPourCasier =
                        subLigneTotal * Number(ligne.quantite || 1);
              ...
              console.error(
                `[COMMISSION] Erreur de parsing sur la vente ${vente.id} (${vente.reference})`,
                err,
              );
            }
          });
          ...
          console.error(
            `[COMMISSION] Crash complet sur l'utilisateur ${user.id} (${user.email}):`,
            err,
          );
          return null;
        }
    ...
    return {
      ...
      totalCommissions: validCommissions.reduce(
        (acc, c) => acc + c.montantCommission,
        0,
      ),
    };
  }
  ...
  async payerCommission(commissionId: string, tenantId: string) {
    ...
    if (commission.estPayee)
      throw new BadRequestException('Commission déjà payée');

    const result = await this.prisma.commission.update({
      where: { id: commissionId },
      data: { estPayee: true, datePaiement: new Date() },
      include: {
        user: { select: { id: true, email: true, role: true, nom: true } },
      },
    });
