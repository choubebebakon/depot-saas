...
export class AuthService {
  ...
  async login(email: string, password: string, meta?: RequestMeta) {
    ...
    if (!user || !(await bcrypt.compare(password, user.password))) {
      if (user) {
        await this.auditService
          ...
          .catch((err) =>
            console.error('[Audit] Échec log ECHEC_CONNEXION:', err),
          );
      }
    ...
    return {
      ...
      user: {
        ...
        tenantId: user.tenantId,
        depotId: user.depotId ?? null,
        depotsAcces: (user.depotsAcces ?? []).map((a) => a.depotId),
  ...
  async validateRefreshTokenFromCookie(token: string) {
    try {
      ...
      const user = await this.prisma.user.findUnique({
        ...
        include: { refreshTokens: true, tenant: true },
      });
      if (
        !user ||
        !user.tenant.estActif ||
  ...
  /** Profil complet relu en base : le payload JWT ne porte ni avatar ni coordonnées. */
  async getFullUser(userId: string) {
    return this.prisma.user.findUnique({
      ...
      select: {
        ...
        tenantId: true,
        depotId: true,
        // Multi-établissements (§13) : périmètre élargi du profil, utilisé
        // par le sélecteur d'établissement du frontend.
        depotsAcces: { select: { depotId: true } },
        avatar: true,
        isActive: true,
        isSuperAdmin: true,
  ...
  async changePassword(
    ...
    if (!isCurrentPasswordValid) {
      await this.auditService
        ...
        .catch((err) =>
          console.error('[Audit] Échec log échec changement mdp:', err),
        );
    ...
    await this.auditService
      ...
      .catch((err) =>
        console.error('[Audit] Échec log CHANGEMENT_MOT_DE_PASSE:', err),
      );
