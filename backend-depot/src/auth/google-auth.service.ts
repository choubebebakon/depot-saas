...
export class GoogleAuthService {
  ...
  private async getGoogleKeys(forceRefresh = false): Promise<GoogleJwk[]> {
    ...
    const maxAgeSeconds = Math.min(
      Math.max(Number(maxAgeMatch?.[1] || 3600), 300),
      86400,
    );
    const body = (await response.json()) as { keys?: GoogleJwk[] };
    if (!Array.isArray(body.keys) || body.keys.length === 0)
      throw new UnauthorizedException('Clés Google indisponibles.');
    this.jwksCache = {
      keys: body.keys,
  ...
  private async verifyIdToken(idToken: string): Promise<GoogleClaims> {
    ...
    if (header.alg !== 'RS256' || !header.kid)
      throw new UnauthorizedException('Algorithme Google non accepté.');
    if (
      !claims ||
      claims.iss !== 'https://accounts.google.com' ||
      claims.aud !== this.getGoogleClientId()
    ) {
      throw new UnauthorizedException(
        'Identifiant Google non destiné à GesTock.',
      );
    }
    ...
    if (!gmail && !workspace) {
      throw new UnauthorizedException(
        'Ce compte Google ne permet pas de vérifier suffisamment la propriété de cette adresse.',
      );
    }
  ...
  private async issueSession(
    ...
    await this.auditService
      ...
      .catch((err) =>
        console.error('[Audit] Échec log CONNEXION_GOOGLE:', err),
      );
  ...
  async loginWithGoogle(
    ...
    if (!user)
      throw new UnauthorizedException(
        'Aucun compte GesTock associé à cette adresse. Créez d’abord votre compte GesTock.',
      );
    if (!user.isActive)
