...
export class CimentBtpService {
  ...
  async updateVehiculeDisponibilite(
    id: string,
    tenantId: string,
    disponible: boolean,
  ) {
    return this.prisma.vehiculeBTP.update({
      where: { id, tenantId },
      data: { disponible },
    });
  }
  ...
  async findAllLivraisons(tenantId: string, pagination: PaginationDto) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.livraisonBTP.findMany({
        where,
        skip,
        take: limit,
        include: { chantier: true, vehicule: true },
      }),
  ...
  async updateLivraisonStatut(
    id: string,
    tenantId: string,
    data: UpdateLivraisonStatutDto,
  ) {
    return this.prisma.livraisonBTP.update({
      where: { id, tenantId },
  ...
  async getLivraisonsByChantier(chantierId: string, tenantId: string) {
    return this.prisma.livraisonBTP.findMany({
  ...
  async getStats(tenantId: string) {
    const [chantiers, livraisons] = await Promise.all([
      ...
      this.prisma.livraisonBTP.count({
        where: {
          tenantId,
          datePlanifiee: { gte: new Date(new Date().setHours(0, 0, 0, 0)) },
        },
      }),
    ]);
    return {
      chantiersActifs: chantiers,
      livraisonsJour: livraisons,
      devisAttente: 0,
      stock: 0,
    };
  }
