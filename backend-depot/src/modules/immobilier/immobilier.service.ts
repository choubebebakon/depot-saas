...
export class ImmobilierService {
  ...
  async findAllBiens(tenantId: string, pagination: PaginationDto) {
    ...
    if (pagination.search) {
      where.OR = [
        ...
        { ville: { contains: pagination.search, mode: 'insensitive' } },
        { adresse: { contains: pagination.search, mode: 'insensitive' } },
      ];
    }
    const [data, total] = await Promise.all([
      this.prisma.bienImmobilier.findMany({
        where,
        skip,
        take: limit,
        include: {
  ...
  async createBien(tenantId: string, dto: CreateBienDto) {
    return this.prisma.bienImmobilier.create({
      data: { ...dto, tenantId, type: dto.type as any },
    });
  }
  ...
  async findAllContrats(tenantId: string, pagination: PaginationDto) {
    ...
    const where: any = { tenantId };
    const [data, total] = await Promise.all([
      this.prisma.contratLocation.findMany({
        where,
        skip,
        take: limit,
        include: {
          bien: true,
          locataire: true,
          paiements: { orderBy: { dateEcheance: 'desc' }, take: 3 },
        },
        orderBy: { createdAt: 'desc' },
      }),
  ...
  async updateContratStatut(id: string, statut: string) {
    return this.prisma.contratLocation.update({
      where: { id },
  ...
  async findAllPaiements(tenantId: string, pagination: PaginationDto) {
    ...
    const where: any = { tenantId };
    const [data, total] = await Promise.all([
      this.prisma.paiementLoyer.findMany({
        where,
        skip,
        take: limit,
  ...
  async findAllInterventions(tenantId: string, pagination: PaginationDto) {
    ...
    const where: any = { tenantId };
    const [data, total] = await Promise.all([
      this.prisma.interventionBien.findMany({
        where,
        skip,
        take: limit,
        include: { bien: true },
  ...
  async createIntervention(tenantId: string, dto: CreateInterventionDto) {
    return this.prisma.interventionBien.create({
      data: { ...dto, tenantId },
      include: { bien: true },
    });
  }
  ...
  async getStats(tenantId: string) {
    const totalBiens = await this.prisma.bienImmobilier.count({
      where: { tenantId },
    });
    const disponibles = await this.prisma.bienImmobilier.count({
      where: { tenantId, disponible: true },
    });
    const contratsActifs = await this.prisma.contratLocation.count({
      where: { tenantId, statut: 'ACTIF' },
    });
    const loyersMensuels = await this.prisma.contratLocation.aggregate({
      where: { tenantId, statut: 'ACTIF' },
      _sum: { loyer: true },
    });
    const enRetard = await this.prisma.paiementLoyer.count({
      where: { tenantId, statut: 'EN_RETARD' },
    });
    return {
      totalBiens,
      disponibles,
      loues: totalBiens - disponibles,
      contratsActifs,
      loyersMensuels: loyersMensuels._sum.loyer || 0,
      enRetard,
    };
  }
}
