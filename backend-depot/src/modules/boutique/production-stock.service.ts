...
export class ProductionBoutiqueVentesService extends VentesService {
  ...
  constructor(
    ...
    // §7 : transmis au VentesService parent (filtre « mes ventes »).
  ...
  override async createVente(tenantId: string, data: any, actor: AuditActor) {
    ...
    const articleIds = data.panier
      .map((item: any) => String(item.articleId))
    ...
    const vente = await this.withSerializableRetry(() =>
      this.db.$transaction(
        async (tx) => {
          ...
          if (updated.count !== 1)
            throw new ConflictException('La vente a été modifiée entre-temps.');
