...
export class DepensesProductionService {
  ...
  private normalizePage(value: unknown): number {
    ...
    if (!Number.isInteger(page) || page < 1 || page > MAX_PAGE)
      throw new BadRequestException(
        `page doit être un entier compris entre 1 et ${MAX_PAGE}.`,
      );
    return page;
  }
  ...
  private normalizeAmount(value: unknown): number {
    ...
    if (!Number.isFinite(amount) || amount <= 0 || amount > MAX_AMOUNT)
      throw new BadRequestException(
        `montant doit être compris entre 0,01 et ${MAX_AMOUNT} FCFA.`,
      );
  ...
  private async resolveDepot(
    ...
    const depotId =
      actor.role === 'PATRON'
        ? requestedDepotId || actor.depotId
        : actor.depotId;
    if (!depotId)
    ...
    if (
      ...
      throw new ConflictException(
        'Accès refusé : ce dépôt ne correspond pas à votre dépôt autorisé.',
      );
  ...
  async findOne(id: string, tenantId: string, actor: AuditActor) {
    ...
    if (actor.role !== 'PATRON' && row.depotId !== actor.depotId)
      throw new NotFoundException('Dépense non trouvée.');
    if (!row.depot || row.depot.isArchived)
      throw new NotFoundException('Dépense non trouvée.');
    return row;
  }
  ...
  async create(
    ...
    if (!Object.values(DepenseCategorieEnum).includes(categorie))
      throw new BadRequestException('Catégorie de dépense invalide.');
    const libelle = this.normalizeText(data?.libelle, 'libelle');
    const motif =
      this.normalizeText(data?.motif, 'motif') ||
      libelle ||
  ...
  async update(id: string, data: any, tenantId: string, actor: AuditActor) {
    ...
    if (data?.montant !== undefined)
      updateData.montant = this.normalizeAmount(data.montant);
    if (data?.categorie !== undefined) {
      if (!Object.values(DepenseCategorieEnum).includes(data.categorie))
        throw new BadRequestException('Catégorie de dépense invalide.');
      updateData.categorie = data.categorie;
    }
    const libelle = this.normalizeText(data?.libelle, 'libelle');
    const motif = this.normalizeText(data?.motif, 'motif');
    if (libelle !== undefined || motif !== undefined)
      updateData.motif = motif || libelle;
    if (!Object.keys(updateData).length)
      throw new BadRequestException('Aucune modification fournie.');
    const row = await this.prisma.depense.update({
      where: { id: current.id },
      data: updateData,
      include: { depot: true },
    });
  ...
  async delete(id: string, tenantId: string, actor: AuditActor) {
    const current = await this.findOne(id, tenantId, actor);
    const deleted = await this.prisma.depense.delete({
      where: { id: current.id },
    });
    await this.audit({
      tenantId,
