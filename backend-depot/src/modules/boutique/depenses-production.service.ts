...
export class DepensesProductionService {
  ...
  private normalizePage(value: unknown): number {
    ...
    if (!Number.isInteger(page) || page < 1 || page > MAX_PAGE) {
      throw new BadRequestException(`page doit être un entier compris entre 1 et ${MAX_PAGE}.`);
    }
    return page;
  }
  ...
  private normalizeLimit(value: unknown): number {
    ...
    if (!Number.isInteger(limit) || limit < 1 || limit > MAX_LIMIT) {
      throw new BadRequestException(`limit doit être un entier compris entre 1 et ${MAX_LIMIT}.`);
    }
    return limit;
  }
  ...
  private normalizeAmount(value: unknown): number {
    ...
    if (!Number.isFinite(amount) || amount <= 0 || amount > MAX_AMOUNT) {
      throw new BadRequestException(`montant doit être compris entre 0,01 et ${MAX_AMOUNT} FCFA.`);
    }
  ...
  private async resolveDepot(tenantId: string, requestedDepotId: string | undefined, actor: AuditActor) {
    ...
    if (actor.role !== 'PATRON' && requestedDepotId && requestedDepotId !== actor.depotId) {
      throw new ConflictException('Accès refusé : ce dépôt ne correspond pas à votre dépôt autorisé.');
    }
  ...
  private audit(payload: Parameters<AuditService['logEvent']>[0]) {
    return this.auditService.logEvent(payload).catch((error) => {
      console.error('[Audit] Échec journalisation dépense:', error);
    });
  }
  ...
  async findAll(tenantId: string, params: any, actor: AuditActor) {
    ...
    if (params?.categorie !== undefined) {
      if (!Object.values(DepenseCategorieEnum).includes(params.categorie)) {
  ...
  async findOne(id: string, tenantId: string, actor: AuditActor) {
    const depot = await this.resolveDepot(tenantId, undefined, actor);
    const row = await this.prisma.depense.findFirst({ where: { id, tenantId, depotId: depot.id }, include: { depot: true } });
  ...
  async create(data: any, tenantId: string, actor: AuditActor, requestedDepotId?: string) {
    ...
    const row = await this.prisma.depense.create({
      data: { tenantId, depotId: depot.id, categorie, montant, motif },
      include: { depot: true },
    });
    await this.audit({
      tenantId,
      ...
      targetType: 'Depense',
      targetId: row.id,
      description: `Dépense enregistrée : ${motif} (${montant} FCFA)`,
      valeurApres: { montant, categorie, motif },
      montant: -montant,
      motif,
      ipAddress: actor.ip,
  ...
  async update(id: string, data: any, tenantId: string, actor: AuditActor) {
    ...
    await this.audit({
      ...
      actorEmail: actor.email,
      actorRole: actor.role,
      action: AUDIT_ACTIONS.DEPENSE_MODIFIEE,
      severite: AuditSeverite.ATTENTION,
      targetType: 'Depense',
      targetId: row.id,
      description: `Dépense ${row.id} modifiée`,
      valeurAvant: { montant: current.montant, categorie: current.categorie, motif: current.motif },
      valeurApres: { montant: row.montant, categorie: row.categorie, motif: row.motif },
      ipAddress: actor.ip,
      userAgent: actor.userAgent,
    });
    return row;
  ...
  async delete(id: string, tenantId: string, actor: AuditActor) {
    ...
    await this.audit({
      ...
      actorEmail: actor.email,
      actorRole: actor.role,
      action: AUDIT_ACTIONS.DEPENSE_SUPPRIMEE,
      severite: AuditSeverite.CRITIQUE,
      targetType: 'Depense',
      targetId: current.id,
      description: `Dépense ${current.id} supprimée`,
      valeurAvant: { montant: current.montant, categorie: current.categorie, motif: current.motif },
