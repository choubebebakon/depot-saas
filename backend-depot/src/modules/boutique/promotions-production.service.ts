...
export class PromotionsProductionService {
  ...
  private text(value: unknown, field: string): string {
    ...
    if (v.length > MAX_NAME)
      throw new BadRequestException(
        `${field} ne peut pas dépasser ${MAX_NAME} caractères.`,
      );
    return v;
  }
  ...
  private async assertPromotion(tenantId: string, depotId: string, id: string) {
    const promotion = await this.prisma.promotion.findFirst({
      where: { id, tenantId },
      include: {
        article: { select: { id: true, designation: true, prixVente: true } },
      },
    });
  ...
  private validateBusiness(
    ...
    if (type === PromotionType.MONTANT_FIXE && valeur > prixVente) {
      throw new BadRequestException(
        'La remise fixe ne peut pas dépasser le prix de vente.',
      );
    }
    if (
      type === PromotionType.PRIX_FIXE &&
      (prixPromo <= 0 || prixPromo > prixVente)
    ) {
      throw new BadRequestException(
        'Le prix promotionnel doit être supérieur à 0 et inférieur ou égal au prix de vente.',
      );
    }
  }
  ...
  private async assertNoOverlap(
    ...
    if (overlap)
      throw new ConflictException(
        `Une promotion active chevauche déjà cette période pour cet article : ${overlap.nom}.`,
      );
  }
  ...
  async create(
    ...
    const prixPromo = this.amount(
      data?.prixPromo ??
        (type === PromotionType.PRIX_FIXE ? data?.valeur : article.prixVente),
      'prixPromo',
      true,
    );
    ...
    await this.audit({
      ...
      action: AUDIT_ACTIONS.PROMOTION_CREEE,
  ...
  async update(
    ...
    const articleId = data?.articleId
      ? this.text(data.articleId, 'articleId')
      : current.articleId;
    const article = await this.assertArticle(tenantId, depotId, articleId);
    const type = data?.type !== undefined ? this.type(data.type) : current.type;
    const valeur =
      data?.valeur !== undefined
        ? this.amount(data.valeur, 'valeur', true)
        : current.valeur;
    const prixPromo =
      data?.prixPromo !== undefined
        ? this.amount(data.prixPromo, 'prixPromo', true)
        : current.prixPromo;
    const dateDebut =
      data?.dateDebut !== undefined
        ? this.date(data.dateDebut, 'dateDebut')
        : current.dateDebut;
    const dateFin =
      data?.dateFin !== undefined
        ? this.date(data.dateFin, 'dateFin')
        : current.dateFin;
    if (dateFin <= dateDebut)
      throw new BadRequestException(
    ...
    this.validateBusiness(type, valeur, prixPromo, article.prixVente);
    if (current.actif || data?.actif === true)
      await this.assertNoOverlap(tenantId, articleId, dateDebut, dateFin, id);
    ...
    if (data?.nom !== undefined || data?.libelle !== undefined)
      updateData.nom = this.text(data.nom ?? data.libelle, 'nom');
    ...
    await this.audit({
      ...
      actorEmail: actor.email,
      actorRole: actor.role,
      action: AUDIT_ACTIONS.PROMOTION_MODIFIEE,
  ...
  async delete(
    ...
    await this.audit({
      ...
      actorEmail: actor.email,
      actorRole: actor.role,
      action: AUDIT_ACTIONS.PROMOTION_SUPPRIMEE,
