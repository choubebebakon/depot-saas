...
export class PromotionsProductionController {
  ...
  private depot(req: any): string {
    return req.depotScope?.depotId || req.headers['x-depot-id'];
  }

  @Get()
  @RequirePermission('promotions', 'read')
  findAll(@Req() req: any) {
    return this.service.findAll(this.tenant(req), this.depot(req));
  }

  @Get(':id')
  @RequirePermission('promotions', 'read')
  findOne(@Req() req: any, @Param('id') id: string) {
    return this.service.findOne(id, this.tenant(req), this.depot(req));
  }
