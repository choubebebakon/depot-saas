  ...
  async update(tenantId: string, depotId: string, id: string, data: any) {
    ...
    if (!data?.tricycleId || !data?.commercialId || !data?.date) {
      throw new BadRequestException(
        'tricycleId, commercialId et date sont requis.',
      );
    }
    ...
    if (!tournee)
      throw new NotFoundException('Tournée introuvable dans ce dépôt.');
    if (tournee.statut !== ('PLANIFIEE' as any)) {
      throw new ConflictException(
        'Une tournée déjà démarrée ou clôturée ne peut plus être modifiée.',
      );
    }
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (!commercial)
          throw new BadRequestException(
            'Commercial introuvable dans ce dépôt.',
          );
