...
export class TourneeWorkflowService {
  ...
  private assertPositiveInt(value: unknown, label: string) {
    if (!Number.isInteger(value) || Number(value) <= 0) {
      throw new BadRequestException(`${label} doit être un entier supérieur à 0.`);
    }
  }

  private assertNonNegativeInt(value: unknown, label: string) {
    if (!Number.isInteger(value) || Number(value) < 0) {
      throw new BadRequestException(`${label} doit être un entier supérieur ou égal à 0.`);
    }
  }
  ...
  async list(tenantId: string, depotId: string) {
    return this.prisma.$queryRaw<any[]>`
      SELECT w.*,
        json_build_object('id', t.id, 'nom', t.nom) AS tricycle,
    ...
    const rows = await this.prisma.$queryRaw<any[]>`
      SELECT w.*,
        ...
        COALESCE((SELECT json_agg(json_build_object(
          ...
          'prixUnitaireFacture', l."prixUnitaireFacture",
          'quantiteRetourPleins', l."quantiteRetourPleins",
          'quantiteRetourVides', l."quantiteRetourVides",
          'quantiteVendueTheorique', l."quantiteVendueTheorique",
      ...
      JOIN "User" u ON u.id = w."commercialId"
      JOIN "Depot" d ON d.id = w."depotId"
      WHERE w.id = ${id} AND w."tenantId" = ${tenantId} AND w."depotId" = ${depotId}
      LIMIT 1
    `;
  ...
  async create(tenantId: string, depotId: string, data: any) {
    if (!data?.tricycleId || !data?.commercialId) {
      throw new BadRequestException('Tricycle et commercial sont obligatoires.');
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const id = randomUUID();
      await tx.$executeRaw`
        INSERT INTO "TourneeWorkflow" ("id","reference","statut","tenantId","depotId","tricycleId","commercialId","tourneeId","datePlanifiee")
        VALUES (${id},${reference},${PLANIFIEE},${tenantId},${depotId},${data.tricycleId},${data.commercialId},${tournee.id},${date})
      `;
  ...
  async addLine(tenantId: string, depotId: string, id: string, data: any) {
    ...
    await this.prisma.$executeRaw`
      INSERT INTO "TourneeWorkflowLine" ("id","workflowId","articleId","quantiteChargee","prixUnitaireFacture")
      VALUES (${lineId},${id},${article.id},${data.quantiteChargee},${price})
    `;
    return this.get(tenantId, depotId, id);
  }
  ...
  async removeLine(tenantId: string, depotId: string, id: string, lineId: string) {
    const current = await this.get(tenantId, depotId, id);
    if (current.statut !== PLANIFIEE) throw new ConflictException('Le chargement est verrouillé après le départ.');
    await this.prisma.$executeRaw`DELETE FROM "TourneeWorkflowLine" WHERE id=${lineId} AND "workflowId"=${id}`;
    return this.get(tenantId, depotId, id);
  }
  ...
  async depart(tenantId: string, depotId: string, id: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const line of lines) {
        ...
        totalQty += Number(line.quantiteChargee);
        totalValue += Number(line.quantiteChargee) * Number(line.prixUnitaireFacture);
      }
      await tx.$executeRaw`UPDATE "TourneeWorkflow" SET "statut"=${EN_COURS}, "dateDepart"=CURRENT_TIMESTAMP, "totalQuantiteChargee"=${totalQty}, "totalValeurChargee"=${totalValue}, "updatedAt"=CURRENT_TIMESTAMP WHERE id=${id}`;
      return this.get(tenantId, depotId, id);
  ...
  async reconcile(tenantId: string, depotId: string, id: string, data: any) {
    ...
    for (const line of data.lignes) {
      this.assertNonNegativeInt(line.quantiteRetourPleins, 'Retour pleins');
      this.assertNonNegativeInt(line.quantiteRetourVides, 'Retour vides');
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const dbLines = await tx.$queryRaw<any[]>`SELECT * FROM "TourneeWorkflowLine" WHERE "workflowId"=${id}`;
      ...
      for (const input of data.lignes) {
        ...
        if (Number(input.quantiteRetourPleins) + 0 > Number(line.quantiteChargee)) throw new BadRequestException('Le retour plein ne peut pas dépasser la quantité chargée.');
        const vendue = Number(line.quantiteChargee) - Number(input.quantiteRetourPleins);
        const ca = vendue * Number(line.prixUnitaireFacture);
        caTheorique += ca;
        await tx.$executeRaw`UPDATE "TourneeWorkflowLine" SET "quantiteRetourPleins"=${input.quantiteRetourPleins}, "quantiteRetourVides"=${input.quantiteRetourVides}, "quantiteVendueTheorique"=${vendue}, "caTheorique"=${ca} WHERE id=${line.id}`;
      }
      ...
      await tx.$executeRaw`UPDATE "TourneeWorkflow" SET "cashReel"=${cash}, "orangeMoneyReel"=${om}, "mtnMomoReel"=${momo}, "montantEncaisseReel"=${total}, "caTheorique"=${caTheorique}, "ecartCaisse"=${ecart}, "updatedAt"=CURRENT_TIMESTAMP WHERE id=${id}`;
      return this.get(tenantId, depotId, id);
    });
  }
  ...
  async close(tenantId: string, depotId: string, id: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const line of lines) {
        ...
        if (retourPleins > 0) {
          const existing = await tx.stock.findFirst({ where: { articleId: line.articleId, depotId }, select: { id: true } });
          if (existing) await tx.stock.update({ where: { id: existing.id }, data: { quantite: { increment: retourPleins } } });
        ...
        await tx.$executeRaw`UPDATE "TourneeWorkflowLine" SET "quantiteRetourPleins"="quantiteRetourPleins", "quantiteRetourVides"="quantiteRetourVides" WHERE id=${line.id}`;
      }
      ...
      if (ecart < 0) {
        ...
        await tx.$executeRaw`INSERT INTO "DetteCommerciale" ("id","reference","montant","commercialId","workflowId","tenantId","depotId") VALUES (${debtId},${reference},${Math.abs(ecart)},${workflow.commercialId},${id},${tenantId},${depotId})`;
      }
      await tx.$executeRaw`UPDATE "TourneeWorkflow" SET "statut"=${CLOTUREE}, "dateCloture"=CURRENT_TIMESTAMP, "immutable"=TRUE, "updatedAt"=CURRENT_TIMESTAMP WHERE id=${id} AND "statut"=${EN_COURS}`;
  ...
  async stock(tenantId: string, depotId: string, articleId: string) {
    const article = await this.prisma.article.findFirst({ where: { id: articleId, tenantId }, select: { id: true, designation: true, prixVente: true, format: true } });
    if (!article) throw new NotFoundException('Article introuvable.');
