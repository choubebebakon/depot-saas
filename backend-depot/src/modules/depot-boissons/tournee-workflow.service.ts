...
export class TourneeWorkflowService {
  ...
  private async serializable<T>(work: (tx: Prisma.TransactionClient) => Promise<T>): Promise<T> {
    let attempt = 0;
    while (attempt < SERIALIZABLE_RETRIES) {
      try {
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2034' && attempt < SERIALIZABLE_RETRIES - 1) {
          attempt += 1;
          continue;
        }
        throw error;
  ...
  async get(tenantId: string, depotId: string, id: string) {
    const rows = (await this.prisma.$queryRaw`
      SELECT w.*, json_build_object('id', t.id, 'nom', t.nom) AS tricycle,
        ...
        ) ORDER BY a.designation) FROM "TourneeWorkflowLine" l
        JOIN "Article" a ON a.id=l."articleId"
        WHERE l."workflowId"=w.id), '[]'::json) AS lignes,
        ...
        COALESCE((SELECT json_agg(json_build_object('articleId', cs."articleId", 'quantite', cs.quantite))
         FROM "DepotConsigneStock" cs
         WHERE cs."tenantId"=w."tenantId" AND cs."depotId"=w."depotId"), '[]'::json) AS stockConsignes
      FROM "TourneeWorkflow" w
      JOIN "Tricycle" t ON t.id=w."tricycleId"
      JOIN "User" u ON u.id=w."commercialId"
      JOIN "Depot" d ON d.id=w."depotId"
  ...
  async create(tenantId: string, depotId: string, data: { tricycleId: string; commercialId: string; date?: string }) {
    ...
    return this.serializable(async (tx) => {
      ...
      const tournee = await tx.tournee.create({
        data: { reference, statut: 'OUVERTE', dateOuverture: date, depotId, tricycleId: data.tricycleId, commercialId: data.commercialId, tenantId },
        select: { id: true },
      });
      const id = randomUUID();
  ...
  async depart(tenantId: string, depotId: string, id: string) {
    return this.serializable(async (tx) => {
      ...
      const changed = await tx.$executeRaw`UPDATE "TourneeWorkflow" SET "statut"=${EN_COURS}, "dateDepart"=CURRENT_TIMESTAMP, "totalQuantiteChargee"=${totalQty}, "totalValeurChargee"=${totalValue}, "updatedAt"=CURRENT_TIMESTAMP WHERE id=${id} AND "tenantId"=${tenantId} AND "depotId"=${depotId} AND "statut"=${PLANIFIEE}`;
  ...
  async reconcile(tenantId: string, depotId: string, id: string, data: { lignes: Array<{ lineId: string; quantiteRetourPleins: number; quantiteRetourVides: number }>; cashReel: number; orangeMoneyReel: number; mtnMomoReel: number }) {
    return this.serializable(async (tx) => {
      ...
      await tx.$executeRaw`UPDATE "TourneeWorkflow" SET "cashReel"=${cash}, "orangeMoneyReel"=${om}, "mtnMomoReel"=${momo}, "montantEncaisseReel"=${total}, "caTheorique"=${caTheorique}, "ecartCaisse"=${ecart}, "updatedAt"=CURRENT_TIMESTAMP WHERE id=${id} AND "tenantId"=${tenantId} AND "depotId"=${depotId} AND "statut"=${EN_COURS}`;
