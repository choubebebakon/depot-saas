...
import { getIdempotencyKey, stockOperationId } from './stock-idempotency';
...
export class ProductionDepotStockService extends SecureDepotBoissonsVenteService {
  ...
  private async assertTransferScope(
    ...
    if (!article)
      throw new NotFoundException('Article introuvable ou non autorisé.');
    if (!sourceDepot)
      throw new NotFoundException('Dépôt source introuvable ou non autorisé.');
    if (!destinationDepot)
      throw new NotFoundException(
        'Dépôt destination introuvable ou non autorisé.',
      );
  }
  ...
  override async entreStock(tenantId: string, data: any, actor: AuditActor) {
    ...
    if (!Number.isSafeInteger(quantite) || quantite <= 0) {
      throw new BadRequestException(
        'La quantité doit être un entier sûr supérieur à 0.',
      );
    }
    await this.assertArticleAndDepot(tenantId, articleId, depotId);
    const motif =
      String(data?.motif ?? 'Entrée manuelle').trim() || 'Entrée manuelle';
    ...
    const mouvement = await this.withSerializableRetry(() =>
      this.db.$transaction(
        async (tx) => {
          if (idempotencyKey) {
            const existing = await tx.mouvementStock.findUnique({
              where: { id: mouvementId },
            });
            if (existing) {
              this.assertSameEntryOrThrow(existing, {
                tenantId,
                articleId,
                depotId,
                quantite,
  ...
  override async sortieStock(tenantId: string, data: any, actor: AuditActor) {
    ...
    await this.assertArticleAndDepot(tenantId, articleId, depotId);
    const motif =
      String(data?.motif ?? 'Sortie manuelle').trim() || 'Sortie manuelle';
    const idempotencyKey = getIdempotencyKey(data);
    const mouvementId = idempotencyKey
      ? stockOperationId(tenantId, idempotencyKey)
      : randomUUID();
  ...
  override async transfertStock(
    ...
    const articleId = String(data?.articleId ?? '').trim();
    const sourceDepotId = String(
      data?.sourceDepotId ?? data?.depotId ?? '',
    ).trim();
    ...
    await this.assertTransferScope(
      tenantId,
      articleId,
      sourceDepotId,
      destDepotId,
          ...
          async (tx) => {
            if (idempotencyKey) {
              const existing = await tx.transfertStock.findUnique({
                where: { reference },
                include: {
                  lignes: { select: { articleId: true, quantite: true } },
                },
              });
              if (existing) {
                ...
                const currentDestination = await tx.stock.findUnique({
                  where: {
                    articleId_depotId: { articleId, depotId: destDepotId },
                  },
                  select: { quantite: true },
                });
                return {
                  transfert: existing,
                  destinationStock: currentDestination?.quantite ?? 0,
                };
              }
            }
            ...
            await tx.mouvementStock.createMany({
              data: [
                ...
                },
                {
                  ...
                  tenantId,
                  motif:
                    motif || `Transfert ${reference} depuis ${sourceDepotId}`,
                },
              ],
            });
