...
/**
 * Production stock boundary for the beverage-depot métier.
 *
 * Stock and its ledger movement are committed in the same transaction.
 * The caller can still use the historical DepotBoissonsService contract,
 * while this class removes the most dangerous non-atomic stock mutations.
 */
@Injectable()
export class ProductionDepotStockService extends SecureDepotBoissonsVenteService {
  ...
  private async assertArticleAndDepot(
    tenantId: string,
    articleId: string,
    depotId: string,
  ) {
    ...
    if (!article) {
      throw new NotFoundException('Article introuvable ou non autorisé.');
    }
    if (!depot) {
  ...
  override async entreStock(
    ...
    if (!articleId || !depotId) {
      throw new BadRequestException('articleId et depotId sont requis.');
    }
  ...
  override async transfertStock(
    ...
    if (!articleId || !sourceDepotId || !destDepotId) {
      throw new BadRequestException(
        'articleId, sourceDepotId et depotDestination sont requis.',
      );
    }
    ...
    const transfert = await this.withSerializableRetry(() =>
      this.db.$transaction(
        async (tx) => {
          ...
          const transfertRow = await tx.transfertStock.create({
            data: {
              reference,
              statut: 'TERMINE',
              sourceDepotId,
            ...
            data: [
              {
                ...
                depotId: sourceDepotId,
                tenantId,
                motif: motif || `Transfert ${reference} vers ${destDepotId}`,
              },
              {
                ...
                depotId: destDepotId,
                tenantId,
                motif: motif || `Transfert ${reference} depuis ${sourceDepotId}`,
              },
            ],
          });

          return { transfert: transfertRow, destinationStock: destination.quantite };
        },
        {
          isolationLevel: Prisma.TransactionIsolationLevel.Serializable,
          maxWait: 5000,
    ...
    await this.audit.logEvent({
      ...
      targetId: transfert.transfert.id,
      reference: transfert.transfert.reference,
      description: `Transfert ${transfert.transfert.reference}: ${quantite} unité(s) ${sourceDepotId} → ${destDepotId}`,
      valeurApres: {
        articleId,
        quantite,
        sourceDepotId,
        destDepotId,
        destinationStock: transfert.destinationStock,
      },
