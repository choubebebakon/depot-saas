...
export class TourneeWorkflowOptionsController {
  ...
  async tricycles(@Req() req: any) {
    ...
    return this.prisma.tricycle.findMany({
      where: { tenantId, depotId, estLibre: true },
      orderBy: { nom: 'asc' },
  ...
  async commerciaux(@Req() req: any) {
    ...
    return this.prisma.user.findMany({
      where: { tenantId, depotId, role: 'COMMERCIAL', isActive: true },
      orderBy: { nom: 'asc' },
  ...
  async articles(@Req() req: any) {
    ...
    const articles = await this.prisma.article.findMany({
      ...
      select: { id: true, designation: true, format: true, prixVente: true },
    });
    ...
    const byArticle = new Map(
      stocks.map((stock) => [stock.articleId, stock.quantite]),
    );
    return articles.map((article) => ({
      ...article,
      quantiteDisponible: byArticle.get(article.id) ?? 0,
    }));
  }
}
