...
import { AddTourneeWorkflowLineDto, CreateTourneeWorkflowDto, ReconcileTourneeDto, UpdateTourneeWorkflowDto } from './dto/tournee-workflow.dto';
...
export class TourneeWorkflowController {
  ...
  async bonSortie(@Req() req: any, @Param('id') id: string, @Res() res: Response) {
    ...
    y -= 18;
    page.drawText('Article', { x: 45, y, size: 9, font: bold }); page.drawText('Qté', { x: 280, y, size: 9, font: bold }); page.drawText('PU', { x: 350, y, size: 9, font: bold }); page.drawText('Total', { x: 455, y, size: 9, font: bold }); y -= 18;
    for (const line of tournee.lignes || []) {
      page.drawText(String(line.designation || '-').slice(0, 34), { x: 45, y, size: 9, font }); page.drawText(String(line.quantiteChargee), { x: 280, y, size: 9, font }); page.drawText(money(line.prixUnitaireFacture), { x: 350, y, size: 9, font }); page.drawText(money(Number(line.quantiteChargee) * Number(line.prixUnitaireFacture)), { x: 455, y, size: 9, font }); y -= 17;
    }
    y -= 5; text(`Total quantité : ${tournee.totalQuantiteChargee || 0}`, 45, 10, true); text(`Valeur totale : ${money(tournee.totalValeurChargee)}`, 45, 11, true); y -= 20; text('Signatures', 45, 11, true);
    page.drawText('Magasinier : ____________________', { x: 45, y, size: 9, font }); page.drawText('Commercial : ____________________', { x: 300, y, size: 9, font }); y -= 35; page.drawText('Responsable : ____________________', { x: 45, y, size: 9, font });
  ...
  create(@Req() req: any, @Body() data: CreateTourneeWorkflowDto) { const { tenantId, depotId } = this.scope(req); return this.service.create(tenantId, depotId, data); }

  @Patch(':id')
  @RequirePermission('tournees', 'write')
  update(@Req() req: any, @Param('id') id: string, @Body() data: UpdateTourneeWorkflowDto) { const { tenantId, depotId } = this.scope(req); return this.service.update(tenantId, depotId, id, data); }

  @Post(':id/lignes')
  @RequirePermission('tournees', 'write')
  addLine(@Req() req: any, @Param('id') id: string, @Body() data: AddTourneeWorkflowLineDto) { const { tenantId, depotId } = this.scope(req); return this.service.addLine(tenantId, depotId, id, data); }
  ...
  @RequirePermission('tournees', 'write')
  reconcile(@Req() req: any, @Param('id') id: string, @Body() data: ReconcileTourneeDto) { const { tenantId, depotId } = this.scope(req); return this.service.reconcile(tenantId, depotId, id, data); }
