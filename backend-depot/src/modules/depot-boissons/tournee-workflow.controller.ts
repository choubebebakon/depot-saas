...
import {
  AddTourneeWorkflowLineDto,
  CreateTourneeWorkflowDto,
  ReconcileTourneeDto,
  UpdateTourneeWorkflowDto,
} from './dto/tournee-workflow.dto';
...
export class TourneeWorkflowController {
  ...
  private scope(req: any) {
    ...
    if (!userTenantId || !scopeTenantId || userTenantId !== scopeTenantId)
  ...
  async bonSortie(
    ...
    // pdf-lib encode en WinAnsi : les tirets longs (—), les espaces fines
    // insécables (U+202F produits par toLocaleString('fr-FR')) et les guillemets
    // typographiques ne sont pas représentables : leur passage faisait échouer
    // la génération (HTTP 500). On neutralise ces caractères et on formate les
    // montants avec des espaces standards.
    ...
    const formatNumber = (n: unknown) =>
      Math.round(Number(n) || 0)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ' ');
    const money = (n: number) => `${formatNumber(n)} FCFA`;
    let y = 800;
    ...
    text(
      `Date planifiée : ${new Date(tournee.datePlanifiee).toLocaleString('fr-FR')}`,
    );
    text(
      `Commercial : ${tournee.commercial?.nom || tournee.commercial?.email || '-'}`,
    );
    ...
    for (const line of tournee.lignes || []) {
      page.drawText(sanitize(String(line.designation || '-')).slice(0, 34), {
        x: 45,
        y,
        size: 9,
        font,
      ...
      page.drawText(money(line.prixUnitaireFacture), {
        x: 350,
        y,
        size: 9,
        font,
      ...
      page.drawText(
        money(Number(line.quantiteChargee) * Number(line.prixUnitaireFacture)),
        { x: 455, y, size: 9, font },
      );
      y -= 17;
    }
    ...
    page.drawText('Magasinier : ____________________', {
      x: 45,
      y,
      size: 9,
      font,
    });
    page.drawText('Commercial : ____________________', {
      x: 300,
      y,
      size: 9,
      font,
    ...
    y -= 35;
    page.drawText('Responsable : ____________________', {
      x: 45,
      y,
      size: 9,
      font,
    ...
    res.set({
      ...
      'Content-Disposition': `inline; filename="bon-sortie-${tournee.reference}.pdf"`,
  ...
  @Get(':id')
  @RequirePermission('tournees', 'read')
  get(@Req() req: any, @Param('id') id: string): Promise<any> {
    const { tenantId, depotId } = this.scope(req);
  ...
  create(@Req() req: any, @Body() data: CreateTourneeWorkflowDto) {
    const { tenantId, depotId } = this.scope(req);
  ...
  update(
    @Req() req: any,
    @Param('id') id: string,
    @Body() data: UpdateTourneeWorkflowDto,
  ) {
    const { tenantId, depotId } = this.scope(req);
  ...
  addLine(
    @Req() req: any,
    @Param('id') id: string,
    @Body() data: AddTourneeWorkflowLineDto,
  ) {
    const { tenantId, depotId } = this.scope(req);
