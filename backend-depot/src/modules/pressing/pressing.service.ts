...
export class PressingService {
  ...
  async findAllTickets(tenantId: string, pagination: PaginationDto) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.ticketPressing.findMany({
        where,
        skip,
        take: limit,
        include: { client: true },
  ...
  async getTicketDetail(id: string, tenantId: string) {
    return this.prisma.ticketPressing.findFirst({
      where: { id, tenantId },
      include: { vetements: true, client: true },
    });
  }

  async updateTicketStatut(
    id: string,
    tenantId: string,
    data: UpdateTicketStatutDto,
  ) {
    return this.prisma.ticketPressing.update({
      where: { id, tenantId },
  ...
  async getTicketsByClient(clientId: string, tenantId: string) {
    return this.prisma.ticketPressing.findMany({
  ...
  async marquerCommeRetire(id: string, tenantId: string) {
    return this.prisma.ticketPressing.update({
      where: { id, tenantId },
      data: { statut: 'RETIRE', dateRetirée: new Date() },
    });
  }

  async getStats(tenantId: string) {
    const [enCours, prets, recettes] = await Promise.all([
      this.prisma.ticketPressing.count({
        where: { tenantId, statut: { in: ['RECU', 'EN_TRAITEMENT'] } },
      }),
      this.prisma.ticketPressing.count({ where: { tenantId, statut: 'PRET' } }),
      this.prisma.ticketPressing.aggregate({
        where: {
          tenantId,
          dateDepot: { gte: new Date(new Date().setHours(0, 0, 0, 0)) },
        },
