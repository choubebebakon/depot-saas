...
export class TransportService {
  ...
  async findAllVehicules(tenantId: string, pagination: PaginationDto) {
    ...
    if (pagination.search) {
      where.OR = [
        {
          immatriculation: { contains: pagination.search, mode: 'insensitive' },
        },
        { marque: { contains: pagination.search, mode: 'insensitive' } },
      ];
    }
    const [data, total] = await Promise.all([
      this.prisma.vehiculeTransport.findMany({
        where,
        skip,
        take: limit,
  ...
  async findAllColis(tenantId: string, pagination: PaginationDto) {
    ...
    if (pagination.search) {
      where.OR = [
        ...
        { destinataire: { contains: pagination.search, mode: 'insensitive' } },
        { villeDest: { contains: pagination.search, mode: 'insensitive' } },
      ];
    }
    const [data, total] = await Promise.all([
      this.prisma.colis.findMany({
        where,
        skip,
        take: limit,
        include: { expediteur: true, trajet: true },
        orderBy: { createdAt: 'desc' },
      }),
  ...
  async createColis(tenantId: string, dto: CreateColisDto) {
    ...
    return this.prisma.colis.create({
      data: { ...dto, tenantId, reference: ref },
      include: { expediteur: true },
    });
  }

  async updateColisStatut(id: string, statut: string) {
    return this.prisma.colis.update({
      where: { id },
  ...
  async findAllTrajets(tenantId: string, pagination: PaginationDto) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.trajet.findMany({
        ...
        skip,
        take: limit,
        include: { vehicule: true, colis: true },
  ...
  async createTrajet(tenantId: string, dto: CreateTrajetDto) {
    return this.prisma.trajet.create({
      data: { ...dto, tenantId, dateDepart: new Date(dto.dateDepart) },
  ...
  async getStats(tenantId: string) {
    const vehicules = await this.prisma.vehiculeTransport.count({
      where: { tenantId },
    });
    const disponibles = await this.prisma.vehiculeTransport.count({
      where: { tenantId, disponible: true },
    });
    const colisEnCours = await this.prisma.colis.count({
      where: { tenantId, statut: { in: ['ENREGISTRE', 'EN_TRANSIT'] } },
    });
    const colisLivre = await this.prisma.colis.count({
      where: { tenantId, statut: 'LIVRE' },
    });
    const trajetsPlanifies = await this.prisma.trajet.count({
      where: { tenantId, statut: { in: ['PLANIFIE', 'EN_COURS'] } },
    });
    return {
      vehicules,
      disponibles,
