...
export class FournisseursService {
    ...
    async createFournisseur(dto: any) { // 'any' permet d'accepter tous les champs du DTO
        if (!dto.nom) {
    ...
    async updateFournisseur(tenantId: string, id: string, dto: any) {
        ...
        return this.prisma.fournisseur.update({
            ...
            data: {
                ...
                telephone: dto.telephone !== undefined ? dto.telephone : fournisseur.telephone,
                ...
                soldeInitial: dto.soldeInitial !== undefined ? Number(dto.soldeInitial) : fournisseur.soldeInitial,
            }
        });
    }
    ...
    async deleteFournisseur(tenantId: string, id: string) {
        ...
        if (slots._count.receptions > 0) {
            throw new BadRequestException('Impossible de supprimer ce fournisseur car des réceptions y sont rattachées.');
        }
    ...
    async createReception(dto: CreateReceptionDto) {
        return await this.prisma.$transaction(async (tx) => {
            ...
            const count = await tx.receptionFournisseur.count({ where: { tenantId: dto.tenantId } });
    ...
    async statsFournisseurs(tenantId: string) {
        ...
        const receptions = await this.prisma.receptionFournisseur.count({ where: { tenantId } });
        return {
